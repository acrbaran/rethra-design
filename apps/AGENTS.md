...
- `apps/closure`: independently distributable Rethra Design Closure content. During the cold-start phase it owns only a Web/daemon-independent lifecycle fixture and component contribution; do not add shell, Store, channel, or generation policy here.
...
pnpm --filter @rethra-design/packaged typecheck
pnpm --filter @rethra-design/packaged build
```
