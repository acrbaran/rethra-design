...
import type { ProcessSnapshot, StopProcessesResult } from "@rethra-design/platform";
...
async function createMacInstalledOuter(): Promise<{
  ...
  if (process.platform === "win32") {
    const launchPath = "/Applications/Rethra Design.app";
    const executablePath = `${launchPath}/Contents/MacOS/Rethra Design`;
    return {
      executablePath,
  ...
  roots.push(root);
  const launchPath = join(root, "Rethra Design.app");
  const executablePath = join(launchPath, "Contents", "MacOS", "Rethra Design");
...
async function createWindowsInstalledOuter(
  executableName = "Rethra Design.exe",
): Promise<{
  executablePath: string;
  inspectInstalledOuterPath?: InspectInstalledOuterPath;
  launchPath: string;
  ...
  if (process.platform !== "win32") {
    const executablePath = `C:\\Program Files\\Rethra Design\\${executableName}`;
...
describe("createObsoleteInstalledOuterRetirement", () => {
  it("stops only the exact installed outer root and its descendants", async () => {
    ...
    const snapshots = [
      ...
      snapshot(102, 101, `${launchPath}/Contents/Frameworks/Rethra Design Helper.app/Contents/MacOS/Rethra Design Helper`),
      snapshot(103, 102, "helper-child"),
      snapshot(104, 1, `${executablePath} Helper`),
      snapshot(105, 1, "/unrelated/Rethra Design"),
      snapshot(900, 1, "/payload/Rethra Design.app/Contents/MacOS/Rethra Design"),
    ];
    const stopProcesses = stopMock();
    const logger = { info: vi.fn(), warn: vi.fn() };
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/payload/Rethra Design.app/Contents/MacOS/Rethra Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger,
      payloadDesktopProcess: true,
      payloadExecutablePath: "/payload/Rethra Design.app/Contents/MacOS/Rethra Design",
      platform: "darwin",
    }, {
      inspectInstalledOuterPath,
  ...
  it("does nothing outside an exact payload desktop process", async () => {
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/installed/Rethra Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
  ...
  it("rejects a symlinked install executable before enumerating processes", async () => {
    ...
    if (process.platform === "win32") {
      launchPath = "/Applications/Rethra Design.app";
      const executablePath = `${launchPath}/Contents/MacOS/Rethra Design`;
      ...
      roots.push(root);
      launchPath = join(root, "Rethra Design.app");
      ...
      await symlink(target, join(executableDirectory, "Rethra Design"));
    }
    const listProcessSnapshots = vi.fn(async () => []);
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "/payload/Rethra Design",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger: { info: vi.fn(), warn: vi.fn() },
      payloadDesktopProcess: true,
      payloadExecutablePath: "/payload/Rethra Design",
      platform: "darwin",
    }, {
      inspectInstalledOuterPath,
  ...
  it("refuses to stop an outer tree that contains the current payload", async () => {
    ...
    const retire = createObsoleteInstalledOuterRetirement({
      ...
      listProcessSnapshots: async () => [
        ...
        snapshot(900, 800, "/payload/Rethra Design"),
...
describe("Windows obsolete installed outer retirement", () => {
  it("stops the revalidated exact installed outer root and its current descendants", async () => {
    ...
    const firstSnapshots = [
      ...
      snapshot(202, 201, "Rethra Design.exe --type=gpu-process"),
      snapshot(203, 202, "Rethra Design.exe --type=utility"),
      snapshot(204, 1, `"${executablePath}" od://project/123`),
      snapshot(205, 1, `${executablePath}.old`),
      snapshot(900, 1, "C:\\payload\\Rethra Design.exe"),
    ];
    const secondSnapshots = [
      snapshot(201, 1, `"${executablePath}"`),
      snapshot(202, 201, "Rethra Design.exe --type=gpu-process"),
      snapshot(206, 202, "Rethra Design.exe --type=renderer"),
      snapshot(204, 1, `"${executablePath}" od://project/123`),
      snapshot(205, 1, `${executablePath}.old`),
      snapshot(900, 1, "C:\\payload\\Rethra Design.exe"),
    ];
    ...
    const logger = { info: vi.fn(), warn: vi.fn() };
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\PAYLOAD\\RETHRA DESIGN.EXE",
      currentPid: 900,
      installedLaunchPath: launchPath,
      logger,
      payloadDesktopProcess: true,
      payloadExecutablePath: "c:\\payload\\Rethra Design.exe",
      platform: "win32",
    }, { inspectInstalledOuterPath, listProcessSnapshots, stopProcesses });
  ...
  it("rejects installed executables whose basename does not match the payload", async () => {
    const { inspectInstalledOuterPath, launchPath } = await createWindowsInstalledOuter("Not Rethra Design.exe");
    const listProcessSnapshots = vi.fn(async () => []);
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\Rethra Design.exe",
  ...
  it("refuses a revalidated outer tree that contains the current payload", async () => {
    ...
    const snapshots = [
      ...
      snapshot(900, 800, "C:\\payload\\Rethra Design.exe"),
    ];
  ...
  it("rejects a symlinked Windows executable before enumerating processes", async () => {
    const executablePath = "C:\\Program Files\\Rethra Design\\Rethra Design.exe";
    const listProcessSnapshots = vi.fn(async () => []);
    const stopProcesses = stopMock();
    const retire = createObsoleteInstalledOuterRetirement({
      currentExecutablePath: "C:\\payload\\Rethra Design.exe",
