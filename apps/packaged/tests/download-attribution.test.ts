...
describe("download attribution token extraction", () => {
  it("extracts the token from the distributor path segment", () => {
    expect(
      extractDownloadAttributionTokenFromUrl(
        "https://download.rethra-design.invalid/mac/arm64/oddl_Abc12345/Rethra-Design.dmg",
      ),
    ).toBe("oddl_Abc12345");
    expect(
      extractDownloadAttributionTokenFromUrl(
        "https://download.rethra-design.invalid/windows/x64/token-123456/Open%20Design-setup.exe",
      ),
    ).toBe("token-123456");
  });
  ...
  it("does not treat GitHub release paths as attributed download URLs", () => {
    expect(
      extractDownloadAttributionTokenFromUrl(
        "https://github.com/acrbaran/rethra-design/releases/download/rethra-design-v1/Rethra-Design-mac-arm64.dmg",
      ),
    ).toBeNull();
  });
});
...
describe("packaged download attribution lifecycle", () => {
  it("does not re-submit a terminal installer observation on the next launch", async () => {
    ...
    const rawUrl = "https://download.rethra-design.invalid/windows/x64/oddl_terminal_123/Rethra-Design-setup.exe";
