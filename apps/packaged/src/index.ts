...
} from "@rethra-design/desktop/main";
...
async function main(): Promise<void> {
  ...
  if (headlessRequest.mcpInstallAgent != null) {
    ...
    if (existingOwner != null) {
      throw new Error(
        `Cannot install MCP while the existing ${existingOwner.stamp.mode} desktop runtime has no healthy daemon. Quit Rethra Design and retry.`,
      );
    }
  }
  ...
  const { runDesktopMain } = await import("@rethra-design/desktop/main");
