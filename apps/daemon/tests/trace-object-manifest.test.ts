...
describe('buildTraceObjectManifests', () => {
  ...
  it('splits relay uploads when combined base64 JSON would exceed the worker batch cap', async () => {
    ...
    const fetchSpy = vi.fn(async (url: string, init: RequestInit) => {
      ...
      expect(init.headers).toMatchObject({
        'X-Rethra-Design-Telemetry': 'object-ingestion-v1',
      });
    ...
    const manifests = await buildTraceObjectManifests({
      ...
      env: {
        NODE_ENV: 'test',
        RETHRA_DESIGN_OBJECT_RELAY_URL: 'https://telemetry.rethra-design.invalid/api/objects/batch',
        RETHRA_DESIGN_OBJECT_MAX_BYTES: '1024',
        RETHRA_DESIGN_OBJECT_BATCH_MAX_BYTES: '2300',
      },
    ...
    expect(manifests?.artifactManifest?.map((entry) => entry.stored_in_rethra_design))
      .toEqual([true, true]);
  });
  ...
  it('derives the object relay endpoint from the telemetry relay endpoint', async () => {
    ...
    const fetchSpy = vi.fn(async (url: string, init: RequestInit) => {
      ...
      expect(url).toBe('https://telemetry.rethra-design.invalid/api/objects/batch');
      expect((parsed as unknown as { upload_token: string }).upload_token).toBe('upload-token');
      expect(parsed.objects).toHaveLength(1);
      return new Response(
        JSON.stringify({
    ...
    expect(fetchSpy).toHaveBeenCalledTimes(2);
    expect(fetchSpy.mock.calls[0]![0]).toBe('https://telemetry.rethra-design.invalid/api/objects/authorize');
    expect(fetchSpy.mock.calls[1]![0]).toBe('https://telemetry.rethra-design.invalid/api/objects/batch');
  ...
  it('skips over-limit project files before loading their contents', async () => {
    ...
    const manifests = await buildTraceObjectManifests({
      ...
      env: {
        NODE_ENV: 'test',
        RETHRA_DESIGN_OBJECT_RELAY_URL: 'https://telemetry.rethra-design.invalid/api/objects/batch',
        RETHRA_DESIGN_OBJECT_MAX_BYTES: '8',
      },
      now: () => new Date('2026-06-08T00:00:00.000Z'),
    });
