...
describe('resolveVelaConsoleOrigin', () => {
  ...
  // entry off `vela.powerformer.net` onto `rethra-design.powerformer.net/cloud`
  ...
  it('resolves the public console origin for a runtime profile selection', () => {
    expect(resolveVelaConsoleOrigin({}, { RETHRA_DESIGN_AMR_PROFILE: 'test' })).toBe(
      'https://rethra-design.powerformer.net/cloud',
    );
    expect(resolveVelaConsoleOrigin({}, { RETHRA_DESIGN_AMR_PROFILE: 'prod' })).toBe(
      'https://rethra-design.invalid/cloud',
    );
    expect(resolveVelaConsoleOrigin({}, { RETHRA_DESIGN_AMR_PROFILE: 'local' })).toBe(
      'http://localhost:5173',
    );
  });
  ...
  it('has no public origin for the internal feature-test profile', () => {
    expect(
      resolveVelaConsoleOrigin({}, { RETHRA_DESIGN_AMR_PROFILE: 'feature-test' }),
    ).toBeUndefined();
  });
