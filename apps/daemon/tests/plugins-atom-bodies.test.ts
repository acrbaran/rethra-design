...
// pure renderer in @rethra-design/contracts then assembles the stage
...
import { renderActiveStageBlock, renderActiveStageBlocks } from '@rethra-design/contracts';
...
beforeEach(async () => {
  ...
  await writeFile(path.join(atomA, 'rethra-design.json'), SAMPLE_MANIFEST('discovery-question-form'));
  await writeFile(path.join(atomA, 'SKILL.md'), SAMPLE_SKILL('discovery-question-form', 'Ask the user about audience.'));
  await writeFile(path.join(atomB, 'rethra-design.json'), SAMPLE_MANIFEST('todo-write'));
