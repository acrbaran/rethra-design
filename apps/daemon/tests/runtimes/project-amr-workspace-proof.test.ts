...
describe('AMR persisted project Workspace scope', () => {
  it('pins each initial spawn and retry to its own persisted A/B binding', async () => {
    ...
    const initialA = await rethraDesignAmrTraceEnvForRun({
      agentId: 'amr',
      runId: 'run-a',
      runAttempt: 0,
      projectId: 'project-a',
    ...
    const retryA = await rethraDesignAmrTraceEnvForRun({
      agentId: 'amr',
      runId: 'run-a',
      runAttempt: 1,
      projectId: 'project-a',
    ...
    const initialB = await rethraDesignAmrTraceEnvForRun({
      agentId: 'amr',
      runId: 'run-b',
      runAttempt: 0,
      projectId: 'project-b',
    ...
    const retryB = await rethraDesignAmrTraceEnvForRun({
      agentId: 'amr',
      runId: 'run-b',
      runAttempt: 1,
      projectId: 'project-b',
    ...
    expect(initialA.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-a');
    expect(retryA.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-a');
    expect(initialB.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-b');
    expect(retryB.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-b');
  });
  ...
  it('keeps a run on its authorized Workspace when the project is rebound before retry', async () => {
    ...
    const initial = await rethraDesignAmrTraceEnvForRun({
      agentId: 'amr',
      runId: 'run-a',
      runAttempt: 0,
      projectId: 'project-a',
    ...
    const retry = await rethraDesignAmrTraceEnvForRun({
      agentId: 'amr',
      runId: 'run-a',
      runAttempt: 1,
      projectId: 'project-a',
    ...
    expect(initial.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-a');
    expect(retry.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-a');
  });
  ...
  it('does not expose membership/current/directory inputs to the billing-scope resolver', async () => {
    expectTypeOf<NonNullable<Parameters<typeof rethraDesignAmrTraceEnvForRun>[1]>>()
      .toEqualTypeOf<{
  ...
  it('passes a persisted Personal Workspace id on initial spawn and retry', async () => {
    ...
    expect(initial.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-personal');
    expect(retry.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-personal');
  });
  ...
  it('keeps an unbound local AMR project account-scoped on initial spawn and retry', async () => {
    ...
    expect(initial).not.toHaveProperty('RETHRA_DESIGN_WORKSPACE_ID');
    expect(retry).not.toHaveProperty('RETHRA_DESIGN_WORKSPACE_ID');
  ...
  it.each(['claude', 'codex', 'opencode', 'byok-opencode'])(
    ...
    async (agentId) => {
      ...
      const outcomes: ProjectWorkspaceScopeOutcome[] = [];
      const env = await rethraDesignAmrTraceEnvForRun({
        agentId,
        runId: `run-${agentId}`,
        runAttempt: 0,
        projectId: 'project-a',
      ...
      expect(env).not.toHaveProperty('RETHRA_DESIGN_WORKSPACE_ID');
