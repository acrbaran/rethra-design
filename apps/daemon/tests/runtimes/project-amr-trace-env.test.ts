...
describe('rethraDesignAmrTraceEnvForRun', () => {
  ...
  it('does not resolve project scope for a non-AMR runtime', async () => {
    ...
    await expect(rethraDesignAmrTraceEnvForRun({
      agentId: 'claude',
      runId: 'run-claude',
      runAttempt: 0,
      projectId: 'project-a',
  ...
  it('carries the persisted Team binding into the final AMR spawn environment', async () => {
    ...
    expect(env).toMatchObject({
      RETHRA_DESIGN_RUN_ID: 'run-a',
      RETHRA_DESIGN_SESSION_ID: 'conversation-a',
      RETHRA_DESIGN_WORKSPACE_ID: 'workspace-a',
    });
  });
  ...
  it('uses the Team Workspace for a private draft bound to that Team', async () => {
    ...
    expect(env.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-team');
  });
  ...
  it('passes a persisted Personal Workspace explicitly instead of treating it as unscoped', async () => {
    ...
    expect(env.RETHRA_DESIGN_WORKSPACE_ID).toBe('workspace-personal');
  });
  ...
  it('spawns a truly unbound local project on the signed-in account wallet', async () => {
    ...
    expect(env.RETHRA_DESIGN_RUN_ID).toBe('run-legacy');
