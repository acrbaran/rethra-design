...
This package captures a source-backed RethraDesign design system for a desktop AI chat workspace. It includes reusable rules, token CSS, focused review previews, preserved assets, preserved fonts, and an applied UI kit.
...
- DESIGN.md is the canonical RethraDesign rules document.
...
Use this skill when creating RethraDesign artifacts that should match the Cherry Studio desktop AI chat workspace.
...
description: Use this skill when creating RethraDesign artifacts that should match the Cherry Studio desktop AI chat workspace.
user-invocable: true
---
...
This package is intended for reusable RethraDesign work, so future agents should keep the output grounded in captured evidence, use preserved assets instead of redrawing brand marks, keep app surfaces compact, and inspect preview cards before introducing any new component pattern. Treat it as a focused product design kit, not a generic style summary.
...
function auditComponent(componentName: string): string {
  ...
  { id: 'review', label: '${componentName} review state', detail: 'Reusable review surface for future RethraDesign projects.' },
];
...
`);
  ...
  it('can audit an external Claude Design reference package without DESIGN.md', async () => {
    ...
    expect(JSON.parse(stdoutOutput.join(''))).toMatchObject({
      ...
      errors: [],
      warnings: expect.arrayContaining([
        expect.objectContaining({ code: 'missing_rethra_design_rules', path: 'DESIGN.md' }),
      ]),
    });
