...
const META = {
  pluginId:          'rethra-design/sample-plugin',
  pluginVersion:     '1.0.0',
  pluginTitle:       'Sample Plugin',
  pluginDescription: 'A fixture for the publish flow.',
  repoUrl:           'https://github.com/rethra-design/sample-plugin',
};
...
describe('buildPublishLink', () => {
  ...
  it('builds a github-issue URL for anthropics/skills with title + body', () => {
    ...
    expect(link.prBody).toContain('https://github.com/rethra-design/sample-plugin');
  });
  ...
  it('points at skills.sh + the npx skills add command (no PR form there)', () => {
    ...
    expect(link.prBody).toContain('npx skills add rethra-design/sample-plugin');
  });
...
describe('upsertMarketplaceJsonEntry', () => {
  it('adds a namespaced plugin entry with a reproducible github source', () => {
    ...
    expect(outcome.entry).toMatchObject({
      name: 'rethra-design/sample-plugin',
  ...
  it('updates existing entries and preserves unrelated catalog metadata', () => {
    const outcome = upsertMarketplaceJsonEntry({
      ...
      manifest: {
        ...
        plugins: [
          {
            name: 'rethra-design/sample-plugin',
            source: 'github:rethra-design/sample-plugin@old',
            version: '0.9.0',
            tags: ['kept'],
          },
        ],
      ...
      meta: {
        ...META,
        pluginVersion: '1.1.0',
        repoUrl: 'https://github.com/rethra-design/sample-plugin/tree/main/plugins/sample',
      },
    });
    ...
    expect(outcome.manifest.plugins[0]).toMatchObject({
      name: 'rethra-design/sample-plugin',
      source: 'github:rethra-design/sample-plugin@main/plugins/sample',
      version: '1.1.0',
      tags: ['kept'],
    });
  });
