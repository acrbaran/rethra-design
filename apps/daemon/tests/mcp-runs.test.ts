...
import { buildProjectRawFileUrl } from '@rethra-design/contracts';
...
describe('public MCP discovery + generation tools', () => {
  ...
  it('get_run returns a recharge link and same-request resume instructions', async () => {
    ...
    expect(parsed).toMatchObject({
      status: 'failed',
      failureAction: 'recharge',
      rechargeUrl: 'https://rethra-design.invalid/amr/dashboard?source=rethra_design',
    });
  ...
  // Discovery-stage / clarifying-question fallback: when RethraDesign's
  ...
  // #2: MCP-driven projects skip RethraDesign's interactive discovery
