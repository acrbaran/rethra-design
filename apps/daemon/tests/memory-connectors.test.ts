  ...
  return {
    ...
    execute: async (request: { connectorId: string; toolName: string; input: unknown }) => {
      ...
      return {
        ...
        outputSummary: 'Found RethraDesign design memory notes in Notion.',
        output: {
          pages: [
            {
              title: 'RethraDesign memory plan',
              text: 'RethraDesign connector memory should collect design preferences, UI decisions, and visual references from Notion.',
            },
          ],
        },
      };
          ...
          message: {
            content: JSON.stringify({
              entries: [
                {
                  type: 'project',
                  name: 'RethraDesign design memory',
                  description: 'Connector memories should stay design-related',
                  body: 'RethraDesign connector memories should focus on design preferences, UI decisions, and visual references rather than generic app activity.',
                },
              ],
            }),
          },
  ...
  it('reads connected app context and returns reviewable memory suggestions', async () => {
    ...
    expect(result.suggestions).toEqual([
      expect.objectContaining({
        id: 'project_rethradesign_design_memory_1',
        type: 'project',
        name: 'RethraDesign design memory',
    ...
    await expect(
      readMemoryEntry(dataDir, 'project_rethradesign_design_memory'),
    ).resolves.toBeNull();
  ...
  it('tries additional connector queries when the first read is empty', async () => {
            ...
            pages: [
              {
                ...
                text: 'RethraDesign should summarize connector findings when no durable memory is obvious.',
              },
            ],
          },
        };
    ...
    expect(executeCalls[0]).toEqual(expect.objectContaining({
      input: expect.objectContaining({
        query: '设计思路 设计偏好 UI UX 视觉风格 品牌 logo 设计系统 RethraDesign',
      }),
    }));
            ...
            pages: [
              {
                ...
                text: 'RethraDesign should read the currently available Notion search tool.',
              },
            ],
          },
        };
      ...
      execute: async (request: { connectorId: string; toolName: string; input: unknown }) => ({
        ...
        output: {
          pages: [
            {
              title: 'Memory source notes',
              text: 'RethraDesign should summarize connector findings before saving them as memory.',
            },
          ],
        },
      }),
              ...
              entries: [
                {
                  ...
                  body: 'RethraDesign read GitHub via List notifications. Summary: Found 5 readable items from GitHub. Save this if it should be reused as context in future chats.',
                },
                {
                  type: 'feedback',
                  name: 'UI density preference',
                  description: 'The user prefers denser design interfaces',
                  body: 'The user prefers RethraDesign UI to use higher information density with clear hierarchy instead of spacious marketing-style cards.',
                },
              ],
            }),
          },
      ...
      entries: [
        {
          ...
          body: 'RethraDesign connector memory extraction should use the same Claude Code Local CLI selected for chat when the memory model is set to same as chat.',
        },
      ],
    }));
      ...
      entries: [
        {
          ...
          body: 'RethraDesign connector memory extraction should use the same OpenCode Local CLI selected for chat instead of falling back to an OpenAI API key.',
        },
      ],
    }));
      ...
      entries: [
        {
          ...
          body: 'RethraDesign connector memory extraction should use the same Codex Local CLI selected for chat instead of falling back to an OpenAI API key.',
        },
      ],
    }));
  ...
  item: {
    ...
    text: JSON.stringify({
      entries: [{
        ...
        body: 'RethraDesign connector memory extraction should pass the compacted prompt to Codex stdin and parse the JSON event stream response.'
      }]
    })
  }
}) + '\\n');
`,
    ...
    try {
      ...
      expect(captured.stdin).toContain('RethraDesign connector memory should collect design preferences');
    } finally {
  ...
  part: {
    ...
    text: JSON.stringify({
      entries: [{
        ...
        body: 'RethraDesign connector memory extraction should pass the compacted prompt to OpenCode on stdin and parse the JSON event stream response.'
      }]
    })
  }
}) + '\\n');
`,
  ...
  it('reads connected app context and saves connector-sourced memory', async () => {
    ...
    expect(result.changed[0]).toMatchObject({
      id: 'project_rethradesign_design_memory',
      type: 'project',
      name: 'RethraDesign design memory',
    });

    const stored = await readMemoryEntry(dataDir, 'project_rethradesign_design_memory');
