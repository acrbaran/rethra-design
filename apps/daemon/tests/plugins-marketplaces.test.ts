...
const VALID_MANIFEST = JSON.stringify({
  ...
  plugins: [
    { name: 'sample-plugin', source: 'github:rethra-design/sample-plugin', version: '0.1.0' },
  ],
});
...
describe('marketplaces', () => {
  ...
  it('normalizes public marketplace urls to the canonical raw registry', async () => {
    ...
    const result = await addMarketplace(db, {
      url: 'https://rethra-design.invalid/marketplace/community/rethra-design-marketplace.json',
      fetcher: async (url) => {
        seenUrls.push(url);
        return {
          ok: true,
  ...
  it('requires a raw rethra-design-marketplace.json document, not a GitHub tree page', async () => {
    const result = await addMarketplace(db, {
      url: 'https://github.com/acrbaran/rethra-design/tree/garnet-hemisphere/plugins/registry/community',
  ...
  it('refresh re-fetches and updates refreshed_at', async () => {
    ...
    updatedManifest.plugins.push({
      name: 'new-plugin',
      source: 'github:rethra-design/new-plugin',
      version: '0.2.0',
    });
  ...
  it('seeds the checked-in default community registry as restricted and resolvable', async () => {
    const communityManifest = await readFile(
      new URL('../../../plugins/registry/community/rethra-design-marketplace.json', import.meta.url),
      'utf8',
    );
    ...
    expect(resolved?.source).toMatch(
      /^github:nexu-io\/rethra-design(?:@[^/]+)?\/plugins\/community\/registry-starter$/,
    );
  });

  it('keeps the checked-in official registry populated from bundled plugins', async () => {
    const officialManifestText = await readFile(
      new URL('../../../plugins/registry/official/rethra-design-marketplace.json', import.meta.url),
      'utf8',
    );
    ...
    expect(officialManifest.plugins?.some((plugin) => plugin.name === 'rethra-design/build-test')).toBe(true);
    expect(officialManifest.plugins?.every((plugin) =>
      /^github:nexu-io\/rethra-design(?:@[^/]+)?\/plugins\/_official\//.test(plugin.source ?? ''),
    )).toBe(true);
    ...
    const resolved = resolvePluginInMarketplaces(db, 'rethra-design/build-test');
    expect(resolved?.marketplaceId).toBe('official');
    expect(resolved?.marketplaceTrust).toBe('official');
  });

  it('keeps example-rethra-design-landing catalog version aligned with the bundled manifest', async () => {
    ...
    const sourceManifest = JSON.parse(await readFile(
      new URL('../../../plugins/_official/examples/rethra-design-landing/rethra-design.json', import.meta.url),
      'utf8',
    )) as { version?: string };
    const catalogEntry = officialManifest.plugins?.find(
      (plugin) => plugin.name === 'rethra-design/example-rethra-design-landing',
    );
  ...
  it('keeps checked-in community registry entries pointed at source folders that can pack', async () => {
    ...
    const sourceSubpath = entry!.source!.replace(/^github:nexu-io\/rethra-design(?:@[^/]+)?\//, '');
    expect(sourceSubpath).toBe('plugins/community/registry-starter');

    const sourceManifest = await readFile(
      new URL(`../../../${sourceSubpath}/rethra-design.json`, import.meta.url),
      'utf8',
    );
    expect(JSON.parse(sourceManifest)).toMatchObject({
      name: 'community-registry-starter',
      plugin: {
        repo: expect.stringContaining('github.com/acrbaran/rethra-design'),
      },
    });
  });
});
...
describe('resolvePluginInMarketplaces', () => {
  it('returns the canonical source string for a known plugin name', async () => {
    ...
    expect(resolved!.source).toBe('github:rethra-design/sample-plugin');
