...
// Regression coverage for the editorial-collage → rethra-design-landing rename.
...
beforeAll(async () => {
  ...
  await mkdir(path.join(skillsRoot, 'rethra-design-landing'), { recursive: true });
  await writeFile(
    path.join(skillsRoot, 'rethra-design-landing', 'SKILL.md'),
    '---\nname: rethra-design-landing\ndescription: Atelier Zero landing.\n---\n\nbody\n',
    'utf8',
  );
  await mkdir(path.join(skillsRoot, 'rethra-design-landing-deck'), {
    recursive: true,
  });
  await writeFile(
    path.join(skillsRoot, 'rethra-design-landing-deck', 'SKILL.md'),
...
describe('SKILL_ID_ALIASES', () => {
  it('maps the editorial-collage rename to its current canonical id', () => {
    expect(SKILL_ID_ALIASES['editorial-collage']).toBe('rethra-design-landing');
...
describe('resolveSkillId', () => {
  it('forwards deprecated ids to their canonical replacement', () => {
    expect(resolveSkillId('editorial-collage')).toBe('rethra-design-landing');
...
describe('findSkillById', () => {
  it('resolves a project saved with the old editorial-collage id to the renamed skill', async () => {
    ...
    expect(skill.id).toBe('rethra-design-landing');
  ...
  it('resolves a project saved with the old editorial-collage-deck id to the renamed deck skill', async () => {
    ...
    expect(skill.id).toBe('rethra-design-landing-deck');
  });

  it('still resolves current ids exactly', async () => {
    const skills = await listSkills(skillsRoot);
    expect(findSkillById(skills, 'rethra-design-landing')?.id).toBe(
  ...
  it('returns undefined for unknown ids and missing inputs', async () => {
    ...
    expect(findSkillById(null, 'rethra-design-landing')).toBeUndefined();
  });
});
