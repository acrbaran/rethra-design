...
const SPEC: McpLaunchSpec = {
  ...
  args: ['/opt/rethra-design/cli.js', 'mcp', '--daemon-url', 'http://127.0.0.1:7456'],
  env: { OD_DATA_DIR: '/home/u/.rethra-design' },
};
...
describe('CLI-driven agents', () => {
  it('claude registers via `claude mcp add --scope user` with env flags', () => {
    ...
    expect(plan.addArgv).toEqual([
      'mcp', 'add', '--scope', 'user',
      'rethra-design',
      '-e', 'OD_DATA_DIR=/home/u/.rethra-design',
      '--',
      SPEC.command, ...SPEC.args,
    ]);
    expect(plan.removeArgv).toEqual(['mcp', 'remove', '--scope', 'user', 'rethra-design']);
  });
  ...
  it('codex uses --env and a -- separator', () => {
    ...
    expect(plan.addArgv).toContain('OD_DATA_DIR=/home/u/.rethra-design');
  ...
  it('reasonix uses its native mcp add/remove/get commands', () => {
    ...
    expect(plan.addArgv).toEqual([
      'mcp', 'add', 'rethra-design',
      '--env', 'OD_DATA_DIR=/home/u/.rethra-design',
      SPEC.command, ...SPEC.args,
    ]);
    expect(plan.removeArgv).toEqual(['mcp', 'remove', 'rethra-design']);
    expect(plan.getArgv).toEqual(['mcp', 'get', 'rethra-design']);
  });
...
describe('JSON-config agents', () => {
  ...
  it('kiro merges a stdio entry into the user MCP settings file', () => {
    ...
    expect(plan.keyPath).toEqual(['mcpServers']);
    expect(plan.serverKey).toBe('rethra-design');
    expect(plan.entry).toEqual({
      command: SPEC.command,
      args: SPEC.args,
      env: SPEC.env,
  ...
  it('raven install and removal preserve existing config and sibling servers', () => {
    ...
    expect(installed.tools.mcpServers['rethra-design']).toEqual(plan.entry);
...
describe('applyJsonInstall', () => {
  ...
  it('creates the file structure from empty input', () => {
    ...
    expect(parsed.mcpServers['rethra-design'].command).toBe(SPEC.command);
    expect(out.endsWith('\n')).toBe(true);
  });
  ...
  it('preserves unrelated keys and sibling servers', () => {
    ...
    expect(out.mcpServers['rethra-design'].type).toBe('stdio');
  });
...
describe('removeJsonInstall', () => {
  ...
  it('removes only the rethra-design entry', () => {
    const existing = applyJsonInstall(
      JSON.stringify({ mcpServers: { other: { command: 'x' } } }),
      plan,
    );
    const out = JSON.parse(removeJsonInstall(existing, plan)!);
    expect(out.mcpServers).not.toHaveProperty('rethra-design');
