...
describe('project skillId validation', () => {
  ...
  describe('POST /api/projects', () => {
    ...
    it('accepts a valid bundled skill id and stores it as-is', async () => {
      ...
      const resp = await createProject({
        id,
        name: 'Bundled skill',
        skillId: 'rethra-design-landing',
      });
      expect(resp.status).toBe(200);
      projectsToClean.push(id);
      const body = (await resp.json()) as { project: { skillId: string } };
      expect(body.project.skillId).toBe('rethra-design-landing');
    });
    ...
    it('canonicalizes an aliased skill id (editorial-collage → rethra-design-landing)', async () => {
      const id = uniqueId('p');
      const resp = await createProject({
        id,
        name: 'Aliased skill',
  ...
  describe('PATCH /api/projects/:id', () => {
    ...
    it('normalizes empty-string skillId on patch to null', async () => {
      const id = uniqueId('p');
      await createProject({ id, name: 'Patch empty', skillId: 'rethra-design-landing' });
      projectsToClean.push(id);
    ...
    it('treats null skillId on patch as unset', async () => {
      const id = uniqueId('p');
      await createProject({ id, name: 'Patch null', skillId: 'rethra-design-landing' });
      projectsToClean.push(id);
    ...
    it('leaves skillId untouched when the field is omitted from patch', async () => {
      const id = uniqueId('p');
      await createProject({ id, name: 'Patch omit', skillId: 'rethra-design-landing' });
      projectsToClean.push(id);
