...
describe('app version helpers', () => {
  ...
  it('prefers packaged app version metadata from the environment', () => {
    expect(resolveAppVersionInfo({
      ...
      resourcesPath: '/Applications/Rethra Design.app/Contents/Resources',
      execPath: '/Applications/Rethra Design.app/Contents/Resources/rethra-design/bin/node',
      platform: 'darwin',
      arch: 'arm64',
    })).toEqual({
  ...
  it('detects packaged runtimes without sidecar protocol knowledge', () => {
    expect(isPackagedRuntime({ resourcesPath: '/Applications/Rethra Design.app/Contents/Resources' })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/Applications/Rethra Design.app/Contents/Resources/rethra-design/bin/node',
      platform: 'darwin',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\Rethra Design\\resources\\rethra-design\\bin\\node.exe',
      platform: 'win32',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/opt/Rethra Design/resources/rethra-design/bin/node',
      platform: 'linux',
    })).toBe(true);
