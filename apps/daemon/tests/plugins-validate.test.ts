...
async function writeManifest(body: Record<string, unknown>) {
  await writeFile(path.join(folder, 'rethra-design.json'), JSON.stringify(body, null, 2));
}
...
describe('validatePluginFolder', () => {
  ...
  it('rejects malformed rethra-design.json with a parse error', async () => {
    await writeFile(path.join(folder, 'rethra-design.json'), '{ this is not json');
    const result = await validatePluginFolder({ folder });
    expect(result.ok).toBe(false);
    expect(result.resolveErrors.some((e) => e.includes('rethra-design.json'))).toBe(true);
  });
