...
describe('browser use diagnostics', () => {
  it('detects Browser Use prompts from the RethraDesign Browser menu', () => {
    expect(isBrowserUseRequested('hello')).toBe(false);
    expect(isBrowserUseRequested('@agent-browser\n\nBrowser tab context:')).toBe(true);
    expect(isBrowserUseRequested('Use the selected RethraDesign Browser tab as the bound target.')).toBe(true);
  });
