...
import { RETHRA_DESIGN_BRIEF_APP_HTML } from '../src/mcp-apps/brief-resource.js';
...
function createBriefAppHarness(options: {
  ...
  const dom = new JSDOM(RETHRA_DESIGN_BRIEF_APP_HTML, {
    beforeParse(window) {
      widgetWindow = window;
...
describe('local RethraDesign MCP brief app', () => {
  it('exposes collect_brief through the canonical MCP Apps resource', () => {
    ...
    expect(collectBrief).toMatchObject({
      ...
      _meta: {
        ui: {
          resourceUri: 'ui://rethra-design/artifact-card-v8.html',
        },
        'ui/resourceUri': 'ui://rethra-design/artifact-card-v8.html',
        'openai/outputTemplate': 'ui://rethra-design/artifact-card-v8.html',
      },
    });
    expect(localMcpResourceDefinitions()).toContainEqual(
      expect.objectContaining({
        uri: 'ui://rethra-design/artifact-card-v8.html',
  ...
  it('keeps one brief card without a self-triggering resize observer', () => {
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).not.toContain('ResizeObserver');
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).toContain(
      'ui/notifications/size-changed',
    );
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).toContain('{ width, height }');
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).toContain('document.body.scrollHeight');
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).toContain(
    ...
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).toContain('notifyIntrinsicHeight');
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).toContain('requestAnimationFrame');
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).toContain(
    ...
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).not.toContain(
      'error instanceof Error ? error.message',
    );
    expect(RETHRA_DESIGN_BRIEF_APP_HTML).not.toContain('setWidgetState');
  });
  ...
  it('keeps user-facing MCP copy on public RethraDesign product terms', () => {
    ...
    expect(userFacingCopy).toContain('RethraDesign Cloud');
