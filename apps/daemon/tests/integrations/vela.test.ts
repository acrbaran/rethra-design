 ...
 * successful `vela login` and RethraDesign just surfaces a small projection.
...
describe('resolveAmrProfile', () => {
  it('defaults to "prod" when RETHRA_DESIGN_AMR_PROFILE is unset or empty', () => {
    expect(resolveAmrProfile({})).toBe('prod');
    expect(resolveAmrProfile({ RETHRA_DESIGN_AMR_PROFILE: '   ' })).toBe('prod');
  });

  it('honors RETHRA_DESIGN_AMR_PROFILE when set to a known profile', () => {
    expect(resolveAmrProfile({ RETHRA_DESIGN_AMR_PROFILE: 'prod' })).toBe('prod');
    expect(resolveAmrProfile({ RETHRA_DESIGN_AMR_PROFILE: 'local' })).toBe('local');
    expect(resolveAmrProfile({ RETHRA_DESIGN_AMR_PROFILE: 'test' })).toBe('test');
    expect(resolveAmrProfile({ RETHRA_DESIGN_AMR_PROFILE: 'feature-test' })).toBe('feature-test');
  });

  it('uses VELA_PROFILE when RETHRA_DESIGN_AMR_PROFILE is unset', () => {
  ...
  it('warns for unknown RETHRA_DESIGN_AMR_PROFILE values and falls back to prod', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    expect(resolveAmrProfile({ RETHRA_DESIGN_AMR_PROFILE: 'evil' })).toBe('prod');
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining('RETHRA_DESIGN_AMR_PROFILE'),
    );
    warn.mockRestore();
  });
});
...
describe('readVelaLoginStatus', () => {
  ...
  it('recognizes a rotated file credential even when the config mtime is unchanged', () => {
    ...
    const env = { RETHRA_DESIGN_AMR_PROFILE: 'local' };
  ...
  it('returns loggedIn=false when ~/.amr/config.json is absent', () => {
    const status = readVelaLoginStatus({ RETHRA_DESIGN_AMR_PROFILE: 'local' });
  ...
  it('reads the Vela CLI config from AMR_HOME when set', () => {
    ...
    expect(readVelaControlApiContext({ RETHRA_DESIGN_AMR_PROFILE: 'local' })).toMatchObject({
      profile: 'local',
      apiUrl: 'http://127.0.0.1:18082',
      controlKey: 'vela_ctrl_custom',
    });
  ...
  it('isolates profiles — a logged-in "local" does not imply logged-in "prod"', () => {
    ...
    expect(readVelaLoginStatus({ RETHRA_DESIGN_AMR_PROFILE: 'local' }).loggedIn).toBe(true);
    expect(readVelaLoginStatus({ RETHRA_DESIGN_AMR_PROFILE: 'prod' }).loggedIn).toBe(false);
  });
  ...
  it('treats malformed JSON as logged-out rather than crashing', () => {
    ...
    expect(readVelaLoginStatus({ RETHRA_DESIGN_AMR_PROFILE: 'local' }).loggedIn).toBe(false);
  });
...
describe('readVelaCredentialRevision', () => {
  it('changes when file-backed logout clears the active profile credentials', async () => {
    ...
    const before = readVelaCredentialRevision({ RETHRA_DESIGN_AMR_PROFILE: 'local' });
    ...
    forgetVelaLogin({ RETHRA_DESIGN_AMR_PROFILE: 'local' });

    const after = readVelaCredentialRevision({ RETHRA_DESIGN_AMR_PROFILE: 'local' });
...
describe('readVelaControlApiContext', () => {
  it('keeps apiUrl and controlKey on the same config snapshot', async () => {
    ...
    const context = vela.readVelaControlApiContext({ HOME: tmpHome, RETHRA_DESIGN_AMR_PROFILE: 'test' });
...
describe('forgetVelaLogin', () => {
  it('removes only the resolved profile credentials and preserves the rest of the config', () => {
    ...
    forgetVelaLogin({ RETHRA_DESIGN_AMR_PROFILE: 'local' });
    expect(readVelaLoginStatus({ RETHRA_DESIGN_AMR_PROFILE: 'local' }).loggedIn).toBe(false);
    expect(readVelaLoginStatus({ RETHRA_DESIGN_AMR_PROFILE: 'prod' }).loggedIn).toBe(true);
  ...
  it('is a no-op when the resolved profile does not exist', () => {
    ...
    expect(() => forgetVelaLogin({ RETHRA_DESIGN_AMR_PROFILE: 'local' })).not.toThrow();
    const next = JSON.parse(readFileSync(file, 'utf8'));
    expect(next.profiles.prod.runtimeKey).toBe('rt-prod');
  });
...
describe('spawnVelaLogin', () => {
  ...
  it('spawns the configured vela binary and writes/reads only the feature-test AMR profile', async () => {
    ...
    expect(readVelaLoginStatus({ RETHRA_DESIGN_AMR_PROFILE: 'feature-test' })).toMatchObject({
      loggedIn: true,
