...
describe('connector routes', () => {
  ...
  it('returns branded callback HTML that notifies the opener', async () => {
    ...
    expect(html).toContain('RethraDesign');
    expect(html).toContain('rethra-design:connector-connected');
    expect(html).toContain('function requestClose()');
    expect(html).toContain('Your browser blocked automatic closing. You can close this tab and return to RethraDesign.');
  ...
  it('executes connected Composio tools through run-scoped tool auth', async () => {
    ...
    const response = await jsonFetch(`${baseUrl}/api/tools/connectors/execute`, {
      ...
      body: JSON.stringify({ connectorId: 'github', toolName: 'github.github_search_repositories', input: { query: 'rethra-design' } }),
    });
