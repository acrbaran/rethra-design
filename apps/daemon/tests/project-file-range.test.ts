...
describe('GET /api/projects/:id/raw/* range request route', () => {
  ...
  beforeAll(async () => {
    ...
    // acrbaran/rethra-design#7410. `<header>` is here because a bare
    // `/<head[^>]*>/` matches it too.
    await writeFile(
    ...
    // shape behind acrbaran/rethra-design#7410. The literal `</body>` inside the
  ...
  it('injects the URL preview scroll bridge after a `</body>` written inside a script string', async () => {
    // acrbaran/rethra-design#7410: splicing at the first raw-text `</body>` puts
  ...
  it('injects the URL preview sandbox shim after a `<head>` written inside a script string', async () => {
    // acrbaran/rethra-design#7410, head-open half: the document has no real
