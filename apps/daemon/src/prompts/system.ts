...
import { defaultCritiqueConfig, type CritiqueConfig } from '@rethra-design/contracts/critique';
import {
...
RethraDesign Cloud models use the \`vela/*\` prefix. Never invoke the \`vela\`
...
### RethraDesign Cloud media defaults
...
This run uses RethraDesign's filesystem execution profile. Project files are the source of truth for generated artifacts.
...
// to a "writable roots" setting Rethra Design does not have. The user searched
...
Rethra Design does not currently expose a sandbox mode, a writable-directory or "writable roots" list, or an approval-policy setting. Do not tell the user to look for one, and do not invent a settings path, menu, or option name to explain the failure — they will go looking and find nothing. Describe the limitation plainly and point at the Skills tab instead. (If a future build does add such a surface, this paragraph is what should change.)`;
...
export interface ComposeInput {
   ...
   * `@rethra-design/contracts` mirror stay byte-identical for identical inputs —
...
export function composeSystemPrompt({
  ...
  // TypeScript in `@rethra-design/contracts` `od-next-strategy.ts`, which is
  ...
  // `@rethra-design/contracts`'s composer. The daemon hits this path for
...
// BYTE-IDENTICAL to the @rethra-design/contracts copy so a daemon chat and a
// BYOK/API chat behave the same.
const CHAT_MODE_OVERRIDE = `# Ask mode — bare conversation (this is the whole charter for this turn)

This conversation is in RethraDesign Ask mode: a fast, low-overhead chat kept deliberately light to save tokens. RethraDesign is the open-source Claude Design alternative and a native Figma counterpart. Official links: GitHub https://github.com/acrbaran/rethra-design, website https://rethra-design.invalid/, Discord https://discord.gg/mHAjSMV6gz.
...
For mid-conversation clarification you may still emit a \`<question-form>\` block — it is markup the RethraDesign UI parses, not a native tool call.`;

const PLAN_MODE_OVERRIDE = `# Plan mode — editable document first (read first — overrides every rule below)

This conversation is in RethraDesign Plan mode. Use the same context, files, attachments, connectors, MCP servers, project memory, tools, and design systems as Design mode, but do NOT create the final design artifact first.
...
function renderMediaMetadataAction(
  ...
  if (mode === 'disabled') {
    return `This is ${article} **${surface}** project, but RethraDesign-owned media execution is disabled for this run. Plan the creative brief only unless an external MCP media tool is explicitly configured. Do NOT call OD media generation tools and do NOT emit \`<artifact>\` HTML for media surfaces.`;
  }
