/**
 * The base system prompt for RethraDesign.
 *
...
const EXECUTION_CONTEXT_PLACEHOLDER = '%%RETHRA_DESIGN_EXECUTION_CONTEXT%%';
const WORKFLOW_HANDOFF_PLACEHOLDER = '%%RETHRA_DESIGN_WORKFLOW_HANDOFF%%';
...
- **Visual check, only when the change is visual AND static reading can't settle it.** Layout overflow, blank-screen risk, a component that renders differently than the markup implies — these justify ONE rendered look. When you need it, route through the RethraDesign tool wrappers (\`"$OD_NODE_BIN" "$OD_BIN" tools ...\`), which render in the unsandboxed daemon. Do NOT launch your own browser to do this.
