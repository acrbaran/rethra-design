...
import type { ByokMediaDefaults, MediaExecutionPolicy, MediaSurface } from '@rethra-design/contracts';
...
- \`update-app\` -- "Rethra Design needs an update before it can generate images."
  Simplified Chinese, exactly:
  需要更新 Rethra Design 才能生成图片。
...
- \`contact-support\` -- "The image didn't come out, and it isn't anything you
  did. This one is on Rethra Design and we've logged it; trying again usually
  recovers, and if it keeps happening, contact us." Simplified Chinese,
  exactly:
  图片没生成出来,不是你的操作有误 —— 这次是 Rethra Design 自己的问题,我们已经记下了。重试一般能恢复;反复出现的话联系我们。
...
RethraDesign-owned media execution is **disabled for this run**. Do not call
...
RethraDesign Cloud image and video models use the \`vela/*\` catalogue prefix.
...
**Default recipe — use Rethra Design's scaffold, don't write from scratch.**
...
# Rethra Design writes the required files itself; HyperFrames init is never run.
