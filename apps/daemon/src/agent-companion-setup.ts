...
import { createPackageManagerInvocation } from '@rethra-design/platform';
import type {
...
import {
  hasRethraDesignProfile,
  resolveRethraDesignProfileDir,
...
const DSH_RUNTIME_PACKAGE_NAME = '@rethra-design/dsh-runtime';
...
async function materializeDevelopmentBundle(
  ...
  if (!(await fileExists(packageJsonPath))) {
    throw new AgentCompanionSetupError(
      'BUNDLED_COMPANION_INVALID',
      'This RethraDesign build does not contain the DeepSeek Harness connection component.',
    );
  }
...
async function stageVerifiedBundleInProfile(
  ...
  const relativeDirectory = '.rethra-design';
  const file = `${manifest.sha256}.tgz`;
  const profileDirectory = resolveRethraDesignProfileDir(env);
  ...
  // packaged-app path such as "RethraDesign" at its spaces.
...
async function installDeepSeekHarnessCompanionOnce(options: {
  ...
  const profileWasPresent = hasRethraDesignProfile(childEnv);
  const packageSpec = await stageVerifiedBundleInProfile(childEnv, manifest, bytes);
  try {
    await execAgentFile(
      launch.launchPath,
      ['plugin', '--profile', 'rethra-design', 'add', packageSpec],
    ...
    throw new AgentCompanionSetupError(
      'COMPANION_INSTALL_FAILED',
      'DeepSeek Harness could not install the RethraDesign connection component. No agent selection was changed.',
    );
  }
