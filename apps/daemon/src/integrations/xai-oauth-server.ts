...
// Once RethraDesign has its own client_id, the daemon's normal HTTP port
  ...
  await new Promise<void>((resolve, reject) => {
    const onError = (err: NodeJS.ErrnoException) => {
      if (err.code === 'EADDRINUSE') {
        reject(
          new Error(
            `Port ${port} is already in use — close any other process listening on ${host}:${port} (e.g. an in-flight Hermes or RethraDesign OAuth flow) and try again`,
          ),
        );
      } else {
        reject(err);
...
function renderResultPage(outcome: CallbackOutcome): string {
  if (outcome.kind === 'ok') {
    return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>RethraDesign — xAI authorized</title></head>
<body style="font:14px system-ui;padding:40px;max-width:480px;margin:auto;text-align:center;color:#222;">
  ...
  <p style="color:#666;">RethraDesign now has access to your SuperGrok subscription. You can close this tab and return to RethraDesign.</p>
</body></html>`;
  }
  const reason = escapeHtml(outcome.error || 'unknown error');
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>RethraDesign — sign-in failed</title></head>
<body style="font:14px system-ui;padding:40px;max-width:480px;margin:auto;text-align:center;color:#222;">
  <h1 style="font-size:18px;margin:0 0 12px;">Sign-in failed</h1>
  <p style="color:#c00;">${reason}</p>
  <p style="color:#666;">Close this tab and click <em>Sign in with X</em> again in RethraDesign.</p>
</body></html>`;
}
