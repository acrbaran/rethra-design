...
export interface OdNextCoordinatorResult {
  ...
  visibleText: string;
  reasonCodes: string[];
  decisionSummary?: RethraDesignPlanContractV2['decisionSummary'];
  instruction?:
    | {
        stage: 'clarification';
 ...
 * ran the frozen plan, delivered a canonical entry Rethra Design resolved itself,
...
export function odNextTurnMayInferProductionCompletion(
  ...
  return task.inputStage === 'production'
    ...
    // Simple only. The inference rests on Rethra Design having resolved the
 ...
 * by evidence Rethra Design resolved itself, which is stronger than the agent's
...
function validateAcceptedTurn(
  ...
  plan: RethraDesignPlanContractV2 | undefined,
  visibleText: string,
  input: {
    toolUseCount: number;
    executionPreflight?: OdNextExecutionPreflightInput;
