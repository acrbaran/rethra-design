...
// HTTP headers (see x-od-analytics-* constants in @rethra-design/contracts);
...
export function readAnalyticsContext(req: Request): AnalyticsContext | null {
  ...
  const entrySurface = boundedHeader(
    req,
    ANALYTICS_HEADER_ENTRY_SURFACE,
    ['rethra_design_ui', 'od_cli', 'external_mcp'] as const,
  );
  ...
  const publisherClass = boundedHeader(
    req,
    ANALYTICS_HEADER_PUBLISHER_CLASS,
    ['rethra_design_first_party', 'third_party', 'unknown'] as const,
  );
...
export function createAnalyticsService(args: {
  ...
  // country enrichment. RethraDesign's daemon runs on the USER'S OWN machine,
...
// path; the canonical hash lives in @rethra-design/contracts/analytics so
