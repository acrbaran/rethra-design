import {
  ...
  type RethraDesignBriefAnswers,
  type RethraDesignBriefArtifactType,
  type RethraDesignBriefQuestion,
...
interface LocalizedBriefCopy {
  artifactNames: Record<RethraDesignBriefArtifactType, string>;
...
const ENGLISH_ARTIFACT_NAMES: Record<RethraDesignBriefArtifactType, string> = {
  website: 'website',
...
const BRIEF_COPY: Record<LocalMcpBriefLocale, LocalizedBriefCopy> = {
  en: {
    ...
    description:
      'Choose one option for each decision. The same readable brief can be used with RethraDesign Cloud or Local Codex.',
    submitLabel: 'Confirm brief',
    completeCard:
      'Complete the rendered RethraDesign brief card. The confirmation returns a readable summary; internal correlation values must remain hidden.',
  ...
  'zh-CN': {
    ...
    description:
      '请为每项决策选择一个选项。确认后的可读需求可用于 RethraDesign Cloud、本地 Codex 或安全 BYOK。',
    submitLabel: '确认需求',
    completeCard: '请填写显示的 RethraDesign 需求卡。确认后会返回可读摘要；不要向用户展示内部关联值。',
    confirmed: '需求已确认。',
  ...
  'zh-TW': {
    ...
    description:
      '請為每項決策選擇一個選項。確認後的可讀需求可用於 RethraDesign Cloud、本機 Codex 或安全 BYOK。',
    submitLabel: '確認需求',
    completeCard: '請填寫顯示的 RethraDesign 需求卡。確認後會回傳可讀摘要；不要向使用者顯示內部關聯值。',
    confirmed: '需求已確認。',
  ...
  ja: {
    ...
    description:
      '各項目から 1 つ選択してください。確認したブリーフは RethraDesign Cloud、ローカル Codex、安全な BYOK で利用できます。',
    submitLabel: 'ブリーフを確認',
    completeCard:
      '表示された RethraDesign のブリーフカードに回答してください。確認後は読みやすい概要が返されます。内部の関連付け値は表示しないでください。',
...
for (const locale of ['zh-CN', 'zh-TW', 'ja'] as const) {
  ...
  for (const questions of Object.values(rethraDesignBriefCatalog)) {
...
export function createLocalMcpBriefStore(
  ...
  return {
    ...
    collect(input) {
      ...
      const decision = collectRethraDesignBrief({
        artifactType,
        knownAnswers,
        skip,
      });
    ...
    confirm(input) {
      ...
      if (!draft) {
        throw new Error(
          'The RethraDesign brief has expired or is unknown. Call collect_brief again.',
        );
      }
      if (draft.nonce !== nonce) {
        throw new Error('The RethraDesign brief nonce is invalid.');
      }
      ...
      if (!decision.complete) {
        ...
        throw new Error(
          `The RethraDesign brief is incomplete. Missing: ${missing.join(', ')}.`,
        );
      }
      ...
      if (draft.confirmation) {
        if (draft.confirmationAnswersDigest !== confirmationAnswersDigest) {
          throw new Error(
            'This RethraDesign brief was already confirmed with different answers.',
          );
        }
...
function readArtifactType(value: unknown): RethraDesignBriefArtifactType {
  if (
    typeof value !== 'string'
    || !Object.hasOwn(rethraDesignBriefCatalog, value)
  ) {
    throw new Error(
      `artifactType must be one of: ${Object.keys(rethraDesignBriefCatalog).join(', ')}`,
    );
  }
  return value as RethraDesignBriefArtifactType;
}
...
function readProjectTitle(value: unknown): string {
  if (value === undefined) return 'Untitled RethraDesign artifact';
...
function stableAnswerDigest(answers: RethraDesignBriefAnswers): string {
