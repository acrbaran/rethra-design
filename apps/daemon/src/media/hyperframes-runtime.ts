...
export function resolveHyperFramesCliPath({
  ...
  try {
    ...
    throw new Error(
      `Bundled HyperFrames CLI is unavailable. Reinstall Rethra Design so its pinned ` +
...
export function resolveHyperFramesBrowserRuntimePath({
  ...
  try {
    ...
    const detail = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Bundled HyperFrames browser runtime is unavailable. Reinstall Rethra Design so its pinned ` +
