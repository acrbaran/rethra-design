...
// and attributes it to the RethraDesign open-source project for shareability.
export function buildDesignSystemSkillsMarkdown(input: {
  ...
  lines.push('');
  lines.push(
    'Generated with **RethraDesign** — the open-source, local-first Claude Design alternative. ' +
      'Generate decks, landing pages, dashboards, and brand systems with your favourite AI ' +
      'coding agent.',
  );
  lines.push('');
  lines.push('https://github.com/acrbaran/rethra-design');
  lines.push('');
...
async function migrateLegacyDesignSystemPackage(
  ...
  const title = normalizeTitle(metadata.title ?? firstHeading(body) ?? id);
  const summary = summarize(body) || 'A reusable RethraDesign design system.';
  ...
  await Promise.all([
    ...
    appKitExists
      ? writeIfMissing(
          ...
          `# ${title} UI Kit\n\nThis package was migrated from an earlier RethraDesign design-system workspace. Use \`index.html\` as the applied interface example and replace it with source-backed modular components when new repository evidence is available.\n`,
        )
...
function generatedDesignSystemFileWrites(
  ...
  const summary = input.summary || 'A user-created RethraDesign design system.';
...
function buildDraftDesignSystemBody(input: UserDesignSystemInput & { title: string }): string {
  ...
  const summary = cleanText(input.summary) || 'A user-authored design system for future RethraDesign projects.';
...
A reusable RethraDesign package for ${input.title}.
...
function renderSkill(input: {
  ...
  return `---
name: ${skillName}
description: Use this skill when generating RethraDesign artifacts that should follow ${input.title}.
user-invocable: true
---
...
function renderOverviewHtml(
  ...
  return renderHtmlDocument(
    title,
    `<main class="overview">
      <p class="eyebrow">RethraDesign system</p>
