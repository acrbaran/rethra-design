...
const OBJECT_RELAY_MARKER_HEADER = 'X-Rethra-Design-Telemetry';
...
function inferRelayUrl(env: NodeJS.ProcessEnv): string | null {
  const explicit = env.RETHRA_DESIGN_OBJECT_RELAY_URL?.trim();
  if (explicit) return normalizeRethraDesignTelemetryRelayUrl(explicit);
  const rawTelemetryRelayUrl = env.RETHRA_DESIGN_TELEMETRY_RELAY_URL?.trim();
  if (!rawTelemetryRelayUrl) return null;
  const telemetryRelayUrl = normalizeRethraDesignTelemetryRelayUrl(rawTelemetryRelayUrl);
  try {
...
function readRelayConfig(env: NodeJS.ProcessEnv): ObjectRelayConfig | null {
  ...
  return {
    ...
    timeoutMs: parsePositiveInt(
      env.RETHRA_DESIGN_OBJECT_RELAY_TIMEOUT_MS ?? env.RETHRA_DESIGN_TELEMETRY_TIMEOUT_MS,
      10_000,
    ),
    objectMaxBytes: parsePositiveInt(
      env.RETHRA_DESIGN_OBJECT_MAX_BYTES,
      DEFAULT_OBJECT_MAX_BYTES,
    ),
    objectBatchMaxBytes: parsePositiveInt(
      env.RETHRA_DESIGN_OBJECT_BATCH_MAX_BYTES ?? env.TRACE_OBJECT_BATCH_MAX_BYTES,
...
function manifestBase(
  ...
  const common = {
    ...
    access_policy: 'rethra_design_auth_required' as const,
...
export async function buildTraceObjectManifests(
  ...
  const merged = manifests.map((entry, index) => {
    ...
    return {
      ...
      stored_in_rethra_design: result.status === 'available',
