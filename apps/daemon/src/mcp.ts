...
// (Claude Code, Cursor, Zed) pull files from a local RethraDesign
...
import {
  RETHRA_DESIGN_BRIEF_APP_HTML,
  RETHRA_DESIGN_BRIEF_APP_VERSION,
...
const SERVER_NAME = 'rethra-design';
const SERVER_VERSION = '0.2.0';
const DEFAULT_MCP_STDIO_IDLE_EXIT_MS = 30 * 60 * 1000;
const MAX_MCP_STDIO_IDLE_EXIT_MS = 24 * 60 * 60 * 1000;
export const RETHRA_DESIGN_BRIEF_APP_RESOURCE =
  'ui://rethra-design/artifact-card-v8.html';

export const MCP_SERVER_INSTRUCTIONS = [
  'Use only these product names in user-facing replies: RethraDesign Cloud and Local Codex.',
...
function isDaemonUnreachableResult(result: McpToolCallResult): boolean {
  return result.isError === true
    && result.content.some((item) =>
      item.text.includes('cannot reach the RethraDesign daemon'),
    );
}
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no RethraDesign activity).',
} as const;

const PLUGIN_WORKFLOW_ID_ARG = {
  type: 'string',
  description:
    'Opaque workflow id issued by the local RethraDesign MCP after the first attributed call. Reuse it for later plugin-attributed calls; never invent or display it.',
} as const;

const EXTERNAL_PLUGIN_CONTEXT_ARG = {
  type: 'object',
  properties: {
    id: { type: 'string', const: RETHRA_DESIGN_PLUGIN_ID },
    version: { type: 'string' },
    ...
    publisherClass: {
      type: 'string',
      enum: ['rethra_design_first_party', 'third_party', 'unknown'],
    },
  },
  required: [
    'id',
...
export const TOOL_DEFS = [
  {
    name: 'collect_brief',
    description:
      'Open an interactive RethraDesign brief card for a new artifact. Use the returned human-readable confirmation with any explicit execution mode; never ask the user to copy an internal draft id or nonce.',
    inputSchema: {
      type: 'object',
      properties: {
        artifactType: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Collect RethraDesign brief' },
    _meta: {
      ui: { resourceUri: RETHRA_DESIGN_BRIEF_APP_RESOURCE },
      'ui/resourceUri': RETHRA_DESIGN_BRIEF_APP_RESOURCE,
      'openai/outputTemplate': RETHRA_DESIGN_BRIEF_APP_RESOURCE,
    },
  },
  {
    name: 'confirm_brief',
    description:
      'Confirm the choices from the rendered RethraDesign brief card. Returns a readable summary; draft ids and nonces are internal widget data, never user-facing copy.',
    inputSchema: {
      type: 'object',
      properties: {
        briefDraftId: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Confirm RethraDesign brief' },
  },
  {
    name: 'list_projects',
    description: 'List every RethraDesign project on this daemon.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List RethraDesign projects' },
  },
  {
    name: 'get_active_context',
    description:
      'Project + file the user has open in RethraDesign right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with RethraDesign (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no RethraDesign activity.",
        },
        include: {
          type: 'string',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Get RethraDesign project' },
  },
  {
    name: 'get_file',
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no RethraDesign activity.',
        },
        offset: {
          type: 'number',
  ...
  },
  {
    ...
    description:
      'Create one normal RethraDesign project artifact entry file. Writes name+content, rejects existing targets, and persists artifactManifest when supplied. HTML, Markdown, and SVG entries get a default manifest when omitted. Project optional; defaults to the active project.',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        artifactManifest: {
          type: 'object',
          additionalProperties: true,
          description: 'Optional ArtifactManifest sidecar. If omitted, RethraDesign infers one for HTML, Markdown, or SVG entry files.',
        },
      },
      required: ['name', 'content'],
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create RethraDesign artifact' },
  },
  {
    name: 'write_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Write RethraDesign project file' },
  },
  {
    name: 'delete_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete RethraDesign project file' },
  },
  {
    name: 'delete_project',
    description:
      'Permanently delete an RethraDesign project including its files and conversations. Requires both an explicit project id/name AND confirm:true — there is no active-project fallback because the operation is irreversible.',
    inputSchema: {
      type: 'object',
      properties: {
        project: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete RethraDesign project' },
  },
  {
    name: 'create_project',
    description:
      'Create a new empty RethraDesign project to generate into, then call start_run against it. Returns the project (with its id) plus a conversationId. The id is derived from name unless you pass one explicitly.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create RethraDesign project' },
  },
  // Discovery + generation. An external coding agent does NOT run a
  // skill itself — it commissions RethraDesign to, via start_run. The
  ...
  {
    name: 'list_skills',
    description: 'List RethraDesign skills you can pass to start_run as a recipe. Discovery only — RethraDesign runs the skill, not you.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List RethraDesign skills' },
  },
  {
    name: 'list_plugins',
    description: 'List installed RethraDesign plugins (packaged design workflows) you can pass to start_run as plugin + inputs.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List RethraDesign plugins' },
  },
  {
    name: 'start_vela_login',
    description:
      'Start RethraDesign Cloud browser sign-in through the local RethraDesign daemon. Returns the activation URL and user code when manual browser completion is needed. The tool name is an internal compatibility identifier and must not be repeated to the user.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    annotations: {
      ...WRITE_ANNOTATIONS,
      openWorldHint: true,
      title: 'Sign in to RethraDesign Cloud',
    },
  },
  {
    name: 'get_vela_login_status',
    description:
      'Check whether RethraDesign Cloud browser sign-in is complete. Does not expose credentials. The tool name is an internal compatibility identifier and must not be repeated to the user.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    annotations: {
      ...READ_ANNOTATIONS,
      openWorldHint: true,
      title: 'Check RethraDesign Cloud sign-in',
    },
  },
  {
    name: 'start_run',
    description:
      'Commission RethraDesign to generate or refine a design. RethraDesign spawns its own agent to do the work and returns a runId immediately. Poll get_run(runId) until status is terminal; its Preview/Studio reference is the default delivery. Call get_artifact only when source context is genuinely needed. Project optional; defaults to the active project. Requires an existing project (create one first with create_project).',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        resume: {
          type: 'boolean',
          description:
            'Set true only after the user has topped up a paused RethraDesign Cloud run. Reuse the exact original requestId and payload; RethraDesign resumes the same logical run.',
        },
        pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG,
      },
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Generate with RethraDesign' },
  },
  {
    name: 'get_run',
    description:
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Check RethraDesign run' },
  },
  {
    name: 'cancel_run',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'List RethraDesign agents' },
  },
];
...
export function localMcpResourceDefinitions() {
  return [
    {
      uri: RETHRA_DESIGN_BRIEF_APP_RESOURCE,
      name: 'RethraDesign brief',
      title: 'Choose the artifact direction',
      description:
        'Interactive local RethraDesign brief card shared by RethraDesign Cloud and Local Codex modes.',
...
export async function _listMcpResources(
  ...
  const resources = [
    ...
    {
      uri: 'od://focus/active',
      name: 'Active RethraDesign context',
      description: 'The project/file the user has open in RethraDesign right now.',
...
export async function _readMcpResource(
  ...
  if (uri === RETHRA_DESIGN_BRIEF_APP_RESOURCE) {
    return {
      contents: [
        {
          uri,
          mimeType: 'text/html;profile=mcp-app',
          text: RETHRA_DESIGN_BRIEF_APP_HTML,
          _meta: { version: RETHRA_DESIGN_BRIEF_APP_VERSION },
        },
      ],
    };
  }
...
function mcpFailureFacts(
  ...
  const errorCode = message.includes('PLUGIN_CONTRACT_REJECTED')
    ...
    : message.includes('cannot reach the RethraDesign daemon')
      ? 'DAEMON_UNREACHABLE'
...
async function observeMcpToolCall(
  ...
  try {
    ...
    const common = {
      ...
      ...(rawPlugin
        ? {
            external_plugin_id: RETHRA_DESIGN_PLUGIN_ID,
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        ...
        '',
        'RethraDesign (OD) is a local-first design workspace. The user typically',
        ...
        '',
        'To make RethraDesign GENERATE or refine a design (rather than just',
        ...
        '    confirm_brief returns the readable brief to reuse with RethraDesign',
        ...
        ' - for RethraDesign Cloud, call the Cloud login-status tool first.',
        ...
        '    RethraDesign spawns its own agent to do the work.',
        ...
        '',
        'Generation patience: RethraDesign runs typically take 5–30',
        ...
        'deliverables — RethraDesign natively produces browser-viewable',
        'HTML/SVG (including HTML-rendered decks), but the user may want a',
        'real binary file (.pptx / .docx / .pdf) which RethraDesign does NOT',
        ...
        '',
        'When extending an RethraDesign design in another codebase, pull',
    ...
    switch (name) {
      ...
      case 'get_active_context': {
        ...
        if (!data || data.active === false) {
          return ok({
            active: false,
            hint: 'RethraDesign has no active project right now. The active context expires about 5 minutes after the last user interaction with RethraDesign, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
          });
        }
        return ok(data);
      }
...
// the user had already created in RethraDesign.
//
...
async function startRun(
  ...
  if (
    ...
    throw new Error(
      'raw API keys are not accepted by RethraDesign MCP. Configure Local BYOK in the RethraDesign UI and start that run from the local product instead.',
    );
  }
  const { id, resolved, active } = await resolveProjectArg(baseUrl, args.project, headers);
    ...
    withActiveEcho(
      {
        ...
        ...(studioUrl
          ...
          : {}),
        hint: 'Run started. RethraDesign generation normally takes 5–30 minutes. Polls showing status:running with no new files / unchanged file mtimes is the inner agent thinking, NOT a hang — DO NOT cancel_run out of impatience and DO NOT substitute write_file to produce the design yourself; OD\'s pipeline is what gives the result its design quality. Poll get_run(runId) every 30–60 seconds; report "still working" to the user between polls and keep waiting. On terminal status, artifactRef is the durable identity; previewUrl and studioUrl are browser links for the current RethraDesign runtime and must be refreshed with get_run after RethraDesign restarts.',
      },
      active,
      resolved,
    ),
...
async function getRun(
  ...
  if (status.status !== 'succeeded' || typeof status.projectId !== 'string' || !status.projectId) {
    ...
    if (status.failureAction === 'recharge') {
      ...
      enriched.hint =
        'RethraDesign Cloud paused this logical run because the account balance is insufficient. Preserve the brief and project, show rechargeUrl to the user, and do not switch modes. After the user confirms the top-up, call start_run once with the exact original payload, the same requestId, and resume:true; RethraDesign Cloud will resume the existing run and billing operation. Do not expose internal runtime or tool identifiers.';
    }
    if (typeof status.eventsLogPath === 'string' && status.eventsLogPath.length > 0) {
      ...
      if (studioUrl) {
        enriched.hint += ` While the run is in flight, studioUrl can be used as an optional workspace progress link — render it as \`[Watch progress in RethraDesign studio](${studioUrl})\` if you choose to show it. This URL is valid for the current RethraDesign runtime; call get_run again after RethraDesign restarts.`;
      }
    }
    return ok(enriched);
  }
  ...
  enriched.hint = previewUrl
    ? `Run finished. artifactRef is the durable project/file identity. previewUrl and studioUrl are browser links for the current RethraDesign runtime only; if either stops working after RethraDesign restarts, call get_run again with this runId to obtain current links. Render previewUrl as a clickable link now. agentMessage carries the inner agent's explanation; show it alongside the link. Call get_artifact({ project: "${status.projectId}" }) when you need the source files — always pass project explicitly; omitting it falls back to the active project, which may differ. eventsLogPath, when present, holds the full inner-agent event log for forensics.`
    : 'Run finished but produced no files. The inner agent\'s output is in agentMessage — relay it to the user verbatim. Most often this is a clarifying question (e.g. a <question-form>) you should answer by calling start_run again with a more specific prompt or a chosen plugin. When studioUrl is present, show it as a clickable markdown link (`[Open RethraDesign studio](STUDIO_URL)`) so the user can navigate to the OD page that shows the chat history — never render it as inline code. eventsLogPath, when present, holds the full event log if you need to inspect what happened.';
  return ok(enriched);
}
...
// host-injected React/Babel do not — those still need the RethraDesign
...
// renamed in the RethraDesign UI shows up within a few seconds.
...
// open in RethraDesign. Returns the resolved id plus, for echo-back to the
...
async function resolveProjectArg(
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and RethraDesign has no active project. The active context expires about 5 minutes after the last user interaction with RethraDesign - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
function formatError(err: unknown, daemonUrl: string): string {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the RethraDesign daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
