...
//
// RethraDesign acts as an MCP CLIENT to one or more external MCP servers
...
// users can copy-paste between RethraDesign and other tools without
// translation.
 ...
 * RethraDesign daemon hand servers to a single `opencode run` invocation
...
export const MCP_TEMPLATES: McpTemplate[] = [
  // ── image-generation ────────────────────────────────────────────────
  {
    ...
    description:
      'Image and video generation MCP from higgsfield.ai. Exposes Soul, Nano Banana, Flux, Kling, Veo, Seedance, and 25+ other models. Endpoint is streamable HTTP at /mcp; click "Connect" after saving — RethraDesign completes OAuth and stores the token server-side, so no terminal step is needed and the connection survives across chat turns and cloud deployments.',
    transport: 'http',
    authMode: 'oauth',
  ...
  },
  {
    ...
    example:
      'Generate a 1200×630 dark-theme OG image titled "RethraDesign 1.0" with a subtitle "Design with agents", with a Lucide "sparkles" icon.',
    command: 'npx',
    args: ['-y', 'ogforge-api'],
  },
  {
    ...
    example:
      'Generate a brand-orange (#FF5722) QR code pointing to https://rethradesign.app, with our logo at /absolute/path/to/logo.png embedded in the center.',
    command: 'npx',
    args: ['-y', 'qr-mcp'],
  },
  {
  ...
  // ── utilities ───────────────────────────────────────────────────────
  {
    ...
    label: 'Filesystem',
    description:
      'Read, write and list files in a sandboxed directory. Useful for letting the agent operate on a folder outside your RethraDesign project.',
    transport: 'stdio',
    category: 'utilities',
