  ...
  const serverHandle: StartedDaemonRuntime = await startDaemonRuntime({
    ...
    desktopFrameRenderer: async (input: DesktopRenderFramesInput): Promise<DesktopRenderFramesResult> => {
      ...
      try {
        ...
        return {
          ok: false,
          error: "Rethra Design desktop is not running. Open or upgrade the desktop client before rendering HyperFrames video.",
      ...
      if (status.state !== "running" || status.capabilities?.frameRenderer !== true) {
        return {
          ok: false,
          error: "This Rethra Design desktop version does not support HyperFrames rendering. Upgrade the desktop client and try again.",
          errorCode: "FRAME_RENDERER_NOT_READY",
        };
      }
