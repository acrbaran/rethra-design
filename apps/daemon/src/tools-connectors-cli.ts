...
export async function auditDesignSystemPackage(
  ...
  if (options.referencePackage === true) {
    if (!fileSet.has('DESIGN.md')) {
      addIssue('warning', 'missing_rethra_design_rules', 'Reference packages may omit DESIGN.md, but generated RethraDesign packages must include it as the canonical rules file.', 'DESIGN.md');
    }
  } else {
