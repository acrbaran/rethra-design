...
//   - rethra-design.json (required; this is what the installer
//     resolves first)
...
export interface PackPluginInput {
  // Path to the plugin folder. Must contain rethra-design.json.
  folder: string;
...
export async function packPlugin(input: PackPluginInput): Promise<PackPluginResult> {
  ...
  // Confirm the folder shape — rethra-design.json must exist + parse.
  let manifestRaw: string;
  try {
    manifestRaw = await fsp.readFile(path.join(folder, 'rethra-design.json'), 'utf8');
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new PackPluginError(`folder ${folder} does not contain rethra-design.json`);
    }
    throw new PackPluginError(`failed to read rethra-design.json: ${(err as Error).message}`);
  }
  ...
  try {
    ...
    throw new PackPluginError(`rethra-design.json failed to parse as JSON: ${(err as Error).message}`);
  }
