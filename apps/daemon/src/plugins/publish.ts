  ...
  switch (args.catalog) {
    ...
    case 'rethra-design': {
      const bodyWithRegistry = [
        body,
        '',
        '## RethraDesign registry entry',
        '',
        '- Target path: `plugins/community/<plugin-name>/rethra-design.json`',
...
export function upsertMarketplaceJsonEntry(args: {
  ...
  const manifest: MarketplaceJsonManifest = {
    ...
    name: typeof existing.name === 'string' ? existing.name : 'rethra-design-marketplace',
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `rethra-design.json` sidecar (additive RethraDesign metadata).');
  lines.push('');
  lines.push('Generated by `od plugin publish` — see https://rethra-design.invalid/docs/plugins-spec.md.');
  return lines.join('\n');
}
