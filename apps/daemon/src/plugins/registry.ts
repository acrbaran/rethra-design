...
//   manifest folders.
// - Resolves a plugin folder into either an `rethra-design.json`-anchored
...
export async function resolvePluginFolder(opts: ResolveOptions): Promise<ResolveResult> {
  ...
  const sidecarPath = path.join(folder, 'rethra-design.json');
  ...
  if (fs.existsSync(sidecarPath)) {
    ...
    if (!parsed.ok) {
      errors.push(...parsed.errors.map((e) => `rethra-design.json: ${e}`));
    } else {
  ...
  if (!sidecar && adapters.length === 0) {
    return {
      ok: false,
      errors: [...errors, `Plugin folder contains no SKILL.md, no .claude-plugin/plugin.json, and no rethra-design.json: ${folder}`],
      warnings,
    };
  }
