...
// frontmatter the skills protocol expects) + rethra-design.json (sidecar
...
export async function scaffoldPlugin(input: ScaffoldInput): Promise<ScaffoldResult> {
  ...
  try {
    ...
    const conflicts = entries.filter((e) =>
      e === 'SKILL.md' || e === 'rethra-design.json' || e === '.claude-plugin' || e === 'README.md',
    );
  ...
  const manifestPath = path.join(folder, 'rethra-design.json');
  ...
  const readme = [
    ...
    '- `rethra-design.json` — the versioned RethraDesign marketplace sidecar.',
    '',
    'Edit `SKILL.md` to teach the agent how to perform the workflow.',
    'Edit `rethra-design.json` to refine the marketplace card and inputs.',
    '',
  ].join('\n');
