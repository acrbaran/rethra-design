 ...
 *
 * RethraDesign 0.3.x ran from the repo and wrote runtime state to
 ...
 * way to know about it. See https://github.com/acrbaran/rethra-design/issues/710.
 *
...
export function migrateLegacyDataDirSync(
  ...
  if (!legacyDirHasPayload(legacyDir)) {
    ...
    throw new LegacyMigrationError(
      'legacy_dir_invalid',
      `OD_LEGACY_DATA_DIR="${legacyDir}" is not a usable legacy data dir (expected app.sqlite directly inside it). Quit RethraDesign, fix the path, and relaunch.`,
    );
  }
