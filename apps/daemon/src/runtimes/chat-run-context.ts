...
function renderWorkspaceContextToolHints(items: WorkspaceContextItem[]) {
  ...
  if (kinds.has('local-code')) {
    hints.push(
      '- Local code folders: use the absolute path as read-only implementation context. Inspect files under that folder when useful, align with its conventions, and make edits only in the active RethraDesign project unless the user explicitly asks otherwise.',
    );
  }
