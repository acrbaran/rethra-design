...
export const RETHRA_DESIGN_PLUGIN_ID = 'rethra-design';
...
export interface ExternalPluginContext {
  id: typeof RETHRA_DESIGN_PLUGIN_ID;
  version: string;
...
export interface ExternalPluginRunAnalyticsHints {
  ...
  externalPluginId: typeof RETHRA_DESIGN_PLUGIN_ID;
...
export function validateExternalPluginContext(
  ...
  if (input.id !== RETHRA_DESIGN_PLUGIN_ID) {
    throw pluginContractError(
      `id must be ${RETHRA_DESIGN_PLUGIN_ID}`,
    );
  }
  if (
