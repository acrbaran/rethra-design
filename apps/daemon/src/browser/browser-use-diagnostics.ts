...
import type { BrowserUseDiscoveryFacts, BrowserUseRunState } from '@rethra-design/contracts';
...
export function isBrowserUseRequested(...values: unknown[]): boolean {
  return values.some((value) => (
    ...
    (
      ...
      value.includes('Use the selected RethraDesign Browser tab as the bound target.')
    )
  ));
}
...
export function renderBrowserUseUnavailablePrompt(state: BrowserUseRunState | null): string {
  ...
  return [
    ...
    '',
    `Browser automation was requested, but RethraDesign has not confirmed a matching in-app browser backend for this run. Reason: \`${state.reason}\`.`,
