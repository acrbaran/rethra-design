...
export function diagnoseClaudeCliFailure(
  ...
  if (authFailure) {
    ...
    const configHint = hasConfigDir
      ...
      : 'If you use multiple Claude profiles, set CLAUDE_CONFIG_DIR in Settings so RethraDesign spawns the same profile that works in your terminal.';
    return withContext(
      'Claude Code could not authenticate. Run `claude`, use `/login`, then retry the RethraDesign request.',
  ...
  if (windowsCredentialMismatch) {
    return withContext(
      ...
      'Re-authenticate Claude Code in the same Windows, WSL, or shell environment that RethraDesign uses. On native Windows, check Windows Credential Manager if `/login` does not repair the session.',
      input,
    );
  }
  ...
  if (configStateFailure) {
    ...
    const detail = hasConfigDir
      ? 'Re-run `claude` and `/login` for that profile, then retry RethraDesign.'
  ...
  if (!text.trim() && input.exitCode === 1) {
    ...
    const detail = hasConfigDir
      ? 'Re-run `claude` and `/login` for that profile, then retry RethraDesign.'
      : 'Run `claude`, use `/login`, and retry. If you use multiple Claude profiles, set CLAUDE_CONFIG_DIR in Settings so RethraDesign uses the same profile as your terminal.';
    return withContext(
      message,
      detail,
      input,
