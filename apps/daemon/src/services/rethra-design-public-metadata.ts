export interface RethraDesignGithubRepoStats {
  stargazersCount: number;
  fetchedAt: number;
  stale: boolean;
}

export interface RethraDesignGithubLatestReleaseInfo {
  tagName: string;
  htmlUrl: string;
  fetchedAt: number;
  stale: boolean;
...
export interface RethraDesignDiscordPresence {
  onlineCount: number;
  memberCount: number;
  fetchedAt: number;
  stale: boolean;
...
export interface RethraDesignPublicMetadataService {
  readGithubRepoStats(): Promise<RethraDesignGithubRepoStats>;
  readLatestReleaseInfo(): Promise<RethraDesignGithubLatestReleaseInfo>;
  readDiscordPresence(): Promise<RethraDesignDiscordPresence>;
}
...
export interface RethraDesignPublicMetadataServiceOptions {
...
export const RETHRA_DESIGN_DISCORD_INVITE_URL = `https://discord.gg/${RETHRA_DESIGN_DISCORD_INVITE_CODE}`;
const RETHRA_DESIGN_DISCORD_INVITE_API = `https://discord.com/api/v10/invites/${RETHRA_DESIGN_DISCORD_INVITE_CODE}?with_counts=true`;
const RETHRA_DESIGN_DISCORD_CACHE_TTL_MS = 5 * 60 * 1000;
const RETHRA_DESIGN_DISCORD_TIMEOUT_MS = 4_000;
...
export function createRethraDesignPublicMetadataService({
  fetchImpl = fetch,
  now = () => Date.now(),
}: RethraDesignPublicMetadataServiceOptions = {}): RethraDesignPublicMetadataService {
  let githubRepoCache: CachedGithubRepoStats | null = null;
  let githubRepoInflight: Promise<RethraDesignGithubRepoStats> | null = null;
  let githubLatestReleaseCache: CachedGithubLatestReleaseInfo | null = null;
  let githubLatestReleaseInflight: Promise<RethraDesignGithubLatestReleaseInfo> | null = null;
  let discordPresenceCache: CachedDiscordPresence | null = null;
  let discordPresenceInflight: Promise<RethraDesignDiscordPresence> | null = null;

  async function readGithubRepoStats(): Promise<RethraDesignGithubRepoStats> {
    const currentTime = now();
    if (
      githubRepoCache &&
      currentTime - githubRepoCache.fetchedAt < RETHRA_DESIGN_GITHUB_CACHE_TTL_MS
    ) {
    ...
    githubRepoInflight = (async () => {
      ...
      const timeout = setTimeout(() => ctrl.abort(), RETHRA_DESIGN_GITHUB_TIMEOUT_MS);
      try {
        const response = await fetchImpl(RETHRA_DESIGN_GITHUB_REPO_API, {
          headers: {
            accept: 'application/vnd.github+json',
            'user-agent': 'rethra-design-daemon',
          },
          signal: ctrl.signal,
        });
        if (!response.ok) {
  ...
  async function readLatestReleaseInfo(): Promise<RethraDesignGithubLatestReleaseInfo> {
    const currentTime = now();
    if (
      githubLatestReleaseCache &&
      currentTime - githubLatestReleaseCache.fetchedAt < RETHRA_DESIGN_GITHUB_CACHE_TTL_MS
    ) {
    ...
    githubLatestReleaseInflight = (async () => {
      ...
      const timeout = setTimeout(() => ctrl.abort(), RETHRA_DESIGN_GITHUB_TIMEOUT_MS);
      try {
        const response = await fetchImpl(RETHRA_DESIGN_GITHUB_RELEASE_LATEST_API, {
          headers: {
            accept: 'application/vnd.github+json',
            'user-agent': 'rethra-design-daemon',
          },
  ...
  async function readDiscordPresence(): Promise<RethraDesignDiscordPresence> {
    const currentTime = now();
    if (
      discordPresenceCache &&
      currentTime - discordPresenceCache.fetchedAt < RETHRA_DESIGN_DISCORD_CACHE_TTL_MS
    ) {
    ...
    discordPresenceInflight = (async () => {
      const ctrl = new AbortController();
      const timeout = setTimeout(() => ctrl.abort(), RETHRA_DESIGN_DISCORD_TIMEOUT_MS);
      try {
        const response = await fetchImpl(RETHRA_DESIGN_DISCORD_INVITE_API, {
          headers: {
