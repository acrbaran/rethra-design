...
export const OWNERSHIP_SENTINEL = ".rethra-design-updater-root.json";
...
export function logStoreError(logger: DesktopUpdaterLogger, error: DesktopUpdateErrorSnapshot): void {
  logger.error("[rethra-design updater] invalid update store", error);
}
      ...
      if (!(await directoryIsEmpty(realRoot))) {
        return {
          ...
          error: createError(
            ...
            `update root is not empty and has no RethraDesign updater ownership marker: ${realRoot}`,
          ),
        };
      }
...
export async function rebuildOwnedUpdateRootForManualClear(
  ...
  try {
    ...
    await writeJson(layout.metadataPath, { version: STORE_METADATA_VERSION });
    logger.warn("[rethra-design updater] rebuilt corrupt owned update store for manual clear", { root: realRoot });
    return true;
  } catch (error) {
    logger.warn("[rethra-design updater] failed to rebuild corrupt update store for manual clear", {
