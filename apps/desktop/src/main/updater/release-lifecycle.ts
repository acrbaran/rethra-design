...
export async function cleanupBackDirectory(root: string, logger: DesktopUpdaterLogger): Promise<void> {
  ...
  if (!entry.isDirectory() || entry.isSymbolicLink()) {
    logger.warn("[rethra-design updater] skipped invalid update backup directory", backDir);
    return;
  }
  const realBackDir = await realpath(backDir).catch(() => null);
  if (realBackDir == null || !containsPath(root, realBackDir)) {
    logger.warn("[rethra-design updater] skipped escaped update backup directory", backDir);
    return;
  }
  ...
  await Promise.all(entries.map(async (entry) => {
    ...
    await rm(resolved, { force: true, recursive: true }).catch((error: unknown) => {
      logger.warn("[rethra-design updater] failed to clean update backup entry", error);
    });
  }));
}
...
export function scheduleBackCleanup(root: string, logger: DesktopUpdaterLogger): void {
  void cleanupBackDirectory(root, logger).catch((error: unknown) => {
    logger.warn("[rethra-design updater] failed to clean update backup directory", error);
  });
}
...
export async function withUpdaterLifecycleLock<T>(
  ...
  if (!acquired && options.reclaimStale === true) {
    ...
    const ownerPid = isRecord(owner) && owner.owner === "rethra-design-updater-lifecycle"
    ...
    if (ownerIsDead) {
      ...
      try {
        ...
        if (acquired) {
          logger.warn("[rethra-design updater] reclaimed stale updater lifecycle lock", {
            lockRoot: layout.lockRoot,
            ownerPid,
          });
        }
      } catch (error) {
        logger.warn("[rethra-design updater] failed to reclaim stale updater lifecycle lock", error);
      }
    }
  }
  if (!acquired) {
    logger.warn("[rethra-design updater] skipped release lifecycle because updater lifecycle lock is held", {
      lockRoot: layout.lockRoot,
    });
    return null;
  }
  try {
    ...
    await rm(layout.lockRoot, { force: true, recursive: true }).catch((error: unknown) => {
      logger.warn("[rethra-design updater] failed to release updater lifecycle lock", error);
    });
  }
}
...
export async function cleanupDeprecatedReleaseEntries(input: {
  ...
  for (const entry of descriptor.releases) {
    ...
    try {
      ...
      logger.warn("[rethra-design updater] failed to clean deprecated release", {
