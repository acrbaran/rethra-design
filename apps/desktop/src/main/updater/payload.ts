...
import type { DesktopUpdateChannel } from "@rethra-design/sidecar-proto";
...
export async function cleanupLauncherPayloadRoots(input: {
  ...
  await rm(versionPaths.stagingRoot, { force: true, recursive: true }).catch((error: unknown) => {
    ...
    logger.warn("[rethra-design updater] failed post-commit launcher staging cleanup", {
  ...
  try {
    ...
    const cleanupError = launcherCleanupErrorFrom(error);
    logger.warn("[rethra-design updater] skipped post-commit launcher cleanup because cleanup state is invalid", {
      error: cleanupError.message,
      errorCode: cleanupError.code,
      event: "launcher-payload-cleanup",
  ...
  try {
    ...
    const cleanupError = launcherCleanupErrorFrom(error);
    logger.warn("[rethra-design updater] failed to scan launcher payload versions after commit", {
      error: cleanupError.message,
      errorCode: cleanupError.code,
      event: "launcher-payload-cleanup",
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      logger.warn("[rethra-design updater] deferred launcher payload cleanup", {
        error: cleanupError.message,
        errorCode: cleanupError.code,
        event: "launcher-payload-cleanup",
        path,
  ...
  try {
    ...
    const cleanupError = launcherCleanupErrorFrom(error);
    logger.warn("[rethra-design updater] failed to persist deferred launcher payload cleanup", {
      error: cleanupError.message,
      errorCode: cleanupError.code,
      event: "launcher-payload-cleanup",
      path: versionPaths.cleanupPath,
...
export async function clearLauncherStateForManualClear(input: {
  ...
  await rm(launcherPaths.attemptsPath, { force: true }).catch((error: unknown) => {
    logger.warn("[rethra-design updater] failed to clear stale launcher attempt", {
  ...
  if (rawHandoff != null) {
    ...
    if (!confirmed) {
      await rm(launcherPaths.handoffPath, { force: true }).catch((error: unknown) => {
        logger.warn("[rethra-design updater] failed to clear stale desktop handoff journal", {
          error: error instanceof Error ? error.message : String(error),
  ...
  try {
    ...
    logger.warn("[rethra-design updater] skipped manual launcher version cleanup because runtime state is unreadable", {
      error: error instanceof Error ? error.message : String(error),
...
export async function runLauncherCleanupLifecycle(input: {
  ...
  try {
    ...
    logger.warn("[rethra-design updater] failed to read launcher cleanup lifecycle inputs", {
      error: error instanceof Error ? error.message : String(error),
  ...
  for (const entry of cleanup.versions) {
    ...
    try {
      ...
      logger.warn("[rethra-design updater] failed to clean deprecated launcher payload", {
        error: error instanceof Error ? error.message : String(error),
