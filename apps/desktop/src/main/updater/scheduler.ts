...
export function createDesktopUpdaterScheduler(
  ...
  const normalizeScheduledDelay = (delayMs: number): number => {
    ...
    if (!warnedZeroDelay) {
      ...
      logger.warn(
        `[rethra-design updater] refusing non-positive scheduled poll delay (${delayMs}ms); `
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          logger.warn("[rethra-design updater] startup silent payload update failed", silentError);
        }
      }
      if (status.installResult != null) {
      ...
      logger.warn("[rethra-design updater] scheduled update check failed", error);
    } finally {
      tickRunning = false;
    }
