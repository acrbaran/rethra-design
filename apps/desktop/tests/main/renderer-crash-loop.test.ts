...
describe("renderer crash-loop breaker wiring", () => {
  ...
  test("the crash screen offers report + save-logs + email actions via already-exposed IPC", () => {
    ...
    expect(runtimeSource).toContain("window.rethraDesignDesktop");
    expect(runtimeSource).toContain("exportDiagnostics");
    expect(runtimeSource).toContain("support@rethra-design.invalid");
...
describe("isSupportMailtoUrl", () => {
  test("allows a mailto to the support address carrying only subject/body", () => {
    expect(isSupportMailtoUrl("mailto:support@rethra-design.invalid")).toBe(true);
    expect(isSupportMailtoUrl("mailto:support@rethra-design.invalid?subject=Crash&body=hi")).toBe(true);
    // Address comparison is case-insensitive.
    expect(isSupportMailtoUrl("mailto:Support@Rethra-Design.AI")).toBe(true);
  });
  ...
  test("rejects any other address or scheme so widening open-external can't be abused", () => {
    ...
    expect(isSupportMailtoUrl("https://rethra-design.invalid")).toBe(false);
  ...
  test("rejects extra recipients/headers smuggled through the query (to/cc/bcc/unknown)", () => {
    ...
    expect(isSupportMailtoUrl("mailto:support@rethra-design.invalid?bcc=attacker@example.com")).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@rethra-design.invalid?cc=attacker@example.com")).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@rethra-design.invalid?to=attacker@example.com")).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@rethra-design.invalid?subject=x&bcc=attacker@example.com")).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@rethra-design.invalid?whatever=1")).toBe(false);
  });
  ...
  test("rejects a CR/LF-injected subject/body that could smuggle a mail header", () => {
    ...
    expect(
      isSupportMailtoUrl("mailto:support@rethra-design.invalid?subject=ok%0D%0ABcc:attacker@example.com"),
    ).toBe(false);
    expect(isSupportMailtoUrl("mailto:support@rethra-design.invalid?body=line1%0Aline2")).toBe(false);
  });
});
...
describe("isFirstPartyMailtoUrl", () => {
  test("covers every address the app itself offers to email", () => {
    ...
    expect(isFirstPartyMailtoUrl("mailto:support@rethra-design.invalid")).toBe(true);
  });
  ...
  test("applies the same recipient/header hardening as the support predicate", () => {
    ...
    expect(isFirstPartyMailtoUrl("https://rethra-design.invalid")).toBe(false);
