...
async function createUpdaterFixture(options: {
  ...
  const artifactBody = Buffer.from(options.artifactBody ?? "rethra design updater fixture");
  ...
  const payloadBody = Buffer.from(options.payloadBody ?? "rethra design updater payload fixture");
  ...
  return new Response(JSON.stringify({
    ...
    platforms: {
      mac: {
        ...
        artifacts: {
          dmg: {
            name: `rethra-design-${version}-mac-arm64.dmg`,
            sha256: "0".repeat(64),
            size: 1,
            url: `https://example.invalid/rethra-design-${version}-mac-arm64.dmg`,
          },
        },
      },
    },
...
async function writeLauncherPayloadFixture(destinationRoot: string, version: string): Promise<void> {
  await mkdir(join(destinationRoot, "payload", "resources", "rethra-design"), { recursive: true });
  await writeFile(join(destinationRoot, "payload", "Rethra Design.exe"), "");
  await writeFile(join(destinationRoot, "payload", "resources", "rethra-design-config.json"), "{}\n");
...
describe("desktop updater", () => {
  ...
  it("adds session and source context to lifecycle logs", async () => {
    ...
    try {
      ...
      expect(logger.info).toHaveBeenCalledWith("[rethra-design updater] lifecycle", expect.objectContaining({
        enabled: true,
        event: "session-start",
  ...
  it("downloads, verifies, persists, and dry-runs opening a mac package", async () => {
    ...
    try {
      ...
      expect(await readFile(checked.downloadPath ?? "", "utf8")).toBe("rethra design updater fixture");
  ...
  it("keeps using the Windows installer when payload metadata exists but launcher context is absent", async () => {
    ...
    const fixture = await createUpdaterFixture({
      includePayload: true,
      payloadBody: "rethra design windows payload fixture",
      platform: "win",
    });
    try {
      const updater = createDesktopUpdater({
  ...
  it("falls back to the installer when launcher context is valid but metadata has no payload artifact", async () => {
    const root = makeRoot();
    const fixture = await createUpdaterFixture({
      artifactBody: "rethra design windows installer fixture",
      channel: "beta",
      platform: "win",
      version: "1.0.0-beta.3",
    });
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherLaunchPath = join(root, "installed", "Rethra Design Beta.exe");
    try {
      ...
      expect(checked.state).toBe(DESKTOP_UPDATE_STATES.DOWNLOADED);
      expect(checked.artifact?.type).toBe("installer");
      expect(await readFile(checked.downloadPath ?? "", "utf8")).toBe("rethra design windows installer fixture");
    } finally {
      await fixture.close();
      rmSync(root, { force: true, recursive: true });
    }
  ...
  it("downloads and applies launcher payload only when launcher runtime context validates", async () => {
    ...
    try {
      ...
      const updater = createDesktopUpdater({
        ...
        processExecPath: "C:\\Program Files\\Rethra Design Beta\\Rethra Design Beta.exe",
        processPid: 4242,
      });
      ...
      expect(checked.artifact?.name).toBe("rethra-design-1.0.0-beta.2-win-x64-payload.7z");
      ...
      expect(await readFile(checked.downloadPath ?? "", "utf8")).toBe("rethra design windows payload fixture");
  ...
  it("defers locked old launcher cleanup without blocking prepare or re-extracting the promoted payload", async () => {
    ...
    const fixture = await createUpdaterFixture({
      channel: "beta",
      includePayload: true,
      payloadBody: "rethra design locked cleanup payload fixture",
      platform: "win",
      version: "1.0.0-beta.2",
    });
  ...
  it("keeps launcher activation successful when unrelated old-version cleanup is deferred", async () => {
    ...
    const fixture = await createUpdaterFixture({
      channel: "beta",
      includePayload: true,
      payloadBody: "rethra design activation cleanup payload fixture",
      platform: "win",
      version: "1.0.0-beta.2",
    });
    const launcherPaths = resolveLauncherPaths({
  ...
  async function runLauncherReseedCheck(
    ...
    if (installedOuterVersion != null) {
      ...
      await writeFile(
        join(root, "installed", "resources", "rethra-design-config.json"),
    ...
    const updaterDeps: NonNullable<Parameters<typeof createDesktopUpdater>[1]> = {
      extractLauncherPayloadArchive: async ({ destinationRoot }) => {
        ...
        await writeFile(
          ...
          `${JSON.stringify({
            channel: "beta",
            entry: { cwd: "payload", executable: "payload/Rethra Design.exe" },
            namespace: "release-beta-win",
            payloadRoot: "payload",
            platform: "win32",
            schemaVersion: LAUNCHER_SCHEMA_VERSION,
  ...
  it("reclaims a dead-owner lifecycle lock during manual cache clear", async () => {
    ...
    try {
      ...
      await writeFile(join(root, "state", "lock", "owner.json"), JSON.stringify({
        ...
        owner: "rethra-design-updater-lifecycle",
        pid: 2_147_483_647,
        version: 1,
      }));
  ...
  it("rebuilds an owned update store with corrupt metadata through clear-cache", async () => {
    const root = makeRoot();
    try {
      await writeFile(join(root, ".rethra-design-updater-root.json"), JSON.stringify({
        createdAt: "2026-01-01T00:00:00.000Z",
        owner: "rethra-design-updater",
        source: "tools-pack",
        version: 1,
      }));
  ...
  it("rebuilds an owned update store with unexpected root entries through clear-cache", async () => {
    ...
    try {
      ...
      expect(existsSync(join(root, ".rethra-design-updater-root.json"))).toBe(true);
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });
  ...
  it("rejects launcher payloads that cannot resolve packaged config before activating them", async () => {
    ...
    const fixture = await createUpdaterFixture({
      channel: "beta",
      includePayload: true,
      payloadBody: "rethra design payload without packaged config",
      platform: "win",
      version: "1.0.0-beta.2",
    });
    ...
    try {
      ...
      expect(checked.error?.message).toContain("rethra-design-config.json");
  ...
  it("keeps using the installer when launcher context has a missing installed launch path", async () => {
    ...
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherLaunchPath = join(root, "missing", "Rethra Design Beta.exe");
    try {
      await mkdir(join(root, "launcher"), { recursive: true });
      ...
      const updater = createDesktopUpdater({
        ...
        processExecPath: "C:\\Users\\runneradmin\\AppData\\Roaming\\Rethra Design Beta\\launcher\\channels\\beta\\namespaces\\release-beta-win\\versions\\1.0.0-beta.1\\payload\\Rethra Design.exe",
      });
  ...
  it("relaunches mac launcher payloads through the prepared payload executable", async () => {
    ...
    const fixture = await createUpdaterFixture({
      artifactBody: "rethra design mac dmg fixture",
      channel: "beta",
      includePayload: true,
      payloadBody: "rethra design mac payload fixture",
      platform: "mac",
      version: "1.0.0-beta.3",
    });
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherRoot = root;
    const launcherLaunchPath = join(root, "installed", "Rethra Design Beta.app");
    const launches: Array<{ appPid: number; launchPath: string; root: string }> = [];
    try {
      ...
      const updater = createDesktopUpdater({
        ...
        extractLauncherPayloadArchive: async ({ destinationRoot }) => {
          await mkdir(join(destinationRoot, "payload", "Rethra Design Beta.app", "Contents", "MacOS"), { recursive: true });
          await mkdir(join(destinationRoot, "payload", "Rethra Design Beta.app", "Contents", "Resources", "rethra-design"), { recursive: true });
          await writeFile(join(destinationRoot, "payload", "Rethra Design Beta.app", "Contents", "MacOS", "Rethra Design Beta"), "");
          await writeFile(join(destinationRoot, "payload", "Rethra Design Beta.app", "Contents", "Resources", "rethra-design-config.json"), "{}\n");
          await writeFile(
            join(destinationRoot, "manifest.json"),
            `${JSON.stringify({
              channel: "beta",
        ...
        processExecPath: join(root, "launcher", "channels", "beta", "namespaces", "release-beta", "versions", "1.0.0-beta.2", "payload", "Rethra Design Beta.app", "Contents", "MacOS", "Rethra Design Beta"),
        processPid: 4243,
      });
  ...
  it("relaunches prerelease mac launcher payloads through the prerelease app bundle", async () => {
    const root = makeRoot();
    const fixture = await createUpdaterFixture({
      artifactBody: "rethra design prerelease mac dmg fixture",
      channel: "prerelease",
      includePayload: true,
      payloadBody: "rethra design prerelease mac payload fixture",
      platform: "mac",
      version: "1.0.0-prerelease.3",
    });
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherRoot = root;
    const launcherLaunchPath = join(root, "installed", "Rethra Design Prerelease.app");
    const launches: Array<{ appPid: number; launchPath: string; root: string }> = [];
    try {
      await mkdir(launcherLaunchPath, { recursive: true });
      await mkdir(join(root, "launcher"), { recursive: true });
      ...
      const updater = createDesktopUpdater({
        ...
        extractLauncherPayloadArchive: async ({ destinationRoot }) => {
          await mkdir(join(destinationRoot, "payload", "Rethra Design Prerelease.app", "Contents", "MacOS"), { recursive: true });
          await mkdir(join(destinationRoot, "payload", "Rethra Design Prerelease.app", "Contents", "Resources", "rethra-design"), { recursive: true });
          await writeFile(join(destinationRoot, "payload", "Rethra Design Prerelease.app", "Contents", "MacOS", "Rethra Design Prerelease"), "");
          await writeFile(join(destinationRoot, "payload", "Rethra Design Prerelease.app", "Contents", "Resources", "rethra-design-config.json"), "{}\n");
          await writeFile(
            join(destinationRoot, "manifest.json"),
            `${JSON.stringify({
              channel: "prerelease",
              entry: {
                cwd: "payload/Rethra Design Prerelease.app",
                executable: "payload/Rethra Design Prerelease.app/Contents/MacOS/Rethra Design Prerelease",
              },
              namespace: "release-prerelease",
              payloadRoot: "payload",
              platform: "darwin",
        ...
        processExecPath: join(root, "launcher", "channels", "prerelease", "namespaces", "release-prerelease", "versions", "1.0.0-prerelease.2", "payload", "Rethra Design Prerelease.app", "Contents", "MacOS", "Rethra Design Prerelease"),
        processPid: 4244,
      });
      ...
      expect(checked.artifact?.type).toBe("payload");
      expect(checked.artifact?.name).toBe("rethra-design-1.0.0-prerelease.3-mac-arm64-payload.zip");
      expect(await readFile(checked.downloadPath ?? "", "utf8")).toBe("rethra design prerelease mac payload fixture");
      ...
      const payloadLaunchPath = join(
        ...
        "1.0.0-prerelease.3",
        "payload",
        "Rethra Design Prerelease.app",
        "Contents",
        "MacOS",
        "Rethra Design Prerelease",
      );
  ...
  it("relaunches Windows launcher payloads through the prepared payload executable", async () => {
    ...
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherRoot = root;
    const launcherLaunchPath = join(root, "installed", "Rethra Design.exe");
    const launches: Array<{ appPid: number; launchPath: string; root: string }> = [];
    try {
      await mkdir(join(root, "installed"), { recursive: true });
      await writeFile(launcherLaunchPath, "");
      ...
      const updater = createDesktopUpdater({
        ...
        processExecPath: "C:\\Users\\runneradmin\\AppData\\Roaming\\Rethra Design Beta\\launcher\\channels\\beta\\namespaces\\release-beta-win\\versions\\1.0.0-beta.2\\payload\\Rethra Design.exe",
        processPid: 4244,
      });
  ...
  it("cleans failed launcher payload staging without deleting an existing version root", async () => {
    ...
    const fixture = await createUpdaterFixture({
      channel: "beta",
      includePayload: true,
      payloadBody: "rethra design bad windows payload fixture",
      platform: "win",
      version: "1.0.0-beta.2",
    });
    const namespaceRoot = join(root, "launcher", "channels", "beta", "namespaces", "release-beta-win");
  ...
  it("recovers from an interrupted artifact download without surfacing an error", async () => {
    const root = makeRoot();
    const fixture = await createUpdaterFixture({
      artifactBody: "rethra design updater fixture with retry",
    ...
    try {
      ...
      // Byte-range resumption is covered by @rethra-design/download. At this
  ...
  it("does not expose raw terminated transport errors when update download retries are exhausted", async () => {
    const root = makeRoot();
    const fixture = await createUpdaterFixture({
      artifactBody: "rethra design updater fixture that keeps failing",
            ...
            mac: {
              ...
              artifacts: {
                dmg: {
                  ...
                  sha256: digest,
                  size: artifactBody.byteLength,
                  url: `https://fixture.test/rethra-design-${version}-mac-arm64.dmg`,
                },
              },
            },
          },
  ...
  it("clears a stale payload install freeze when the promised relaunch never became the running version", async () => {
    ...
    const fixture = await createUpdaterFixture({
      channel: "beta",
      includePayload: true,
      payloadBody: "rethra design rollback freeze payload fixture",
      platform: "win",
      version: "1.0.0-beta.2",
    });
    const launcherPaths = resolveLauncherPaths({
    ...
    try {
      ...
      await writeFile(join(root, "metadata.json"), JSON.stringify({
        incoming: {
          arch: "x64",
          artifact: {
            name: "rethra-design-1.0.1-win-x64-setup.exe",
            platformKey: "win",
            type: "installer",
            url: "https://fixture.test/rethra-design-1.0.1-win-x64-setup.exe",
          },
          channel: "stable",
          cycleId,
          metadata: {},
  ...
  symlinkIt("refuses to use a symlinked updater root", async () => {
    ...
    try {
      ...
      expect(checked.state).toBe(DESKTOP_UPDATE_STATES.ERROR);
      expect(checked.error?.code).toBe("update-root-not-owned");
      expect(existsSync(join(realRoot, ".rethra-design-updater-root.json"))).toBe(false);
    } finally {
      await fixture.close();
