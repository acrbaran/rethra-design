...
const VALID =
  "rethradesign://workspace/invite/continue?workspace_id=ws-1&member_id=wm-1&invite_id=inv-1&nonce=n-1";
...
describe("findDeeplinkArg", () => {
  it("finds the rethradesign url in an argv list", () => {
...
describe("continueInviteFromUrl", () => {
  ...
  it("focuses without any daemon call for the workspace/open hand-off deeplink", async () => {
    ...
    const focus = vi.fn();
    const onCompleted = vi.fn();
    const out = await continueInviteFromUrl("rethradesign://workspace/open", {
  ...
  it("accepts workspace/open with query params and trailing slash", async () => {
    ...
    const out = await continueInviteFromUrl(
      "rethradesign://workspace/open/?source=cli_activate",
  ...
  // Any web page can fire the `rethradesign://` scheme, so pin where the focus
  ...
  it.each([
    "rethradesign://workspace/openx",
    "rethradesign://workspace/open/extra",
    "rethradesign://other/open",
    "rethradesign://workspace/OPEN",
    "rethradesign://Workspace/open",
  ...
  it("ignores a url that is not an invite deeplink (no daemon call)", async () => {
    const fetchImpl = vi.fn() as unknown as typeof fetch;
    const out = await continueInviteFromUrl("rethradesign://something/else", {
      resolveDaemonBaseUrl: async () => "http://x",
      fetch: fetchImpl,
    });
    expect(out).toEqual({ ok: false, reason: "not_an_invite_deeplink" });
