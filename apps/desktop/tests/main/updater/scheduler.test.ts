...
import { DESKTOP_UPDATE_CHANNELS, DESKTOP_UPDATE_STATES } from "@rethra-design/sidecar-proto";
...
describe("desktop updater scheduler", () => {
  it("silently applies a ready launcher payload during the startup poll when enabled", async () => {
    ...
    const payloadStatus = {
      arch: "arm64",
      artifact: {
        name: "rethra-design-1.0.1-mac-arm64-payload.zip",
        platformKey: "mac",
        size: 1024,
        type: "payload",
      ...
      downloadPath: "/tmp/rethra-design-updates/payload.zip",
      enabled: true,
    ...
    const installedStatus = {
      ...
      installResult: {
        ...
        path: "/tmp/rethra-design-updates/payload.zip",
      },
    };
    const updater = {
  ...
  it("floors non-positive scheduled delays instead of spinning a zero-ms loop", async () => {
    ...
    const updater = {
      ...
      config: {
        ...
        currentVersion: "1.0.0",
        downloadRoot: "/tmp/rethra-design-updates",
        enabled: true,
