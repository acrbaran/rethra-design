...
describe("desktop updater feed", () => {
  it("resolves the installed outer version from the platform bundle layout", async () => {
    const root = makeRoot();
    try {
      const appRoot = join(root, "Rethra Design.app");
      await mkdir(join(appRoot, "Contents", "Resources"), { recursive: true });
      await writeFile(join(appRoot, "Contents", "Resources", "rethra-design-config.json"), '{"appVersion":"0.7.0"}\n');
      ...
      const winExe = join(root, "win-install", "Rethra Design Beta.exe");
      await mkdir(join(root, "win-install", "resources"), { recursive: true });
      await writeFile(winExe, "");
      await writeFile(join(root, "win-install", "resources", "rethra-design-config.json"), '{"appVersion":"0.8.0-beta.2"}\n');
      ...
      const malformedExe = join(root, "broken-install", "Rethra Design.exe");
      await mkdir(join(root, "broken-install", "resources"), { recursive: true });
      await writeFile(malformedExe, "");
      await writeFile(join(root, "broken-install", "resources", "rethra-design-config.json"), "not json\n");
