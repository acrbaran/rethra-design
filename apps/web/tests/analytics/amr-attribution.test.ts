...
describe('AMR attribution helper', () => {
  ...
  it('accepts every AMR entry source defined for RethraDesign entry points', () => {
    const track = vi.fn();
    const sources = [
      'onboarding_amr_card',
  ...
  it('adds RethraDesign attribution params to AMR wallet URLs', () => {
    expect(
      attributedAmrUrl('https://rethra-design.invalid/amr/dashboard?tab=recharge', {
      ...
      'https://rethra-design.invalid/amr/dashboard?tab=recharge&od_origin=rethra_design&od_entry_id=od-amr-entry-123&od_entry_source=generation_preview_recharge&od_entry_at=2026-06-03T12%3A00%3A00.000Z',
    );
  });
  ...
  it('forwards campaign and conversion attribution for final payment joins', () => {
    ...
    const url = new URL(
      attributedAmrUrl('https://rethra-design.invalid/zh/pricing/', attribution),
    );
