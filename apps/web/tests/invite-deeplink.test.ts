...
describe('parseInviteDeeplink / buildInviteDeeplink', () => {
  it('round-trips a well-formed continuation deeplink', () => {
    ...
    expect(url.startsWith('rethradesign://workspace/invite/continue?')).toBe(true);
    expect(parseInviteDeeplink(url)).toEqual(PAYLOAD);
  });

  it('parses the documented baseline URL shape', () => {
    const url =
      'rethradesign://workspace/invite/continue?workspace_id=ws-1&member_id=mem-9&invite_id=inv-42&nonce=nonce-abc';
    expect(parseInviteDeeplink(url)).toEqual(PAYLOAD);
  });
  ...
  it('rejects a URL missing any required field', () => {
    for (const drop of ['workspace_id', 'member_id', 'invite_id', 'nonce']) {
      ...
      params.delete(drop);
      const url = `rethradesign://workspace/invite/continue?${params.toString()}`;
  ...
  it('rejects a required field that is present but empty', () => {
    const url =
      'rethradesign://workspace/invite/continue?workspace_id=ws-1&member_id=&invite_id=inv-42&nonce=nonce-abc';
    expect(parseInviteDeeplink(url)).toBeNull();
  });
  ...
  it('rejects the wrong scheme, host, or path', () => {
    ...
    expect(
      parseInviteDeeplink(
        'rethradesign://team/invite/continue?workspace_id=ws-1&member_id=mem-9&invite_id=inv-42&nonce=n',
      ),
    ).toBeNull();
    expect(
      parseInviteDeeplink(
        'rethradesign://workspace/invite/accept?workspace_id=ws-1&member_id=mem-9&invite_id=inv-42&nonce=n',
      ),
    ).toBeNull();
  });
