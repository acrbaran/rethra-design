...
describe('buildAgentGuideMarkdown', () => {
  it('emits a top-level header and the setup checklist by default', () => {
    ...
    expect(md).toMatch(/^# RethraDesign — agent setup guide/);
  ...
  it('surfaces version and CLI hints in the checklist when supplied', () => {
    ...
    expect(md).toContain('Reported RethraDesign version: `0.42.0`');
  ...
  it('omits hint sentences when the corresponding option is not provided', () => {
    const md = buildAgentGuideMarkdown();
    expect(md).not.toContain('Reported RethraDesign version');
  ...
  it('uses daemon install-info for the MCP config instead of assuming od is on PATH', () => {
    const md = buildAgentGuideMarkdown({
      ...
      mcpInstallInfo: {
        command: 'C:\\Program Files\\Rethra Design\\Rethra Design.exe',
        args: [
          'C:\\Program Files\\Rethra Design\\resources\\app\\apps\\daemon\\dist\\cli.js',
          'mcp',
        ],
        env: {
          ELECTRON_RUN_AS_NODE: '1',
          OD_DATA_DIR: 'C:\\Users\\Ada\\AppData\\Roaming\\Rethra Design',
        },
      },
    });

    expect(md).toContain('"command": "C:\\\\Program Files\\\\Rethra Design\\\\Rethra Design.exe"');
    expect(md).toContain(
      '"C:\\\\Program Files\\\\Rethra Design\\\\resources\\\\app\\\\apps\\\\daemon\\\\dist\\\\cli.js"',
    );
    expect(md).toContain('"ELECTRON_RUN_AS_NODE": "1"');
    expect(md).toContain('"OD_DATA_DIR": "C:\\\\Users\\\\Ada\\\\AppData\\\\Roaming\\\\Rethra Design"');
