...
/**
 * 报错卡**标题**那一行的三格:S01 / S02 / S04(《Rethra Design 报错文案｜精简版》)。
 *
 ...
 *      S04(Rethra Design 智能体没授权,新稿「Rethra Design 尚未登录」)今天共用
...
describe('报错卡标题 · S01 / S02 / S04', () => {
  ...
  it('S04:Rethra Design 智能体没授权 —— 标题说的是 Rethra Design 自己', () => {
    const title = errorCardTitle({ agentId: 'amr', code: 'AMR_AUTH_REQUIRED' });

    expect(title).not.toMatch(/\{agent\}/);
    expect(title).toBe('Rethra Design 尚未登录');
  });
  ...
  it('S02 和 S04 不是同一句话 —— 一个键装不下两格', () => {
     ...
     * AMR 的 agent 名恰好是 `RethraDesign`,所以哪怕两格共用一个键,渲染出来
     * 也是「Claude 尚未登录」对「RethraDesign 尚未登录」—— 仍然不相等。
     * 判据要钉在 S04 说的**是不是产品那句**:`RethraDesign`(无空格)是 agent
     * 标签,`Rethra Design`(有空格)才是产品名。
     */
    expect(s04).not.toContain('RethraDesign');
  });
