...
import type { RethraDesignHostUpdaterStatusListener, RethraDesignHostUpdaterStatusSnapshot } from '@rethra-design/host';
...
function downloadedStatus(overrides: Partial<RethraDesignHostUpdaterStatusSnapshot> = {}): RethraDesignHostUpdaterStatusSnapshot {
  return {
    ...idleStatus(),
...
function payloadDownloadedStatus(overrides: Partial<RethraDesignHostUpdaterStatusSnapshot> = {}): RethraDesignHostUpdaterStatusSnapshot {
...
describe('UpdaterPopup', () => {
  ...
  it('shows only the ready indicator until the user opens the install prompt', async () => {
    ...
    expect(screen.getByText('RethraDesign 1.2.3-beta.4 is ready. RethraDesign will close and open the installer.')).toBeTruthy();
  ...
  it('uses reinstall copy and the learn-more link for forced installer reinstalls', async () => {
    ...
    expect(
      screen.getByText('RethraDesign 1.2.3-beta.4 requires a full reinstall. RethraDesign will close and open the installer.'),
    ).toBeTruthy();
  ...
  it('uses localized ready prompt copy from the app i18n provider', async () => {
    ...
    expect(screen.getByText('RethraDesign 1.2.3-beta.4 已就绪。RethraDesign 会关闭并打开安装器。')).toBeTruthy();
  });
  ...
  it('uses install-and-restart copy for payload updates', async () => {
    ...
    expect(screen.getByText('RethraDesign 1.2.3-beta.4 已就绪。RethraDesign 会关闭并自动重启。')).toBeTruthy();
  });
  ...
  it('keeps the prompt in handoff loading after opening the installer', async () => {
    ...
    let resolveInstall: (status: RethraDesignHostUpdaterStatusSnapshot) => void = () => undefined;
    const install = vi.fn(() => new Promise<RethraDesignHostUpdaterStatusSnapshot>((resolve) => {
    ...
    await act(async () => {
      status = downloadedStatus({
        installResult: {
          ...
          path: '/tmp/rethra-design-updater/Rethra Design Beta.dmg',
        },
      });
  ...
  it.each(['darwin', 'win32'] as const)('keeps accepted %s payload shutdown pending beyond the cleanup grace', async (platform) => {
    ...
    restoreHost = installMockRethraDesignHost({ host: { updater: {
  ...
  it('reacts to updater subscription events by showing the ready indicator only', async () => {
    const listeners = new Set<RethraDesignHostUpdaterStatusListener>();
    restoreHost = installMockRethraDesignHost({
      host: {
        updater: {
