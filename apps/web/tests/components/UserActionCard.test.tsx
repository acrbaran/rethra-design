...
describe('UserActionCard', () => {
  ...
  it('keeps full localized recovery labels actionable in the footer', () => {
        ...
        footerActions={
          <>
            <button type="button" onClick={onRecharge}>
              为 RethraDesign Cloud 充值
            </button>
            <button type="button" onClick={onRetry}>
              Retry the RethraDesign Cloud request
            </button>
          </>
        }
      />,
    ...
    const recharge = screen.getByRole('button', { name: '为 RethraDesign Cloud 充值' });
    const retry = screen.getByRole('button', {
      name: 'Retry the RethraDesign Cloud request',
    });
