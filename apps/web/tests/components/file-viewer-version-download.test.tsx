...
const {
  ...
  isRethraDesignHostAvailableMock,
  ...
  isRethraDesignHostAvailableMock: vi.fn(() => false),
...
vi.mock('../../src/runtime/exports', async () => {
  ...
  return {
    ...
    isRethraDesignHostAvailable: isRethraDesignHostAvailableMock,
...
describe('FileViewer version download actions', () => {
  afterEach(() => {
    cleanup();
    vi.clearAllMocks();
    isRethraDesignHostAvailableMock.mockReturnValue(false);
    vi.unstubAllGlobals();
  });

  it('routes current version PDFs through the main download PDF exporter', async () => {
    isRethraDesignHostAvailableMock.mockReturnValue(true);
