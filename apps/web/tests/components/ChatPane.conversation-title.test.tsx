...
import type { RunFailureDetail } from '@rethra-design/contracts';
  ...
  it('opens the profile-scoped console from the AMR recharge action', () => {
    ...
    render(
      <ChatPane
        ...
        onSelectConversation={vi.fn()}
        onDeleteConversation={vi.fn()}
        config={{ agentCliEnv: { amr: { RETHRA_DESIGN_AMR_PROFILE: 'test' } } } as unknown as AppConfig}
      />,
    );
    ...
    expect(`${parsedConsoleUrl.origin}${parsedConsoleUrl.pathname}`).toBe(
      'https://rethra-design.powerformer.net/cloud/dashboard',
    );
