...
describe('PluginShareMenu', () => {
  ...
  it('copies an install command for marketplace plugins using the registry entry name', async () => {
    renderMenu(
      make({
        id: 'mp-plugin',
        sourceKind: 'github',
        source: 'github:rethra-design/plugins/mp-plugin',
        marketplaceId: 'official',
        marketplaceEntryName: 'rethra-design/mp-plugin',
      }),
    );
    openPopover();
    clickItem('Copy install command');
    await Promise.resolve();
    expect(writes).toContain('od plugin install rethra-design/mp-plugin');
  });
  ...
  it('copies a README badge that links back to the marketplace detail page', async () => {
    renderMenu(make({
      ...
      title: 'Badge Plugin',
      marketplaceId: 'official',
      marketplaceEntryName: 'rethra-design/badge-plugin',
    }));
    openPopover();
    ...
    expect(writes.some((value) => (
      ...
      value.includes('https://rethra-design.invalid/plugins/badge-plugin')
    ))).toBe(true);
  });
  ...
  it('localizes the plugin action menu labels', () => {
    renderMenu(
      make({
        ...
        marketplaceId: 'official',
        marketplaceEntryName: 'rethra-design/zh-plugin',
  ...
  it('points Open in marketplace at the public rethra-design.invalid page for bundled plugins', () => {
    ...
    // rethra-design.invalid URL — not a local /marketplace path.
    expect(marketplaceLink?.getAttribute('href')).toBe(
      'https://rethra-design.invalid/plugins/plain/',
    );
  });

  it('builds a public rethra-design.invalid share link for bundled plugins', () => {
    expect(buildPluginShareUrl(make({ id: 'simple-deck' }))).toBe(
      'https://rethra-design.invalid/plugins/simple-deck/',
    );
  });

  it('builds a public rethra-design.invalid share link for community marketplace plugins', () => {
    ...
    ).toBe('https://rethra-design.invalid/plugins/registry-starter/');
  });
    ...
    expect(
      writes.some(
        (value) =>
          ...
          value.includes('https://rethra-design.invalid/plugins/registry-starter/'),
      ),
    ).toBe(true);
  });
  ...
  it('points Open in marketplace at the public page for community marketplace plugins', () => {
    ...
    expect(marketplaceLink?.getAttribute('href')).toBe(
      'https://rethra-design.invalid/plugins/registry-starter/',
    );
  });
  ...
  it('renders official bundled repo links as anchors', () => {
    ...
    const repoLinks = Array.from(
      container.querySelectorAll<HTMLAnchorElement>(
        'a.plugin-share-item[href="https://github.com/acrbaran/rethra-design"]',
      ),
    );
