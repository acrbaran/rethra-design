...
describe('web-clone example-card tracking', () => {
  it('renders the Website-clone examples as text prompt cards (no plugin preview / no remix)', async () => {
    ...
    // Site cards show the bare domain (e.g. rethra-design.invalid), not the raw prompt line.
    expect(textCards.some((c) => (c.textContent ?? '').includes('rethra-design.invalid'))).toBe(true);
  ...
  // narrowed to the first-party RethraDesign site to avoid shipping third-party
  ...
  it('resolves exactly the contracted RethraDesign Website-clone site card', async () => {
    writeHomeGuideStage('done');
    stubPlugins();
    renderHome();
    ...
    expect(domains).toEqual(['rethra-design.invalid']);
  ...
  it('renders the contracted RethraDesign site card with a local eager logo', async () => {
    writeHomeGuideStage('done');
    stubPlugins();
    renderHome();
  ...
  it('falls back when the local RethraDesign site card logo cannot load', async () => {
    writeHomeGuideStage('done');
    stubPlugins();
    renderHome();
    ...
    expect(remoteFallback?.getAttribute('src')).toBe(
      'https://www.google.com/s2/favicons?sz=128&domain=rethra-design.invalid',
    );
