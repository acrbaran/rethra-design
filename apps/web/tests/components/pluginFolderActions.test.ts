...
// `acrbaran/rethra-design`; `publish` drives `gh repo create / push` against the
...
describe('buildPluginFolderAgentActionPrompt', () => {
  ...
  describe('publish (repo-publish flow)', () => {
    ...
    it('bans the registry-submission CLI explicitly', () => {
      ...
      expect(prompt).toMatch(
        /Do NOT (call|route through) `?od plugin publish --to rethra-design`?/i,
      );
      expect(prompt).toMatch(
        /registry[- ]submission|registry-submission flow|RethraDesign PR/i,
      );
    });
  ...
  describe('contribute (PR-based flow)', () => {
    ...
    it('delegates RethraDesign PR creation to the deterministic plugin CLI helper', () => {
      expect(prompt).toContain('acrbaran/rethra-design');
      expect(prompt).toContain(`"$OD_NODE_BIN" "$OD_BIN" plugin rethra-design-pr ${FOLDER}`);
    });
    ...
    it('states the CLI-owned PR workflow instead of re-listing shell steps', () => {
      ...
      // ban ("Do NOT call the legacy `od plugin publish --to rethra-design`")
      // — verify the ban is in place, not the bare command.
      expect(prompt).toMatch(/do not call the legacy `od plugin publish --to rethra-design`/i);
    });
    ...
    it('interpolates the actual folder path into manifest and copy steps', () => {
      ...
      expect(prompt).toContain(`plugin rethra-design-pr ${FOLDER}`);
      expect(prompt).not.toContain('${folderPath}');
    });
