...
describe('MemorySection', () => {
  ...
  it('renders the memory tree and opens an entry editor from a tree node', async () => {
    ...
    const entry = {
      ...
      description: 'RethraDesign should evolve from accepted work',
      type: 'project',
  ...
  it('renders connector model authentication failures without raw provider JSON', async () => {
    ...
    globalThis.fetch = vi.fn(async (input: RequestInfo | URL) => {
          ...
          extractions: [
            {
              ...
              userMessagePreview: 'Suggest durable RethraDesign memories from connected apps.',
	  ...
	  expect(
	    screen.queryByText('Suggest durable RethraDesign memories from connected apps.'),
	  ).toBeNull();
	  expect(
	    within(document.querySelector('.memory-unified-list') as HTMLElement)
	      .queryByText('Suggest durable RethraDesign memories from connected apps.'),
	  ).toBeNull();
	  expect(screen.getByText('OpenAI authentication expired')).toBeTruthy();
    expect(
      screen.getByText('Connected apps were read, but RethraDesign could not turn that context into memory.'),
    ).toBeTruthy();
    expect(
