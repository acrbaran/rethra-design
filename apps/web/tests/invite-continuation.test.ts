...
function acceptResponse(overrides: Partial<WorkspaceInviteAcceptResponse> = {}): WorkspaceInviteAcceptResponse {
  return {
    ...
    continuation: {
      nonce: 'nonce-abc',
      deeplinkUrl:
        'rethradesign://workspace/invite/continue?workspace_id=ws-1&member_id=mem-7&invite_id=inv-42&nonce=nonce-abc',
      expiresAt: 10_000,
      fallbackDownloadUrl: 'https://rethra-design.example/download',
    },
  ...
  it('replaces a prior continuation (single slot)', () => {
    ...
    const next = pendingContinuationFromAccept(
      acceptResponse({
        ...
        continuation: {
          nonce: 'nonce-xyz',
          deeplinkUrl: 'rethradesign://workspace/invite/continue?workspace_id=ws-1&member_id=mem-7&invite_id=inv-99&nonce=nonce-xyz',
          expiresAt: 20_000,
          fallbackDownloadUrl: 'https://rethra-design.example/download',
        },
      }),
