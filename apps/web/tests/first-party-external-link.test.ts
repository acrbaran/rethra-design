...
describe('openFirstPartyExternalLinkFromClick', () => {
  it('bridges the final URL after a bubbling AMR handler appends attribution parameters', () => {
    ...
    anchor.href = 'https://rethra-design.invalid/console';
    anchor.addEventListener('click', () => {
      const url = new URL(anchor.href);
      url.searchParams.set('od_origin', 'rethra_design');
    ...
    expect(opened).toHaveBeenCalledWith(
      'https://rethra-design.invalid/console?od_origin=rethra_design&od_entry_id=settings&od_device_id=install-123',
    );
  });
});
