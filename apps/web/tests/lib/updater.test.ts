    ...
    const model = deriveUpdaterModel(
      downloadedStatus({
        incoming: {
          arch: 'arm64',
          artifact: {
            name: 'Rethra Design Beta 1.2.3-beta.5.dmg',
            platformKey: 'macAppleSilicon',
            type: 'dmg',
            url: 'https://fixture.test/Rethra Design Beta 1.2.3-beta.5.dmg',
          },
          channel: 'beta',
  ...
  it('routes status, check, download, install, and quit requests through host helpers with flexible payloads', async () => {
    ...
    const install = vi.fn(async () => downloadedStatus({
      installResult: {
        dryRun: true,
        openedAt: '2026-05-19T00:00:00.000Z',
        path: status.downloadPath ?? '/tmp/rethra-design-updater/Rethra Design Beta.dmg',
      },
    }));
