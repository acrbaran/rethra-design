...
describe('message center client', () => {
  ...
  it('clears legacy anonymous window keys with anonymous state', () => {
    ...
    storage.set('rethra-design.message-center.anonymous-started-at.v1', '2026-07-16T00:00:00.000Z');
    storage.set('rethra-design.message-center.anonymous-messages.v1', '[]');
    storage.set('rethra-design.message-center.anonymous-read-ids.v1', '[]');
    clearAnonymousState(adapter);
    expect(storage.has('rethra-design.message-center.anonymous-started-at.v1')).toBe(false);
    expect(storage.has('rethra-design.message-center.anonymous-messages.v1')).toBe(false);
    expect(storage.has('rethra-design.message-center.anonymous-read-ids.v1')).toBe(false);
  });
