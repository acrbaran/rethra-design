...
describe('derivePluginSourceLinks · github sources', () => {
  it('builds tree URL with ref + subpath when source has both', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'github',
        source:     'github:rethra-design/plugins@v1.2.0/make-a-deck',
      }),
    );
    expect(out.sourceUrl).toBe('https://github.com/rethra-design/plugins/tree/v1.2.0/make-a-deck');
    expect(out.sourceLabel).toBe('rethra-design/plugins @v1.2.0/make-a-deck');
    expect(out.sourceKindLabel).toBe('GitHub');
    expect(out.contributeUrl).toBe('https://github.com/rethra-design/plugins/issues/new');
  ...
  it('falls back to repo-root URL when no ref / subpath', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'github',
        source:     'github:rethra-design/plugins',
      }),
    );
    expect(out.sourceUrl).toBe('https://github.com/rethra-design/plugins');
    expect(out.sourceLabel).toBe('rethra-design/plugins');
  });
  ...
  it('uses pinnedRef when source has no inline ref', () => {
    ...
    expect(out.sourceUrl).toBe('https://github.com/rethra-design/plugins/tree/a1b2c3d4');
    expect(out.sourceLabel).toBe('rethra-design/plugins @a1b2c3d4');
  });
  ...
  it('preserves slash-separated branch refs (release/1.0)', () => {
    ...
    expect(out.sourceUrl).toBe('https://github.com/rethra-design/plugins/tree/release/1.0');
  });
...
describe('derivePluginSourceLinks · url + local + bundled sources', () => {
  ...
  it('routes bundled official sources to the RethraDesign repo', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'bundled',
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        manifest: {
          name:    'p',
          version: '1.0.0',
          author:  { name: 'RethraDesign', url: 'https://github.com/nexu-io' },
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.authorName).toBe('RethraDesign');
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        ...
        manifest: {
          name:    'p',
          version: '1.0.0',
          homepage: 'https://github.com/acrbaran/rethra-design',
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.contributeUrl).toBe('https://github.com/acrbaran/rethra-design/issues/new');
    expect(out.contributeOnGithub).toBe(true);
    expect(out.homepageUrl).toBe('https://github.com/acrbaran/rethra-design');
  });
...
describe('authorInitials', () => {
  it('builds two-letter monograms', () => {
    expect(authorInitials('RethraDesign')).toBe('OD');
