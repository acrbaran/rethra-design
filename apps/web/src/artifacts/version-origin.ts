import type { ArtifactOrigin, ProjectFileVersion } from '@rethra-design/contracts';
import type { ArtifactExportResultProps } from '@rethra-design/contracts/analytics';

const CONTENT_DIGEST_RE = /^[a-f0-9]{64}$/u;
const ORIGIN_ID_RE = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u;
const EXTERNAL_PLUGIN_IDS = new Set(['rethra-design']);
...
function isValidArtifactOrigin(origin: ArtifactOrigin): boolean {
  if (
    origin.entrySurface !== 'rethra_design_ui'
...
export async function artifactExportOriginProps(
  ...
  const unknownBase = {
    entry_surface: 'rethra_design_ui' as const,
