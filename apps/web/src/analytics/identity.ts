...
// headers (see @rethra-design/contracts/analytics).

import type { AnalyticsClientType } from '@rethra-design/contracts/analytics';
import { detectRethraDesignHostClientType } from '@rethra-design/host';

const ANONYMOUS_ID_KEY = 'rethra-design:analytics.anonymous_id';
const SESSION_ID_KEY = 'rethra-design:analytics.session_id';
const RUN_TURN_INDEX_KEY = 'rethra-design:analytics.run_turn_index';
// Per-project counter keys are this prefix + the project id (localStorage).
const PROJECT_TURN_INDEX_KEY_PREFIX = 'rethra-design:analytics.project_turn_index:';
...
const FIRST_SESSION_ID_KEY = 'rethra-design:analytics.first_session_id';
...
// Desktop packaged builds install the RethraDesign host bridge so the
...
export function detectClientType(): AnalyticsClientType {
  ...
  return detectRethraDesignHostClientType();
}
