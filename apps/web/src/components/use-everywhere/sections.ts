// Content fixtures for the "Use RethraDesign everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'RethraDesign works wherever your agent works',
    intro:
      'RethraDesign is more than a window — it is a local privileged daemon ' +
    ...
    bullets: [
      ...
      'MCP server — wires RethraDesign as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without RethraDesign at all.',
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          ...
          '# Then start RethraDesign in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
      'The daemon writes to `./.od/` (project-local) by default. Set ' +
      '`OD_DATA_DIR=~/.rethra-design` to share data across projects.',
  },
  {
    id: 'cli',
    tabLabel: 'CLI · od',
    heading: 'Drive RethraDesign from any shell',
    intro:
  ...
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose RethraDesign as an MCP server to any coding agent',
    intro:
      'RethraDesign ships with a Model Context Protocol server (`od mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover RethraDesign tools ' +
    ...
    footer:
      'In the RethraDesign app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    intro:
      ...
      'Server-Sent Events with the contract types in `@rethra-design/contracts`.',
    bullets: [
    ...
    footer:
      ...
      '`@rethra-design/contracts` — import them in your script for full ' +
      'autocomplete without wiring a generator.',
  },
  {
    id: 'skills',
    tabLabel: 'Skills & headless',
    heading: 'Drop-in Skills for any agent — even without RethraDesign running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). RethraDesign extends the format with the ' +
      '`od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside RethraDesign and by a vanilla ' +
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + RethraDesign extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        body:
          ...
          '# Shell 1: start RethraDesign after ingesting.\n' +
          'pnpm tools-dev\n' +
