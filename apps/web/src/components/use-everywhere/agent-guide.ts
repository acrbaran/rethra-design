...
// everything it needs to install RethraDesign, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional RethraDesign version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# RethraDesign — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants RethraDesign wired into your workflow. RethraDesign is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported RethraDesign version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the RethraDesign app (packaged).');
  lines.push('');
  if (installInfo) {
