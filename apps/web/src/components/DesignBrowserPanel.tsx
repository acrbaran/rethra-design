...
import type { TrackingReferenceBoardCategory } from '@rethra-design/contracts/analytics';
...
export function browserUsePrompt(action: BrowserUseAction, context: BrowserUsePromptContext = {}): string {
  ...
  return [
    '@agent-browser',
    '',
    'Use the selected RethraDesign Browser tab as the bound target.',
    'Browser tab context:',
    `- tab: ${tabLabel}`,
    `- title: ${title}`,
    `- url: ${url}`,
...
const ADD_IMAGE_TO_CHAT_MESSAGE = '__rethra_design_add_image_to_chat__:';
const browserImageHoverScript = (addToChatLabel: string) => String.raw`(() => {
  const rootId = '__rethra_design_image_hover_layer__';
  window.__rethraDesignImageHoverCleanup?.();
  ...
  window.__rethraDesignImageHoverCleanup = () => {
    controller.abort();
    layer.remove();
    delete window.__rethraDesignImageHoverCleanup;
  };
  return true;
})()`;
...
export function DesignBrowserPanel({
  ...
  const desktopHostAvailable = isRethraDesignHostAvailable();
  ...
  async function savePageSnapshot(
    ...
    try {
      ...
      const canRethraDesignFiles = Boolean(onRethraDesignFiles);
      setStatusMessage({
        actionFileName: manifestFile,
        actionLabel: canRethraDesignFiles
          ? t('designBrowser.status.viewDesignFiles')
          : t('workspace.designFiles'),
        actionTarget: canRethraDesignFiles ? 'design-files' : 'file',
        message,
        source: 'page-snapshot',
      });
      emitPageSnapshotToast({
        ...
        <div className="db-status" role="status">
          ...
          {archiveSaving ? (
            ...
            <button
              ...
              className="db-status-action"
              onClick={() => {
                if (statusAction.actionTarget === 'design-files' && onRethraDesignFiles) {
                  onRethraDesignFiles();
                } else {
