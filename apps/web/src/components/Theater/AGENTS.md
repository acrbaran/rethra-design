...
| `hooks/useCritiqueTheaterEnabled.ts` | Reads the Settings toggle from `rethra-design:config` localStorage and stays in sync with cross-tab `storage` events plus the same-tab `rethra-design:critique-theater-toggle` CustomEvent. Pairs with the `setCritiqueTheaterEnabled` setter so a Settings save reflects in every mounted hook without a reload. |
...
2. Add an i18n key to `apps/web/src/i18n/types.ts`, then define it in
   ...
   English.
3. Run `pnpm --filter @rethra-design/web exec vitest run tests/components/Theater`
   before pushing. The suite has 100+ cases pinning the reducer
   shape, SSE validation, host lifecycle, and component renders.
4. Visual regression: run `pnpm --filter @rethra-design/e2e test:ui:extended`
