...
export const BROWSER_PAGE_ARCHIVE_SCHEMA = 'rethra-design.browser-page-archive.v1';
...
(() => {
  const cancel = window.__rethraDesignBrowserPickerCancel;
...
export function browserElementPickerScript(filePath: string): string {
  return `
(() => new Promise((resolve) => {
  const previousCancel = window.__rethraDesignBrowserPickerCancel;
  if (typeof previousCancel === 'function') {
    try { previousCancel(); } catch (_) {}
  }
  const filePath = ${JSON.stringify(filePath)};
  const style = document.createElement('style');
  style.setAttribute('data-rethra-design-browser-picker', 'true');
  style.textContent = [
    '* { cursor: crosshair !important; }',
    '.__rethra_design_browser_pick_hover__ { outline: 2px solid #1677ff !important; outline-offset: 2px !important; }',
    '.__rethra_design_browser_pick_hover__::selection { background: rgba(22, 119, 255, 0.22) !important; }'
  ].join('\\n');
  ...
  function setHover(el) {
    ...
    if (hovered) hovered.classList.remove('__rethra_design_browser_pick_hover__');
    hovered = el;
    if (hovered) hovered.classList.add('__rethra_design_browser_pick_hover__');
  }
  function cleanup(result) {
    ...
    if (hovered) hovered.classList.remove('__rethra_design_browser_pick_hover__');
    style.remove();
    window.__rethraDesignBrowserPickerCancel = null;
    resolve(result || null);
  }
  function onMove(ev) {
  ...
  window.__rethraDesignBrowserPickerCancel = () => cleanup(null);
