...
//   - Copy README badge       (RethraDesign powered, includes link)
...
import { pluginShareUrl } from '@rethra-design/contracts';

const PUBLIC_RETHRA_DESIGN_MARKETPLACE_ID = 'official';
...
export function buildPluginShareUrl(record: InstalledPluginRecord): string | null {
  // Only plugins with a public detail page on rethra-design.invalid get a shareable
  ...
  const hasPublicPage =
    ...
    record.sourceMarketplaceId === PUBLIC_RETHRA_DESIGN_MARKETPLACE_ID ||
...
function buildMarkdownBadge(record: InstalledPluginRecord, url: string): string {
  return `[![${record.title} — RethraDesign plugin](https://img.shields.io/badge/Open%20Design-${encodeURIComponent(record.title)}-d65a31?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%2C)](${url})`;
}
...
export function PluginShareMenu({ record, variant = 'default' }: Props) {
  ...
  openItems.push({
    ...
    icon: 'eye',
    // Prefer the public rethra-design.invalid detail page; fall back to the in-app
