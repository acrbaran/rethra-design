          ...
          {onShareAction ? (
            ...
            >
              ...
              <button
                type="button"
                className="plugins-home__action plugins-home__action--secondary plugins-home__action--compact"
                onClick={() => onShareAction(record, 'contribute-rethra-design')}
                disabled={pendingAny || shareBusy}
                aria-busy={sharePendingAction === 'contribute-rethra-design' ? 'true' : undefined}
                aria-label={t('pluginCard.contributeAria', { title })}
                title={t('pluginCard.contributeTitle')}
                data-testid={`plugins-home-contribute-rethra-design-${record.id}`}
              >
                <Icon
                  name={sharePendingAction === 'contribute-rethra-design' ? 'spinner' : 'share'}
                  size={12}
                />
                <span>{sharePendingAction === 'contribute-rethra-design' ? t('pluginCard.starting') : t('pluginCard.contribute')}</span>
              </button>
            </div>
          ) : null}
        </div>
