...
export function buildPluginFolderAgentActionPrompt(
  ...
  return [
    ...
    `Manifest: \`${folderPath}/rethra-design.json\``,
    '',
...
// `contribute` opens a draft PR against the `acrbaran/rethra-design` community
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the RethraDesign community catalog at `acrbaran/rethra-design`.',
    ...
    `Manifest: \`${folderPath}/rethra-design.json\``,
    '',
    'Run this deterministic RethraDesign CLI workflow from the current project workspace:',
    '',
    `\`"$OD_NODE_BIN" "$OD_BIN" plugin rethra-design-pr ${folderPath}\``,
    '',
    ...
    '- Do NOT call the legacy `od plugin publish --to rethra-design` CLI — that flow produces an issue URL, which is the old path we are replacing.',
  ].join('\n');
}
...
// submission path — `od plugin publish --to rethra-design` produces an
// RethraDesign issue URL and belongs to the "RethraDesign PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/rethra-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to rethra-design`. That command emits an RethraDesign issue URL and belongs to the "RethraDesign PR" button.',
    '',
    'Run this deterministic RethraDesign CLI workflow from the current project workspace:',
    '',
    ...
    '',
    '**Hard constraints.** Treat these as inviolable:',
    '- Do NOT call `od plugin publish --to rethra-design` (or any `--to <catalog>` variant). That is the registry-submission flow, not the repository-publish flow.',
