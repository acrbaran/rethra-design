...
export { isRethraDesignHostAvailable } from '@rethra-design/host';
...
export function buildDesignManifestContent(opts: {
  ...
  const title = opts.title || 'RethraDesign artifact';
  ...
  return JSON.stringify({
    schema: 'rethra-design.design-manifest.v1',
    title,
    kind: opts.kind ?? 'html',
    entryFile,
    sourceFiles: {
...
- Keep generated product UI free of RethraDesign chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or RethraDesign chrome in the production UI.
...
export async function captureHostRegionSnapshot(
  ...
  if (!isRethraDesignHostAvailable()) return null;
