...
export function derivePluginSourceLinks(
  ...
  const authorName = typeof author.name === 'string' && author.name.trim().length > 0
    ? author.name.trim()
    : null;
  const authorProfileUrl = officialBundled ? RETHRA_DESIGN_REPO_URL : safeHttpUrl(author.url);
  const homepageUrl = officialBundled ? RETHRA_DESIGN_REPO_URL : safeHttpUrl(homepageRaw);
  ...
  if (record.sourceKind === 'github') {
    ...
    sourceUrl = RETHRA_DESIGN_REPO_URL;
    sourceLabel = RETHRA_DESIGN_REPO_LABEL;
  } else {
 ...
 * "RethraDesign" still yield a two-letter monogram ("OD").
 */
