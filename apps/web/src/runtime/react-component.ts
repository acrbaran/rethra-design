    ...
    <script>
      (function(){
        ...
        try {
          ...
          (0, eval)(compiled);
          var Component = window.__RethraDesignComponent ||
...
window.__RethraDesignComponent = window.__RethraDesignComponent || (${componentFallbackExpression(transformed.defaultName)});`;
}
...
function transformExports(source: string): { code: string; defaultName: string | null } {
  ...
  code = code.replace(
    ...
    (_match, name: string | undefined) => {
      defaultName = name || 'RethraDesignComponent';
  ...
  code = code.replace(/export\s+default\s+/g, () => {
    defaultName = 'RethraDesignComponent';
    return 'const RethraDesignComponent = ';
  });
  code = code.replace(
