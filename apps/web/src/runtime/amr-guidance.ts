...
import type { RunFailureAction } from '@rethra-design/contracts';
import { byokApiKeyIsEditableInSettings } from '../utils/byokProvider';

// AMR model-gateway console (account, balance, top-up, plans).
// `source=rethra_design` tags the landing page_view so vela analytics can
// attribute the visit to RethraDesign (per-product revenue/traffic attribution).
//
...
const AMR_CONSOLE_PATH = '/dashboard?source=rethra_design';
...
const AMR_CONSOLE_URL_BY_PROFILE: Record<string, string> = {
  ...
  test: 'https://rethra-design.powerformer.net/cloud/dashboard?source=rethra_design',
  local: 'http://localhost:5173/dashboard?source=rethra_design',
};
...
export type RunFailureTitleKey =
  ...
  // S04 Rethra Design 智能体(主语固定,卡内一键授权)。
 ...
 * carries 〔switch to RethraDesign Cloud〕 in its primary slot (`cloudSwitchCta`),
...
/**
 * Rung 3 — "switch to the Rethra Design agent" — is not an answer for a run that
...
const AGENT_AGNOSTIC_DETAIL_FAILURE_UI: Record<string, RunFailureUi> = {
  ...
  // The fix is updating RethraDesign to a build that bundles a compatible
  ...
  //
  // No switch card: 推荐 Rethra Design 智能体 is reserved for「本地 agent 没登录」
...
function resolveRunFailureUiIgnoringSelfPromotion(
  ...
  if (agentId === CLOUD_NATIVE_AGENT_ID) {
    ...
    if (
      ...
      // 「Rethra Design 尚未登录」—— 主语固定,和 S02 那句**不是**同一句话:
