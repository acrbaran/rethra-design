...
description: |
  ...
  Rethra Design browser-tab context, testing web apps, dogfooding Rethra Design
  previews, QA, bug hunts, or reviewing app quality. Prefer local Rethra Design
...
Use `agent-browser` for local Rethra Design preview validation: inspect rendered
...
For selected Rethra Design browser tabs and browser-use/browser-harness-style
...
5. If the selected Rethra Design context only provided a URL/title and no browser
...
Do not run Rethra Design's own daemon CLI as a browser automation tool. Commands
...
With the Rethra Design preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `Rethra Design`, current URL under `127.0.0.1:17573`,
visible Rethra Design UI text in the snapshot, and a screenshot at
