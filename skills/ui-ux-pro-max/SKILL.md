...
name: ui-ux-pro-max
description: |
  Catalog-only UI/UX Pro Max entry. The full upstream templates, data, and search workflow are not bundled in Rethra Design.
triggers:
  - "ui ux patterns"
  - "design patterns"
  - "ux heuristics"
...
## Current Rethra Design scope

Rethra Design currently ships this entry as discovery metadata only. If this `SKILL.md`
...
This catalogue entry advertises the skill in Rethra Design so the agent
...
If those upstream files are not installed, explain that Rethra Design only has the
catalog entry for this skill and ask whether to continue with Rethra Design's
