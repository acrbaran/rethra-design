...
checked into the Rethra Design repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Rethra Design agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Rethra Design and
...
2. Replace the contents of this directory with the upstream snapshot,
   preserving only this `README.md` and any Rethra-Design-specific notes
   inside `SKILL.md`'s `> **Rethra Design integration.**` blockquote.
...
flow (including how Rethra Design behaves when Codex is not installed).
