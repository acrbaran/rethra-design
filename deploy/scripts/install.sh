#!/usr/bin/env bash
# RethraDesign — One-Click Installer
...
printf "${BOLD}  │${RESET}   ${CYAN}◈${RESET}  ${BOLD}RethraDesign${RESET}                     ${BOLD}│${RESET}\n"
...
# ---------------------------------------------------------------------------
# 2b. Check if RethraDesign is already running
# ---------------------------------------------------------------------------
if $CONTAINER_CMD ps --filter "name=rethra-design" --format '{{.Names}}' 2>/dev/null | grep -q 'rethra-design'; then
  STATUS="$($CONTAINER_CMD inspect --format '{{.State.Status}}' rethra-design 2>/dev/null || echo 'unknown')"
  IMAGE="$($CONTAINER_CMD inspect --format '{{.Config.Image}}' rethra-design 2>/dev/null || echo 'unknown')"
  PORTS="$($CONTAINER_CMD port rethra-design 2>/dev/null || echo 'unknown')"

  error "RethraDesign is already running."
  printf "\n"
  printf "  Container:  rethra-design\n"
  printf "  Status:     %s\n" "$STATUS"
  printf "  Image:      %s\n" "$IMAGE"
  printf "  Ports:      %s\n" "$PORTS"
  printf "\n"
...
RETHRA_DESIGN_IMAGE=${IMAGE}
RETHRA_DESIGN_PORT=${PORT}
RETHRA_DESIGN_ALLOWED_ORIGINS=${ALLOWED_ORIGINS}
RETHRA_DESIGN_MEM_LIMIT=${MEM_LIMIT}
...
if [ "$OS" = "Linux" ] && [ "$OPT_NO_SYSTEMD" = "0" ]; then
  if command -v systemctl >/dev/null 2>&1; then
    ...
    SYSTEMD_UNIT="${SYSTEMD_DIR}/rethra-design.service"
    ...
    {
      echo "[Unit]"
      echo "Description=RethraDesign daemon (${CONTAINER_RUNTIME} Compose)"
    ...
    systemctl --user enable rethra-design 2>/dev/null || true
    ok "systemd unit installed: rethra-design.service"
    step "Manage with: systemctl --user start|stop|status rethra-design"
  else
...
printf "  Image:        %s\n" "$IMAGE"
printf "  Data volume:  rethra_design_data\n"
printf "  Config:       %s\n" "$ENV_FILE"
if [ "$OS" = "Linux" ] && [ -f "${HOME}/.config/systemd/user/rethra-design.service" ]; then
  printf "  Service:      systemd (rethra-design.service)\n"
fi
printf "\n"
printf "  Next steps:\n"
