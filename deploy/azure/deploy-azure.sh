...
# Deploy RethraDesign to Azure from the Bicep templates in this directory.
#
...
RESOURCE_GROUP="${RESOURCE_GROUP:-rethra-design-rg}"
LOCATION="${LOCATION:-eastus}"
NAME="${NAME:-rethra-design}"
IMAGE="${IMAGE:-docker.io/vanjayak/rethra-design:latest}"
API_TOKEN="${API_TOKEN:-}"
EXTRA_ALLOWED_ORIGINS="${EXTRA_ALLOWED_ORIGINS:-}"
DEPLOYMENT_NAME="${DEPLOYMENT_NAME:-rethra-design}"
...
Options:
  ...
  --resource-group <name>         resource group to deploy into (default: rethra-design-rg)
  --location <region>             Azure region, used to create the group (default: eastus)
  --name <name>                   base name for resources (default: rethra-design)
  --image <image-ref>             container image (default: docker.io/vanjayak/rethra-design:latest)
  ...
  --deployment-name <name>        ARM deployment name (default: rethra-design)
  -h, --help

Examples:
  deploy/azure/deploy-azure.sh --target app-service --resource-group od-rg --location westeurope
  deploy/azure/deploy-azure.sh --target aci --resource-group od-rg --image docker.io/vanjayak/rethra-design@sha256:<digest>
EOF
}
