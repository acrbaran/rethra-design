...
test("cleanup removes the default and legacy swap paths", async () => {
  ...
  assert.equal(result.state.RM_TARGETS, "/swapfile-colima-build:/swapfile-rethra-design-build");
});
...
test("cleanup removes custom swap paths when force is enabled", async () => {
  ...
  assert.equal(result.state.RM_TARGETS, "/custom-swapfile:/swapfile-rethra-design-build");
});
