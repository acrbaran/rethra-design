...
source_profile="${RETHRA_DESIGN_RELEASE_PROFILE:-}"
...
prepare_mac_signing() {
  ...
  local cert_path="$RUNNER_TEMP/rethra-design-signing.p12"
...
install_mac_signing_keychain() {
  local cert_path="$1"
  local helper="${RETHRA_DESIGN_MAC_SIGNING_HELPER:-}"
  if [ -z "$helper" ]; then
    return 1
  fi

  local password_path="$RUNNER_TEMP/rethra-design-signing.p12.password"
  ...
  export CSC_KEYCHAIN="${RETHRA_DESIGN_MAC_SIGNING_KEYCHAIN:-/Library/Keychains/rethra-design-release-signing.keychain}"
  local wrapper_dir="${RETHRA_DESIGN_MAC_SIGNING_WRAPPER_DIR:-/usr/local/libexec/rethra-design/wrappers}"
...
capture_framework_diagnostics() {
  ...
  built_framework="$tools_pack_dir/out/mac/namespaces/$namespace/builder/mac-arm64/Rethra Design Beta.app/Contents/Frameworks/Electron Framework.framework"
...
if [ "$sign_mode" != "no" ]; then
  ...
  if [ -n "${RETHRA_DESIGN_MAC_SIGNING_HELPER:-}" ]; then
