...
source_appimage="$RUNNER_TEMP/tools-pack/out/linux/namespaces/$TOOLS_PACK_NAMESPACE/builder/Rethra Design-$TOOLS_PACK_NAMESPACE.AppImage"
if [ ! -f "$source_appimage" ]; then
  echo "expected AppImage not found at $source_appimage" >&2
  exit 1
fi

versioned_appimage="rethra-design-${RELEASE_VERSION}${asset_suffix}-linux-x64.AppImage"
