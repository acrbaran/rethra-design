...
$sourceInstaller = Join-Path $builderDir "Rethra Design-${env:TOOLS_PACK_NAMESPACE}-setup.exe"
$sourceZip = Join-Path $builderDir "Rethra Design-${env:TOOLS_PACK_NAMESPACE}-portable.zip"
...
$versionedInstaller = "rethra-design-${env:RELEASE_VERSION}$assetSuffix-win-x64-setup.exe"
$versionedZip = "rethra-design-${env:RELEASE_VERSION}$assetSuffix-win-x64-portable.zip"
...
$releaseNotes = if ([string]::IsNullOrWhiteSpace($env:RELEASE_NOTES)) {
  "Rethra Design ${env:RELEASE_VERSION}$assetSuffix"
} else {
  $env:RELEASE_NOTES
}
