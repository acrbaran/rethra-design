...
test('[P0] @critical onboarding lets AMR Cloud sign in and complete setup after the login poll succeeds', async ({ page }) => {
  ...
  // Signed-out cloud landing: the primary button reads "Sign in to RethraDesign
  ...
  await expect(primary).toHaveText(/Sign in to RethraDesign|登录 RethraDesign/i);
  ...
  await continueWithModelSource(page, /RethraDesign Hosted/i);
...
test('[P0] signed-out onboarding can open Local CLI setup without Cloud authorization', async ({ page }) => {
  ...
  await expect(cloudPrimaryButton(page)).toHaveText(/Sign in to RethraDesign|登录 RethraDesign/i);
...
test('[P0] completed BYOK setup stays usable while the unrelated Cloud session is signed out', async ({ page }) => {
  ...
  // PRODUCT INVARIANT: Cloud identity gates RethraDesign Cloud execution only.
...
async function gotoOnboarding(page: Page) {
  ...
  // step now opens on a centered RethraDesign Cloud sign-in landing whose
...
function connectLandingHeading(page: Page): Locator {
  return page.getByRole('heading', { name: /Sign in to RethraDesign|登录 RethraDesign/i });
}
