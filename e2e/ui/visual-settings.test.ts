...
test('[P1] captures the settings RethraDesign account balance surface', async ({ page }) => {
  ...
  await expect(amrCard).toContainText('RethraDesign');
  ...
  await captureVisual(page, 'visual-settings-rethra-design-account');
});
