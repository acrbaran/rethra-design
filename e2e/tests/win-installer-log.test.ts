...
describe('working Windows installer overwrite log contract', () => {
  it('accepts the current remove, extract, and launcher-runtime sync lifecycle', () => {
    const lines = [
      ...
      'event=install_dir_before_remove target=C:\\Rethra Design exists=1',
      'install dir remove exit=0',
      'event=install_dir_after_remove target=C:\\Rethra Design exists=0',
      'payload base extraction exit=0',
      'payload overlay extraction exit=0',
      'event=install_dir_after_extract target=C:\\Rethra Design exists=1',
      'event=installed_exe_after_extract target=C:\\Rethra Design\\Rethra Design.exe exists=1',
  ...
  it('reports lifecycle gaps without requiring the reverted transactional markers', () => {
    const lines = [
      'existing installation found; silent install will overwrite it',
      'event=install_dir_after_quarantine target=C:\\Rethra Design.back exists=1',
      'event=install_dir_after_commit target=C:\\Rethra Design exists=1',
