...
describe("postinstall script contract", () => {
  ...
  it("[P2] keeps postinstall build targets aligned with dist-backed workspace bins", () => {
    ...
    expect(dependencySpecifier(rootManifest, "@rethra-design/daemon")).toBe("workspace:*");
  ...
  it("[P2] skips absent tsconfig targets in partial install contexts on the default path", () => {
    ...
    try {
      writeTarget(sandbox, "packages/release", { name: "@rethra-design/release" });
      writeTarget(sandbox, "packages/contracts", {
        dependencies: { "@rethra-design/release": "workspace:*" },
        name: "@rethra-design/contracts",
      });
      writeTarget(sandbox, "packages/components", {
        dependencies: { "@rethra-design/contracts": "workspace:*" },
        name: "@rethra-design/components",
      });
      writeTarget(sandbox, "apps/daemon", { name: "@rethra-design/daemon", tsconfig: false });
      const invocationLog = writePnpmStub(sandbox);

      const result = runFixturePostinstall(sandbox, { RETHRA_DESIGN_POSTINSTALL_CONCURRENCY: "" });
  ...
  it("[P2] preserves workspace dependency ordering when postinstall builds in parallel", () => {
    ...
    try {
      ...
      writeTarget(sandbox, "packages/download", { name: "@rethra-design/download" });
      const invocationLog = writePnpmStub(sandbox);

      const result = runFixturePostinstall(sandbox, { RETHRA_DESIGN_POSTINSTALL_CONCURRENCY: "2" });
      expect(result.status, String(result.stderr)).toBe(0);
  ...
  it("[P2] rebuilds better-sqlite3 when the native addon cannot load", () => {
    ...
    try {
      ...
      expect(readStubEvents(invocationLog)).toContainEqual({
        args: ["--filter", "@rethra-design/daemon", "rebuild", "better-sqlite3"],
        event: "start",
        target: "",
      });
    } finally {
