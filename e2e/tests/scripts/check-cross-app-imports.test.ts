...
const registry: AppDirectoryRegistry = {
  packageNameByDirectory: new Map([
    ["daemon", "@rethra-design/daemon"],
    ["web", "@rethra-design/web"],
  ]),
};
...
test("cross-app import check rejects another app's package name", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "import type { RunRecord } from '@rethra-design/daemon/src/server.ts';",
      "const mod = await import('@rethra-design/daemon');",
      "const legacy = require('@rethra-design/daemon/dist/cli.js');",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects another app's package name in require.resolve", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "const daemonManifest = require.resolve('@rethra-design/daemon/package.json');",
    registry,
  );

  assert.equal(violations.length, 1);
  assert.equal(violations[0]?.specifier, "@rethra-design/daemon/package.json");
  assert.equal(violations[0]?.targetApp, "daemon");
  assert.equal(violations[0]?.lineNumber, 1);
});
...
test("cross-app import check rejects createRequire-based cross-app resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "const directDaemonManifest = createRequire(import.meta.url).resolve('@rethra-design/daemon/package.json');",
      "const aliasDaemonManifest = makeRequire(import.meta.url).resolve('@rethra-design/daemon/package.json');",
      "const nodeRequire = nodeModule.createRequire(import.meta.url);",
      "const daemonCli = nodeRequire('@rethra-design/daemon/dist/cli.js');",
      "const daemonPackageJson = nodeRequire.resolve('@rethra-design/daemon/package.json');",
    ].join("\n"),
    registry,
  );
  ...
  assert.deepEqual(
    ...
    [
      "@rethra-design/daemon/package.json",
      "@rethra-design/daemon/package.json",
      "@rethra-design/daemon/dist/cli.js",
      "@rethra-design/daemon/package.json",
    ],
  );
  assert.deepEqual(
...
test("cross-app import check rejects CommonJS node:module namespace createRequire resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      'nodeRequire.resolve("@rethra-design/daemon/package.json");',
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects cross-app imports from app-owned mjs entrypoints", () => {
  ...
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { startDaemon } from '@rethra-design/daemon/src/server.ts';",
    registry,
  );
...
test("cross-app import check allows packages, same-app relatives, and externals", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "import type { ChatRunEvent } from '@rethra-design/contracts';",
...
test("cross-app import check ignores quoted snippets and comments", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "const snippet = \"import { x } from '@rethra-design/daemon/src/server.ts';\";",
      "// import { y } from '@rethra-design/daemon/src/server.ts';",
      "/*",
      "require('@rethra-design/daemon/dist/cli.js');",
      "*/",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check allows allowlisted packaged -> desktop main export", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { applyOsLocaleSwitch, createSplashWindow } from '@rethra-design/desktop/main';",
    { packageNameByDirectory: new Map([["packaged", "@rethra-design/packaged"], ["desktop", "@rethra-design/desktop"]]) },
  );
...
test("app registry loading fails loudly when an app manifest is malformed", async () => {
  const appsRoot = await mkdtemp(path.join(os.tmpdir(), "rethra-design-apps-"));
