...
Rethra Design is a local-first design workspace where projects contain generated design files and agent conversations. This glossary records domain language only, not implementation details.
...
**Active Project**:
The project the user most recently interacted with in the Rethra Design UI and that MCP tools may use when no project is specified.
...
**AMR Cloud**:
The user-facing cloud runtime option for Rethra Design's official model router, shown in onboarding and login-oriented product surfaces.
...
The separately owned release contract that provides the native AMR CLI builds Rethra Design can package.
_Avoid_: Rethra Design release channel, package build step, source checkout
...
_Avoid_: Rethra Design supported platforms, release channel, future platform promise
...
- The **AMR CLI Distribution Contract** is owned separately from Rethra Design; Rethra Design release packaging consumes it instead of defining the native CLI release itself.
