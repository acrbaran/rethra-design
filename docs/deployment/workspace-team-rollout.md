...
本手册用于把 Workspace Team 从 Vela `feature-test` 推进到 `test`，最终再发布到 production。它同时覆盖 Vela DB/API/Link/Web/CLI 和 Rethra Design 桌面包；任何一层没有明确版本和验收证据，都不能把整体标记为已上线。
...
| Rethra Design owner | CLI pin、beta/prerelease/stable 包和 updater 元数据            |
...
版本必须是不可变标识：Git SHA、ECR digest、npm 精确版本、Rethra Design release version。分支名或 `latest` 不能单独作为证据。
...
### 2.2 Rethra Design production 目前是显式关闭状态

当前 Rethra Design 代码只允许 `feature-test` 和 `test` profile 启用 Workspace Team transport；`prod` 即使拿到 Vela Web URL 也会保持关闭。证据在：
...
- 未登录 Rethra Design 仍能使用本地 Coding Agent 和 BYOK，不能因 AMR 不可用被余额弹窗阻断。
...
6. Vela Web。
7. Vela CLI 精确版本发布。
8. Rethra Design pin 新 CLI，构建对应环境的包。
...
记录 workflow 产出的精确 `@powerformer/vela-cli@<version>-test.<n>`，确认 macOS arm64/x64、Windows x64 和 Linux 包发布与 smoke 均成功。随后在 Rethra Design 更新 `tools/pack/package.json` 的精确 pin 和 lockfile；不能依赖 npm dist-tag 漂移。
...
`release-beta` 必须报告 `release_state=complete`。核对 beta metadata 的 `github.commit` 等于冻结的 Rethra Design SHA，平台 artifact 均有 checksum；应用身份必须是 `Rethra Design Beta`。
...
用 `amr_profile=test` 重跑 Rethra Design beta；其余输入与 feature-test 相同。Feature-test 包不能作为 test 证据，因为 profile 和后端不同。
...
记录 GitHub release tag、npm 精确版本、各平台 checksum 和 Windows smoke。然后让 Rethra Design release branch pin 该 production CLI 精确版本。

### 7.2 Rethra Design prerelease 与 stable
...
- Tempo：按 `rethra_design.run_id` 核对 CLI -> API -> Link trace 和 Workspace 计费归属。
...
- [ ] QA、Vela owner、Rethra Design owner、SRE、Release commander 签字。
...
- Rethra Design `.github/workflows/ci.yml`
- Rethra Design `.github/workflows/release-beta.yml`
- Rethra Design `.github/workflows/cut-release.yml`
- Rethra Design `.github/workflows/release-prerelease.yml`
- Rethra Design `.github/workflows/release-stable.yml`
- Rethra Design `tools/pack/AGENTS.md`
