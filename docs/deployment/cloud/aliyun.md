...
This guide covers self-hosting Rethra Design on Alibaba Cloud for users in mainland China and the broader Asia-Pacific region. It documents the supported deployment paths, image-pull optimisations, and ICP filing considerations for public-facing instances.
...
For users in mainland China, deploying Rethra Design to Alibaba Cloud (阿里云) instead of an overseas provider gives you:
...
This path puts Rethra Design on a single ECS instance using the same Docker Compose stack documented in [`docs/deployment/docker.md`](../docker.md).
...
| Image | Ubuntu 24.04 LTS 64-bit | Rethra Design's Docker image is `linux/amd64` and `linux/arm64` |
...
Rethra Design does not yet ship an official Helm chart. The minimal manifest below is a starting point — production users should harden it (resource limits, PodDisruptionBudget, NetworkPolicy, persistent storage class).
...
>
> Also note that the daemon reads `OD_ALLOWED_ORIGINS` directly. The `RETHRA_DESIGN_ALLOWED_ORIGINS` name documented in `deploy/.env.example` is a Compose-only alias mapped in `deploy/docker-compose.yml`; for the direct-container ACK path you must set `OD_ALLOWED_ORIGINS` instead.

Create the API-token secret first:

```bash
# Generate a random token and store it in the cluster
kubectl create secret generic rethra-design-secrets \
...
metadata:
  name: rethra-design
spec:
  replicas: 1  # Rethra Design uses local SQLite; multi-replica needs shared storage
  selector:
    matchLabels: { app: rethra-design }
  template:
    metadata:
      labels: { app: rethra-design }
    spec:
      containers:
        - name: rethra-design
          image: registry.cn-hangzhou.aliyuncs.com/<your-namespace>/rethra-design:latest
          ports:
            - containerPort: 7456
          env:
            ...
            - name: OD_API_TOKEN
              valueFrom:
                secretKeyRef:
                  name: rethra-design-secrets
                  key: api-token            # required whenever OD_BIND_HOST is non-loopback
            - name: OD_ALLOWED_ORIGINS
              value: "https://design.example.cn"  # set when fronting with an Ingress; daemon reads OD_*, not RETHRA_DESIGN_*
...
spec:
  type: ClusterIP
  selector: { app: rethra-design }
  ports:
    - port: 80
      targetPort: 7456
```

Apply with `kubectl apply -f rethra-design.yaml`. Front the Service with an Ingress (NGINX Ingress Controller is preinstalled on ACK Pro) and an ACM-issued certificate. With `OD_API_TOKEN` set, every `/api/*` request from non-loopback origins must carry an `Authorization: Bearer <token>` header — wire that into your Ingress / proxy auth layer.
...
Resource Orchestration Service (资源编排 ROS) provisions the same resources declaratively. There is no first-party ROS template for Rethra Design yet. Operators with Alibaba Cloud access are welcome to contribute one as a follow-up to this guide; the natural location is `deploy/aliyun/ros/`.
...
Rethra Design's daemon binds to `127.0.0.1:7456` and is never exposed directly. Public access must go through:

1. A reverse proxy that terminates TLS (Nginx, Caddy, Alibaba Cloud SLB/ALB).
2. An ICP-filed domain (for mainland regions).
3. CORS allowlist set via `RETHRA_DESIGN_ALLOWED_ORIGINS` (Compose / `deploy/.env` path) or `OD_ALLOWED_ORIGINS` (direct-container / Kubernetes path).
...
- Rethra Design Docker deployment: [`docs/deployment/docker.md`](../docker.md)
- Rethra Design one-click installer: [`docs/install-guide.md`](../../install-guide.md)
