...
This note describes how an external service can use Rethra Design as a creative
runtime while keeping media provider governance outside Rethra Design.

Rethra Design contributes project context, skills, design systems, previews,
...
Run-scoped `mediaExecution` controls only Rethra Design-owned media generation.
It applies to:

- token-gated `/api/tools/media/generate`
- in-run `od media generate` when `OD_TOOL_TOKEN` is present
- Rethra Design's Codex image generation prompt override
- Rethra Design's media generation prompt contract
...
Rethra Design should not grow a generic provider router, provider account pool,
...
Use HTTP/SSE between the external service and Rethra Design. Avoid shelling out to
...
2. The external service creates or selects an Rethra Design project.
3. The external service starts a run with `mediaExecution` set to the desired
   Rethra Design-owned media policy.
4. The run includes skills and MCP tools that describe the external media
   workflow.
5. Rethra Design handles design/runtime work. External MCP media tools handle
...
For runs where Rethra Design may use only a narrow part of its own media path,
use allowlists:
...
Use Rethra Design project context, design-system guidance, and artifact previews
to plan the campaign. When media bytes are needed, use the configured external
media MCP tools. Do not call Rethra Design-owned media generation unless the run
...
without making Rethra Design own provider auth or budgets.
...
the external service, not to Rethra Design core.

The MCP server should receive its own credentials from the external service or
from its deployment environment. Rethra Design should only see the MCP tool
...
- The external service uploads the fulfilled asset through an Rethra Design
...
Rethra Design project files. Project artifacts should describe creative output,
...
still available for normal Rethra Design media generation outside the in-run tool
...
- `request-only` run mode in Rethra Design core
- Rethra Design media request persistence for external provider work
- a generic HTTP executor provider inside Rethra Design
- provider credentials or account pools in Rethra Design
- Rethra Design-owned provider budgets, retries, or global media limits
