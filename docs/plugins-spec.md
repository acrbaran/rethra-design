# Rethra Design Plugin & Marketplace Spec (v1)

> **In one sentence:** Rethra Design plugins turn portable `SKILL.md` capabilities into marketplace-ready, one-click design workflows while preserving compatibility with existing agent skill catalogs, headless CLI use, and self-hosted deployment.
...
A **Plugin** is the unit of distribution for Rethra Design. Where a [Skill](skills-protocol.md) describes a single capability that an agent can run, a Plugin is the shippable bundle around it: one or more skills, an optional design system reference, optional craft rules, optional Claude-plugin assets, a preview, a use-case query, an asset folder, and a small machine-readable sidecar that powers OD's marketplace surface. A plugin is always anchored to a portable `SKILL.md` so it is publishable to every existing skill catalog without modification.

> **Compatibility promise (extends [`skills-protocol.md`](skills-protocol.md)):** Any plugin folder that ships a `SKILL.md` works as a plain agent skill in Claude Code, Cursor, Codex, Gemini CLI, OpenClaw, Hermes, etc. Adding `rethra-design.json` is purely additive — it unlocks OD's marketplace card, preview, one-click "use" flow, and typed context-chip strip, but it never changes how the underlying skill runs. **One repo, two consumption modes.**
...
1. **Plugin author ships portable capability.** `SKILL.md` remains the executable agent contract; `rethra-design.json` adds OD marketplace metadata, input fields, defaults, previews, and context wiring.
...
| Question | Figma-era plugin assumption | Rethra Design v1 answer |
| --- | --- | --- |
...
**Current implementation clarification:** `discovery -> plan -> generate -> critique` is a reference pipeline shape, not a fixed hard-coded wizard. A plugin snapshot can carry `od.pipeline.stages[].atoms[]`; the daemon resolves that snapshot, injects the active plugin block plus active stage atom blocks into the system prompt, emits stage events, and lets the agent work through the pipeline. When the user has not explicitly selected a plugin, OD still does **not** launch a generic naked agent: the base Rethra Design designer prompt and discovery rules are always present. Product entry points bind sensible defaults on top of that base: Home free-form input routes through the bundled, hidden `od-default` scenario, while typed New Project flows choose the default bundled scenario for the project kind. `od-default` is a router and task-shaper; it should guide the run into the normal design pipeline, not be treated as a standalone "make it beautiful" aesthetic engine.
...
6. [`rethra-design-marketplace.json` schema](#6-rethra-design-marketplacejson--federated-catalog)
...
13. [Public web surface](#13-public-web-surface-rethra-designaimarketplace)
...
19. [Why this is a meaningful step for Rethra Design](#19-why-this-is-a-meaningful-step-for-rethradesign)
...
- Defaults locked from the planning round (overridable in review):
  - **Compatibility = wrap-then-extend.** Existing `SKILL.md` and `.claude-plugin/plugin.json` repos run as-is; `rethra-design.json` is an additive sidecar.
...
2. A vanilla skill or claude-plugin repo becomes an OD plugin by adding an optional `rethra-design.json` sidecar — no rename, no body changes.
3. Three install sources: local folder, GitHub repo (with optional ref/subpath), arbitrary HTTPS archive, plus federated `rethra-design-marketplace.json` indexes.
...
6. The OD core engine, atomic capabilities, and plugin runtime are all reachable from CLI so any code agent can drive Rethra Design headlessly.
...
| `rethra-design.json` only                                            | metadata-only  | no                                        | no                | no                   | no      | no        |
...
The takeaway: **`SKILL.md` is the lowest common denominator**. Every plugin recommended for distribution should ship a `SKILL.md` so it lands cleanly in every major catalog, then add `rethra-design.json` to gain OD's product surface.

A folder that contains only `rethra-design.json` is not a runnable plugin in v1; it is a **metadata-only preset**. OD may read it to show a marketplace card, aggregate remote references, or act as a future install stub, but it cannot trigger an agent run and cannot be listed in cross-agent catalogs. `od plugin doctor` must mark this shape as `metadata-only` and prompt the author to add `SKILL.md` or `.claude-plugin/plugin.json` before publishing it as a runnable plugin.
...
- `rethra-design.json` only ever **points** at SKILL.md / DESIGN.md / craft files; it never duplicates their bodies.
- Existing OD-specific frontmatter on SKILL.md (the `od:` namespace already documented in [`skills-protocol.md`](skills-protocol.md) and used in [`design-templates/blog-post/SKILL.md`](../design-templates/blog-post/SKILL.md)) is honored as a fallback for plugins without `rethra-design.json`. We do not deprecate it; we layer over it.
- A runnable v1 plugin must contain at least one of `SKILL.md` or `.claude-plugin/plugin.json`. `rethra-design.json` does not define agent behavior by itself; it only tells OD how to display, resolve, and apply that behavior.
...
- `specVersion` — the Rethra Design plugin spec version used to interpret the manifest. This is distinct from plugin `version` and is frozen into apply snapshots for replay.
...
If `rethra-design.json` and `SKILL.md` frontmatter both exist, `rethra-design.json` wins, but the loader must preserve adapter warnings. Authors can migrate incrementally: first keep the old skill runnable as-is, then add OD marketplace metadata.

## 6. `rethra-design-marketplace.json` — federated catalog
...
The marketplace top-level `version` is the catalog snapshot version; every `plugins[]` entry also declares the listed plugin version. Installers still verify the target folder's own `rethra-design.json` after fetching, but registry search, audit logs, and marketplace refresh events can now reason about catalog and plugin versions before install.
...
| -------- | ------------------------------------------------ | ------------------ | ---------------------------------------------------------------------- |
| 1        | `<projectCwd>/.rethra-design/plugins/<id>/`        | plugin bundle      | New; explicitly installed into the project and committed with user code |
...
| Frontend SDK compatibility | OD desktop / web with built-in renderer | `@rethra-design/agui-adapter` lets CopilotKit / other AG-UI clients consume an OD run unchanged |
...
- `parsers/manifest.ts` — read `rethra-design.json` → `PluginManifest` (Zod-validated).
...
- `merge.ts` — merge sidecar + adapters with `rethra-design.json` winning; foreign content lands in `compat.*`.
...
- A user with **only Claude Code** (or any code agent) plus `npm i -g @rethra-design/cli` plus a running headless daemon can do the entire user journey: install plugin → create project → run → consume artifacts. No OD desktop required.
...
- CI is a first-class citizen: a GitHub Action can `npm i -g @rethra-design/cli && od daemon start --headless && od plugin install … && od run start --project … --follow`. No display, no electron, no per-step UI scripting.
...
A code agent driving Rethra Design through the CLI typically does:
...
## 13. Public web surface (rethra-design.invalid/marketplace)

The product site already lives at [rethra-design.invalid](https://rethra-design.invalid). The public marketplace ships as a path on that same site — `rethra-design.invalid/marketplace` (canonical) with `rethra-design.invalid/plugins` as an alias — not as a separate domain. It is a static-rendered catalog rendered from the official `rethra-design-marketplace.json` index, with plugin detail pages backed by the same `rethra-design.json` files inside each listed repo. Visually it mirrors what [`skills.sh`](https://skills.sh/) does for skills, but its detail pages render OD-specific previews (the `od.preview.entry` HTML, sample outputs, the use-case query, the chip preview).

The site shares one source of truth with the in-app marketplace:

- Same JSON Schemas (`https://rethra-design.invalid/schemas/plugin.v1.json`, `https://rethra-design.invalid/schemas/marketplace.v1.json`).
- Same federated listing format (`rethra-design-marketplace.json`).
- Same plugin manifests (`rethra-design.json` inside each repo).
...
| Public marketplace (`rethra-design.invalid/marketplace`)      | Anonymous visitors, SEO, share | Deep-link `od://plugins/<id>?apply=1` (auto-installs and applies in the desktop app), plus "Copy install command" |
...
The desktop app registers the `od://` URL scheme; clicking a button on `rethra-design.invalid/marketplace` either launches the desktop or, if it is not installed, falls back to a "How to install Rethra Design" flow.
...
| `rethra-design-marketplace.json`                                                                   | entry referencing `github:owner/repo`                                      | `rethra-design.json` enriches the listing                |

### 14.1 Author tooling

- `od plugin scaffold` — writes a starter folder containing both `SKILL.md` (industry-standard, with `od:` frontmatter for backward compat) and `rethra-design.json` (OD enrichment with `compat.agentSkills` pointing at the SKILL.md).
...
1. **Skill-only consumption (no OD).** A Cursor user runs `npx skills add rethra-design/make-a-deck`. Cursor reads `SKILL.md` and runs the workflow. No OD CLI, no OD daemon. The plugin's marketplace polish (`rethra-design.json`) is ignored — Cursor sees a vanilla skill.
...
1. **Self-hosted enterprise.** A company hosts a private OD instance, registers an internal `rethra-design-marketplace.json` (`od marketplace add https://internal/...`), restricting plugins to internally vetted ones. Their designers and PMs use the desktop client locally; their CI uses `docker exec od od …`.
...
- JSON Schemas at `docs/schemas/rethra-design.plugin.v1.json` and `rethra-design.marketplace.v1.json`.
...
- `pnpm --filter @rethra-design/plugin-runtime test` (parser fixtures: pure SKILL.md, pure claude plugin, metadata-only rethra-design.json, all three combined, SKILL frontmatter mapping).
...
- **§10.3.5 full AG-UI alignment:**
  - New package `@rethra-design/agui-adapter` — bidirectionally maps OD's `PersistedAgentEvent` + `GenUIEvent` onto AG-UI canonical events (`agent.message`, `tool_call`, `state_update`, `ui.surface_requested`, `ui.surface_responded`).
...
- **Per-cloud one-click templates** in a separate `rethra-design/deploy` repo (CloudFormation, Deployment Manager, ARM, Aliyun ROS, Tencent TIC, Huawei RFS) — non-blocking; track separately.
...
OD reads it as a plugin via the existing `od:` frontmatter loader documented in [`skills-protocol.md`](skills-protocol.md). No `rethra-design.json` needed — the plugin lacks marketplace polish but is fully runnable.
...
`SKILL.md` stays portable — Cursor / Codex / OpenClaw read it directly. `rethra-design.json` adds preview, query, chip strip, capabilities. The repo lists cleanly on every catalog in §14 without modification.
...
| Risk                                                        | Mitigation                                                                                          |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Schema drift between OD plugin and the broader skill spec   | `rethra-design.json` is sidecar-only; it never modifies SKILL.md. CI tests run against the public anthropics/skills repo. |
...
| Project-local plugins committed to user repos | Discovery only at `<projectCwd>/.rethra-design/plugins/`; opt-in via `od plugin install --project`. |
...
| Multi-cloud testing matrix is large                         | Phase 5 ships a single canonical compose smoke (one cloud), then adds clouds incrementally; per-cloud one-click templates live in `rethra-design/deploy` and can move at their own cadence (§15.5). |
...
| AG-UI ecosystem may evolve, drifting OD's wire format from canonical AG-UI | OD-native `GenUIEvent` remains the internal source of truth. `@rethra-design/agui-adapter` is an external projection layer, so upstream protocol revs do not couple to the daemon or web renderer release cadence. |
...
- ~~**Timing of AG-UI protocol adoption**~~ — **resolved.** `@rethra-design/agui-adapter` and `GET /api/runs/:runId/agui` have shipped as optional interoperability. OD-native GenUI remains the internal renderer and CopilotKit is not a required product dependency.

## 19. Why this is a meaningful step for Rethra Design

- **Inherited supply.** Every public agent skill on `anthropics/skills`, `awesome-agent-skills`, `clawhub`, and `skills.sh` is one optional `rethra-design.json` away from being an OD plugin — and reciprocally, every OD plugin is publishable to all four catalogs without modification.
...
- `plugins/_official/scenarios/od-figma-migration/rethra-design.json` supplies the bundled reference pipeline.
...
- `plugins/_official/scenarios/od-code-migration/rethra-design.json` supplies the bundled patch/edit ↔ build/test devloop and diff-review handoff.
...
- `@rethra-design/agui-adapter` and `/api/runs/:runId/agui` provide external AG-UI event projection without changing OD's internal GenUI renderer.
...
- An atom plugin is a folder with `rethra-design.json` (`od.kind: 'atom'`) plus `SKILL.md`.
...
- `plugins/_official/scenarios/od-figma-migration/rethra-design.json`
- `plugins/_official/scenarios/od-code-migration/rethra-design.json`
