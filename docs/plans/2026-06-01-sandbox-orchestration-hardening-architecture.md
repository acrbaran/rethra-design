...
keep Rethra Design safe to embed under an external orchestrator without adding
consumer-specific infrastructure to this repository.

The plan is intentionally upstream-neutral. Rethra Design should refer to this
...
runtime contract while adding normal Rethra Design features. The durable defense is
...
1. An external orchestrator launches Rethra Design with `OD_SANDBOX_MODE=1` and
...
3. Rethra Design contributes design context, skills, previews, artifacts, CLI/UI
...
Without `OD_SANDBOX_MODE=1`, Rethra Design is the normal local product runtime, not
...
- process matching must not classify a foreign process tree as an Rethra Design
...
- Keep the allowlist narrow so normal references to Rethra Design, GitHub
...
- Do not add a generic provider router, provider account pool, global media
  budget system, or external executor API to Rethra Design.
- Do not add consumer-specific conformance tests to Rethra Design CI.
