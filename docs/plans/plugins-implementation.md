# Rethra Design Plugin & Marketplace — Implementation Plan (living)
...
packages/plugin-runtime/             ← pure TS; reusable in web / daemon / CI
  ...
  ├── merge.ts                       ← sidecar + adapter merge; rethra-design.json wins
...
| `plugins/_official/atoms/<atom>/{SKILL.md,rethra-design.json}` | shipped | Phase 4 / 6 / 7 / 8 — 13 first-party atom plugins (4 implemented + 9 reserved fragments) |
| `plugins/_official/scenarios/<id>/{SKILL.md,rethra-design.json}` | shipped | Phase 4 (§23.3.3) — bundled scenario/router/export plugins, including the four taskKind defaults plus `od-default` Home free-form routing |
...
| `tools/pack/helm/rethra-design/templates/**` | shipped | Phase 5 — Deployment / Service / Secret / ConfigMap / PVCs / Ingress / NOTES |
| `tools/pack/helm/rethra-design/values-{aws,gcp,azure,aliyun,tencent,huawei,self}.yaml` | shipped | Phase 5 — per-cloud overrides (volume + ingress diffs) |
...
| `GET /api/runs/:runId/agui` | shipped | Phase 4 — pipes events through `@rethra-design/agui-adapter` |
...
- [x] `docs/schemas/rethra-design.plugin.v1.json` — JSON Schema v1.
- [x] `docs/schemas/rethra-design.marketplace.v1.json` — JSON Schema v1.
...
- [x] `pnpm --filter @rethra-design/plugin-runtime test`
- [x] `pnpm guard && pnpm typecheck`
- [x] CI digest stability: re-running `digest()` on the fixtures matches the pinned hex.

Exit criterion

- Importing `import type { ApplyResult, AppliedPluginSnapshot } from '@rethra-design/contracts'` works from daemon and web. ✓ verified.
...
- [x] `pnpm --filter @rethra-design/plugin-runtime test` covers: digest stability, `parseManifest` + `parseMarketplace`, SKILL frontmatter adapter, sidecar+adapter merge precedence, `validateSafe` cross-field rules.
...
- [x] `@rethra-design/agui-adapter` package; `GET /api/runs/:runId/agui` SSE endpoint emits AG-UI canonical events.
...
- [x] `pnpm --filter @rethra-design/contracts test` clean.
- [x] `pnpm --filter @rethra-design/plugin-runtime test` clean.
- [x] `pnpm --filter @rethra-design/daemon test` — all 56 `plugins-*.test.ts` (391 tests) green. Three unrelated pre-existing failures remain (`finalize-design.test.ts` resolveCurrentArtifact path normalization, `chat-route.test.ts` stalled-json-stream timeout, `connection-test.test.ts` hard-cancel timeout). They were inherited from PR #832 and the chat/connection timeout test refactors and do not block the plugin loop; tracked separately.
- [x] `pnpm --filter @rethra-design/web test` clean.
