...
This PR wires Rethra Design's beta mac arm64 packaging path to Vela's npm-owned CLI distribution contract.
...
Rethra Design depends only on the Vela meta package:

```json
"@powerformer/vela-cli": "0.0.1-test"
```

Rethra Design does not depend directly on platform packages such as `@powerformer/vela-cli-darwin-arm64`. Vela owns that platform matrix through optional dependencies and its resolver API.

The Vela CLI binary resolution order is:

1. `RETHRA_DESIGN_VELA_CLI_BIN`
...
1. Vela publishes `@powerformer/vela-cli` to npm before Rethra Design packaging starts.
2. Rethra Design pins that package in `tools/pack/package.json` and `pnpm-lock.yaml`.
...
4. `tools-pack` enters the `resource-tree` phase and resolves/copies the Vela binary into the Rethra Design resource tree.
...
Rethra Design should install only `@powerformer/vela-cli`. The meta package pulls the macOS arm64 binary package as an optional dependency on supported machines.
...
```text
.tmp/release-beta-dry-run/out/mac/namespaces/release-beta/resources/rethra-design/bin/vela
```
...
- Rethra Design depends only on `@powerformer/vela-cli`.
- `RETHRA_DESIGN_VELA_CLI_BIN` remains highest priority.
