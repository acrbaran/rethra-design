...
Maintainer de `acrbaran/rethra-design`. A composição individual do Core Team é
...
| **Core Team** | Time interno do Rethra Design. Possui acesso total de escrita ao repositório e é a autoridade final em decisões de governança. Composição mantida internamente. |
...
- **≥ 20 merged PRs** em `acrbaran/rethra-design`.
...
Enquanto `acrbaran/rethra-design` tiver menos de seis meses desde o commit inicial,
