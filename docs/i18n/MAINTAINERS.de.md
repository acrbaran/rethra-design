...
Dieses Dokument legt die Regeln dafür fest, wie man Maintainer von `acrbaran/rethra-design` wird, diese Rolle ausübt und sich aus ihr zurückzieht. Die individuelle Zusammensetzung des Core Teams wird intern verwaltet und ist hier nicht aufgeführt — was öffentlich zählt, sind die Regeln, an die sich alle halten.
...
| **Core Team** | Das interne Team von Rethra Design. Verfügt über vollen Schreibzugriff auf das Repository und ist die letzte Instanz bei Governance-Entscheidungen. Die Zusammensetzung wird intern verwaltet. |
...
- **≥ 20 merged PRs** zu `acrbaran/rethra-design`.
...
Solange `acrbaran/rethra-design` jünger als sechs Monate seit dem initialen Commit ist, kann das Veto zur **projektübergreifenden Aktivität** (#6) durch Konsens des Core Teams ausgesetzt werden, wenn:
