...
本文档定义了 `acrbaran/rethra-design` 项目中**成为、担任、退出 Maintainer** 的规则。Core Team 个人名册由内部维护，本文档不公开列出——对外公开的是大家共同遵守的规则。
...
| **Core Team**（核心团队） | Rethra Design 内部团队。拥有完整仓库写权限，是治理决策的最终权威。名册由内部维护。 |
...
- **≥ 20 个 merged PR** 到 `acrbaran/rethra-design`
...
`acrbaran/rethra-design` 自首个 commit 起 6 个月内，**跨项目活跃**一票否决条款（#6）可由 Core Team 共识豁免，前提是：
