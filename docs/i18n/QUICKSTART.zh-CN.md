...
- **可选的本地 agent CLI：** Rethra Design 通过 registry 支持 Claude Code、Codex、Devin for Terminal、OpenCode、Cursor Agent、Qwen、Qoder CLI、GitHub Copilot CLI 等本地 runtime；当前清单以 [`apps/daemon/src/runtimes/registry.ts`](../../apps/daemon/src/runtimes/registry.ts) 为准。即使未安装任何本地 runtime，也可使用在 Settings 中配置的 BYOK runtime。
...
pnpm --filter @rethra-design/daemon build  # 构建 apps/daemon/dist/cli.js，供 `od` 使用
pnpm --filter @rethra-design/web build     # 在需要时构建 web package
...
在一个完全容器化的环境中运行 Rethra Design，无需安装 Node.js 或 pnpm。
...
随后，在 Rethra Design 应用中**重新打开**该 project，不要复用之前 terminal 中的 agent 会话。由 daemon 启动的 agent 应当能够看到类似如下的值：
...
│   ├── sidecar-proto/         # Rethra Design sidecar 协议契约
...
- **媒体生成报错 `OD_BIN` 缺失、或 daemon URL 为 `:0`** —— 运行上述媒体 dispatcher 排查步骤。请勿复用已有的 CLI 会话；从 Rethra Design 应用中重新打开 project，daemon 才会注入新的 `OD_*` 变量。
- **Codex 加载的插件上下文过多** —— 使用 `OD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev` 启动 Rethra Design，daemon 启动 Codex 时会传入 `--disable plugins`。
