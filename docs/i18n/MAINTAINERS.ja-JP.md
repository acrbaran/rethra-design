...
このドキュメントは、`acrbaran/rethra-design` の Maintainer になるため、その役割を務めるため、そして退任するためのルールを定めるものです。Core Team の個別のメンバー一覧は内部で管理されており、ここには列挙していません。公に重要なのは、全員が従うルールそのものです。
...
- `acrbaran/rethra-design` への **20 件以上の merged PR**。
...
`acrbaran/rethra-design` が初回コミットから 6 ヶ月未満である間は、以下の場合に **クロスプロジェクト活動** の veto（#6）を Core Team の合意により免除できます：
