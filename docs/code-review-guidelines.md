...
Run this test **before** reviewing implementation details. A PR passes only when the changed behavior is visible in, required by, or directly validates an Rethra Design–owned surface.

A PR passes the relevance test when **all** of the following hold:

- The PR description identifies the Rethra Design feature, command, protocol, package, resource format, or runtime path being changed.
- Tests exercise existing Rethra Design flows through their public seams: artifact generation/rendering via documented daemon APIs, daemon HTTP/SSE endpoints, web UI on shipped routes, desktop/packaged launch behavior, sidecar status/IPC, `tools-dev`/`tools-pack` lifecycle commands, functional-skill/design-template/design-system/plugin/craft resource loading, or documented cross-boundary smoke behavior. Rendering domain content through a generic primitive does **not** count.
...
- New sample content is a maintained fixture for an Rethra Design capability, minimal enough to support the test, and stored under the [first-party fixture rule](#first-party-fixture-rule).
...
Domain-specific UI or content (a customer vertical, marketing experiment, unrelated rendering demo, arbitrary product page) is out of scope unless it is explicitly a first-party Rethra Design fixture and the PR explains which Rethra Design capability it validates.
...
A generally reasonable direction (e.g. "add Arabic/RTL coverage") is **not** sufficient justification to keep an unsalvageable PR open. Close with a comment that names which condition was met and asks for a fresh PR that uses the actual Rethra Design app and harness. If the contributor disputes the close, escalate to another maintainer rather than reopening unilaterally.
...
- Rethra Design app/mode/source/stamp details hard-coded inside generic `packages/sidecar` or `packages/platform` code.
...
- Adds tests, fixtures, scripts, or UI for a separate product/domain that does not exercise an existing Rethra Design feature.
...
- Instructions are general-purpose enough for Rethra Design workflows, not a single unrelated external service or business process.
...
- It adds broad external integrations or credentials not necessary for a design-focused Rethra Design workflow.
...
- Each portable plugin has a `SKILL.md`. Rethra Design display, input, preview, pipeline, capability, and source metadata lives in `rethra-design.json`, not in the portable skill body.
- `rethra-design.json` conforms to `docs/schemas/rethra-design.plugin.v1.json` and references only supported atoms, design systems, craft docs, assets, scripts, MCP servers, or connectors.
...
- Rethra Design-only marketplace/runtime metadata is embedded in `SKILL.md` instead of `rethra-design.json`.
...
- An enriched or bundled example lacks its skill, `rethra-design.json`, referenced assets, or required bilingual documentation update.
...
- **Domain-specific app smuggled in as tests.** A PR adding tests for a "Quranic Arabic Learning App" with selectors like `#dashboard`, `#exercise`, `.vocabulary-item`, `.arabic-word` when those surfaces are not part of Rethra Design — violates the "tests target real routes/DOM/APIs" rule.
- **Standalone-app E2E URLs.** Playwright tests hard-coding `http://localhost:17573/index.html` instead of using the configured Rethra Design app flow — violates the "use the existing harness, base URL, and lifecycle conventions" rule.
...
- **Ad hoc launcher script.** A new root script that hard-codes `~/projects/rethra-design`, assumes a user-specific tool such as `fnm`, or bypasses `pnpm tools-dev` — violates the "scripts use the documented control plane" rule and recreates a [Forbidden surface](#2-forbidden-surfaces).

A reasonable underlying intent (e.g. "we need RTL or Arabic coverage") does not justify keeping such a PR open. Ask for a fresh PR that exercises the actual Rethra Design app through the existing harness.
