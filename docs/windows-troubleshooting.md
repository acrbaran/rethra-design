...
Rethra Design runs on Windows natively, but the path is less travelled than macOS, Linux, or WSL2. This guide covers the most common errors you will hit on a fresh Windows machine and the exact fix for each.
...
When you run the downloaded installer (for example `rethra-design-0.11.0-win-x64-setup.exe`), a blue Windows Defender SmartScreen dialog appears:

```text
Windows protected your PC
Microsoft Defender SmartScreen prevented an unrecognized app from starting.
App:       rethra-design-x.y.z-win-x64-setup.exe
...
This is expected and does not mean the app is unsafe or broken. SmartScreen warns about any installer that is not signed with a code-signing certificate it already recognizes. Rethra Design ships unsigned Windows builds today, so the installer reports `Publisher: Unknown publisher` and SmartScreen flags it until a given signed binary builds up download reputation. The warning is about verifying who published the file, not about detecting a threat.
...
```powershell
Get-FileHash .\rethra-design-x.y.z-win-x64-setup.exe -Algorithm SHA256
```
...
```text
Rethra Design dev server ready
...
OpenCode is one of the local agent CLIs Rethra Design can drive. If you want to use it:
...
If Rethra Design still shows OpenCode as *not installed* in **Settings → Execution mode**, click **Rescan** after confirming the `opencode.cmd` directory is on your user `PATH`.
