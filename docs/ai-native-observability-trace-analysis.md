...
This document records how to read the new Rethra Design Langfuse trace shape for
...
- PR: https://github.com/acrbaran/rethra-design/pull/3714
- Issue: https://github.com/acrbaran/rethra-design/issues/3733
...
content should remain in Rethra Design object storage and be referenced by
...
Raw sensitive content belongs in Rethra Design object storage with access control,
...
Issue #3733 targets the first durable bridge between Rethra Design-owned object
...
Rethra Design-owned project storage today and to a ProjectStorage or
...
| Storage reference | Emits stable `od://objects/...` references. | Treats `storage_ref` as an Rethra Design registry reference, future-compatible with ProjectStorage/S3 backends. | No conflict; current refs should remain non-dereferenceable from Langfuse. |
...
1. Make the manifest builder resolve through a concrete Rethra Design object
...
4. Preserve `storage_ref` as a stable, non-secret reference and require any
   future download or preview access to go through Rethra Design-controlled
   authorization.
...
- `pnpm install`
- `pnpm --filter @rethra-design/daemon exec vitest run -c vitest.config.ts tests/langfuse-trace.test.ts tests/langfuse-bridge.test.ts tests/claude-stream-thinking.test.ts`
...
1. Store complete raw/redacted prompt payloads in Rethra Design object storage and
