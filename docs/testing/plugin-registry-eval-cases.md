...
`rethra-design-marketplace.json`，`Available` 是供应池，`Installed` 才是 agent
...
| ID | 场景 | 核心断言 | 覆盖文件 |
| --- | --- | --- | --- |
| REG-001 | Sources 添加的是 raw `rethra-design-marketplace.json`，不是 GitHub tree 页面 | GitHub tree HTML 会被 marketplace parser 拒绝并返回 422 | `apps/daemon/tests/plugins-marketplaces.test.ts` |
| REG-002 | 默认 official registry seed 是真实 catalog，不是空数组 | `plugins/registry/official/rethra-design-marketplace.json` 包含 bundled official entries，trust 为 `official`，且 `rethra-design/build-test` 可 resolve | `apps/daemon/tests/plugins-marketplaces.test.ts` |
| REG-003 | 默认 community registry seed 可被 daemon 当作 restricted source 加载 | `plugins/registry/community/rethra-design-marketplace.json` 可 seed，`community/registry-starter` 可 resolve，trust 为 `restricted` | `apps/daemon/tests/plugins-marketplaces.test.ts` |
...
| REG-C04 | `rethra-design-marketplace.json` 生成器 | 输入多个 `plugins/community/**/rethra-design.json`，输出排序稳定、schema 通过、source/digest 完整 |
...
| ID | 场景 | 原因 |
| --- | --- | --- |
| REG-M01 | rethra-design.invalid marketplace 页面视觉、SEO、插件详情叙事 | 强依赖品牌表达和真实内容质量，适合人工验收 |
