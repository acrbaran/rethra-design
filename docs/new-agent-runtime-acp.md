...
This note documents the preferred integration shape for a new Rethra Design agent runtime.

## Recommendation

New agent runtimes should expose an **ACP over stdio** CLI mode.

In practice, Rethra Design expects to spawn a local executable and speak JSON-RPC over the child process streams:
...
That wrapper keeps Rethra Design on the standard ACP subprocess transport and avoids requiring a daemon-side network transport adapter.
...
ACP's remote HTTP/WebSocket transport is still described as a draft/proposal rather than the established compatibility path. Rethra Design's implemented ACP adapters therefore use stdio subprocesses today.

## Messages Rethra Design sends

For `streamFormat: 'acp-json-rpc'`, Rethra Design currently drives a session with these JSON-RPC methods:

1. `initialize`
   - Sent first.
   - Includes Rethra Design client metadata and `clientCapabilities`.
2. `session/new` or `session/load`
   - Creates a working session, or resumes the durable upstream session from a
     prior turn when Rethra Design has a saved session handle.
   - Includes the project working directory.
   - `session/new` may include MCP server descriptors when the runtime is
     allowed to use Rethra Design-provided tools.
3. `session/set_config_option` or `session/set_model` *(optional)*
   - Sent when the user selected a non-default model.
   - Rethra Design prefers `session/set_config_option` when `session/new` reports a model config option; otherwise it falls back to `session/set_model`.
4. `session/prompt`
...
## Messages Rethra Design expects from the agent
...
2. Response to `session/new` or `session/load`.
   ...
   - A new session may also return the durable upstream session handle that
     Rethra Design records for the next turn.
...
4. Optional `session/request_permission` requests.
   - Rethra Design auto-selects an approve/allow-style option when available.
...
5. Response to `session/prompt`.
   ...
   - This response tells Rethra Design the turn is finished.
...
- After `session/prompt` completes, either exit cleanly when stdin closes or tolerate Rethra Design sending `SIGTERM` after a short grace period.
- Implement `session/cancel` if possible. Rethra Design falls back to process termination when the transport is no longer usable.
- Avoid interactive terminal prompts. If permission is required, use ACP permission requests instead.

## Rethra Design adapter shape

An ACP runtime definition in Rethra Design is intentionally small:
