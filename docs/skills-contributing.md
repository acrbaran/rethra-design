...
```bash
# 1. Fork & clone
git clone git@github.com:<your-username>/rethra-design.git
cd rethra-design
...
**Third option: ship an external plugin bundle.** If your workflow is genuinely a recipe (not a daemon feature) but is too vendor-specific or audience-narrow to land in-tree, publish a portable `SKILL.md` bundle with an `rethra-design.json` manifest and distribute it through the plugin workflow. The current `od skills` CLI is read-only; installation belongs to `od plugin`, not the retired `od skill add` shape. This is the right path for payment-provider workflows, regional marketplace integrations, in-house design systems, and similar — not a rejection, just a different distribution channel.

If you're not sure your idea fits, **open a discussion first** ([github.com/acrbaran/rethra-design/discussions](https://github.com/acrbaran/rethra-design/discussions)) — we'd rather spend 5 minutes redirecting than have you build the wrong thing for a week.
...
- [ ] **Run `pnpm --filter @rethra-design/web test` and `pnpm --filter @rethra-design/e2e test tests/localized-content.test.ts`** locally before pushing. These suites catch undisplayable discovered resources and verify localized fallback behavior.
...
- [ ] Ran `pnpm --filter @rethra-design/web test` and `pnpm --filter @rethra-design/e2e test tests/localized-content.test.ts`; localized-content coverage is green
