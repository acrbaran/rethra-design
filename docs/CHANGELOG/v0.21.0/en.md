---
title: Rethra Design 0.21.0 — Reliable, Start to Finish
description: From Agent connections to live HTML previews and app relaunches, Rethra Design 0.21.0 makes the entire creation experience more reliable—and easier to recover when something goes wrong.
---

### 🌟 Codename: *Reliable, Start to Finish*

🛡️ **42 PRs · 13 contributors · 4 days** — **From Agent connections to live
HTML previews and app relaunches, Rethra Design 0.21.0 makes the entire creation
...
- 🧰 **A working agent no longer disappears behind a broken shim.** Rethra Design
...
- 🌙 **Kimi works again on 0.37 and newer.** Those releases stopped accepting the
  stdio MCP transport that Rethra Design attached to every session, turning nearly
  every run into an opaque handshake error. Rethra Design now sends only the
...
- 🚪 **Relaunching the app recovers from a stale local engine.** A web sidecar
  ...
  quit at startup. Rethra Design now reclaims that stale owner safely, and its
