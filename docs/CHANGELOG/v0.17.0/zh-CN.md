...
description: 在 Codex 中直接调用 Rethra Design 设计工作区，把想法变成可继续编辑的真实成果，并在 Studio 中完成精修。
---

### 🌟 Codename: *Rethra Design Plugin for Codex*

🚀 **62 个 PR · 23 位贡献者 · 11 天** — **Codex 现在有了自己的设计工作区。** 在 Codex 对话中调用 `@rethra-design`，把想法与 brief 变成可继续编辑的真实设计成果；之后仍可在 Rethra Design Studio 中持续创作和精修，不必中断 Codex 工作流。🚀

## 🔥 亮点

- 🚀 **Codex 中的 Rethra Design——从 `@rethra-design` 到真实成果。** Codex Desktop 和 CLI 现在可以把 Rethra Design 当作一套完整的创作引擎：确认视觉 brief，选择 Rethra Design Cloud 或受支持的本地执行方式，并获得真实的 Preview 或 Studio 结果。需要时，已签名的 Rethra Design runtime 会在后台启动，无需一直开着第二个应用，也无需手工拼接整套工具。如果外部宿主中的 Studio 无法正常加载，Codex 仍会立即交付稳定预览。 (#6055, #6273, #6362)
...
- ⚡ **需求已经说清楚，就直接开始。** Rethra Design 不再把每个首轮请求都变成一次访谈。信息足够时，Agent 会立即动手；只有缺失信息确实会改变结果时，才在真正需要的时刻提出一个有针对性的问题。 (#6223)
...
- 🎞️ **Codex Slides 加入 Rethra Design 产品家族。** 从 prompt 到完整演示文稿，新的 Codex Slides 产品体验展示了场景与风格选择、提纲塑形、编辑和导出的完整流程。 (#6050) 感谢 @joeylee12629-star。
...
- **Raven 加入一键 MCP 配置。** Rethra Design 可以直接生成正确的 Raven 配置，无需把其他客户端格式手工翻译一遍。 (#5969) 感谢 @roian6。

- **Windows 上更容易找到 Grok Build。** Rethra Design 现在会自动发现官方用户目录中的安装。 (#5843) 感谢 @thebtf。
...
- **BYOK 保持本地，也保持可用。** 服务商配置继续保存在当前设备的 Rethra Design 浏览器存储中，只会传给当前正在运行的本地任务。启动时不再尝试把凭据迁移到操作系统 profile；Rethra Design Cloud 和 Local Codex 的行为不变。 (#6356)
...
- **AMR 登录可以从较晚发生的失败中恢复。** 如果直接认证在激活前退出，Rethra Design 会进行一次有边界的 fallback，而不是让用户一直停留在加载状态。缓慢、已完成或已取消的登录不会启动重复认证。 (#5986) 感谢 @Siri-Ray。

- **Web 界面意外退出后，打包应用可以自行恢复。** Rethra Design 会在有限次数内重启失败的 sidecar，并重新连接 `od://` 请求，无需完整退出并重启应用。 (#6364) 感谢 @lefarcen。
