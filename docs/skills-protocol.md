...
Rethra Design does not distribute an active bundle by symlinking it into every
...
- **Output:** defined by the skill. A portable workflow may emit only
  `DESIGN.md`; current Rethra Design import/create flows build a package with
...
license preserved and Rethra Design metadata applied:
...
3. Rethra Design stores the selected template id as the project's primary
