...
plan-selection modal was replaced by the shared Rethra Design `/pricing/` page.
...
The public Pricing page's direct Rethra Design PostHog capture is a different
...
pipeline rather than Rethra Design PostHog.

## Scope

- Two repositories and two PRs: Vela provides the authenticated ingestion
  boundary; Rethra Design emits from the migrated Pricing interactions.
...
- Anonymous or direct public Pricing visitors continue to use the existing
  Rethra Design Pricing analytics only.
...
- Do not change Pricing rendering, plan selection, checkout URLs, payment
  behavior, entitlement behavior, or existing Rethra Design analytics.
...
2. validates the request origin against the existing Rethra Design hosted-origin
...
### Rethra Design: Pricing compatibility emitter
...
allowlisted Rethra Design/Vela origin and an exact `/wallet`, `/dashboard`,
...
- `entryPoint=rethra_design_entry`
...
- validated optional Rethra Design campaign attribution already allowed by the
  Vela registry
...
- Enterprise form submit intent: `team_lead_submit` before client validation or
  ...
  `lead_submit_failed` events continue independently in Rethra Design PostHog.
...
Deploy the Vela endpoint first. The Rethra Design emitter may then be deployed
without a compatibility window or feature migration. Until Vela is available,
Rethra Design continues its existing local Pricing analytics and checkout behavior;
