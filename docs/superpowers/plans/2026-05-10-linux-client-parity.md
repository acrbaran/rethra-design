...
Issue: https://github.com/acrbaran/rethra-design/issues/709
...
- Linux release artifact bundle contains the expected AppImage, metadata, and
  uploaded `rethra-design-release-linux-e2e-report`.
...
```bash
corepack pnpm guard
corepack pnpm --filter @rethra-design/tools-pack test -- linux.test.ts
corepack pnpm --filter @rethra-design/tools-pack typecheck
corepack pnpm --filter @rethra-design/e2e test -- tests/linux-helpers.test.ts tests/packaged-smoke-workflow.test.ts
corepack pnpm --filter @rethra-design/e2e typecheck
git diff --check
```
