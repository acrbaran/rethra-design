...
A small community plugin for exercising the Rethra Design import UI. It is meant
to be boring in exactly the useful way: the folder has the portable `SKILL.md`
floor, the enriched `rethra-design.json` sidecar, and a minimal Claude-compatible
plugin manifest.
...
- From GitHub: use `github:acrbaran/rethra-design@main/plugins/community/import-smoke-test` after the entry is available on `main`.
- Marketplace name: use `community/import-smoke-test` after the community marketplace has been registered or refreshed.

## Expected Result

Rethra Design should install the plugin as a user/community plugin, preserve the
