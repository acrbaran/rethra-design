...
Rethra Design plugins are intentionally shaped so one folder can travel across multiple agent ecosystems. The safest publishing model is:

1. Keep the source of truth in a public GitHub repository or in an Rethra Design PR.
2. Keep `SKILL.md` portable and registry-friendly.
3. Add `rethra-design.json` as the Rethra Design sidecar.
...
2. Push a public GitHub repository or open an Rethra Design PR.
3. Add README install instructions for Rethra Design and generic Agent Skills clients.
...
| --- | --- | --- | --- |
| Rethra Design | OD marketplace, composer chips, pipelines, GenUI, artifact provenance | `SKILL.md` + `rethra-design.json` | Open a PR to Rethra Design or publish a marketplace index entry pointing to the plugin repo. |
...
For Rethra Design plugin authors:

- Ensure the repo or subpath contains a valid `SKILL.md`.
- Keep `rethra-design.json` additive; generic skill clients should be able to ignore it.
...
- Use a GitHub topic and README keywords that match the plugin's lane, such as `rethra-design-plugin`, `agent-skill`, `prototype`, `deck`, `hyperframes`, or `design-system`.
...
Use this path for normal Rethra Design plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and Rethra Design PR.
- Keep changelog text honest and versioned.
- Keep `rethra-design.json` `specVersion` fixed to the spec kit version and bump plugin `version` for every publishable behavior change.
