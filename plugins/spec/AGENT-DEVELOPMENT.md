...
Give this file to a coding agent when you want it to create or improve an Rethra Design plugin.

## Mission

Create a portable Rethra Design plugin that can:

1. Run as a normal Agent Skill through `SKILL.md`.
2. Install into Rethra Design through `rethra-design.json`.
3. Be validated locally.
4. Be published as an independent open source repo or submitted as a PR to Rethra Design.
...
- `plugins/spec/PUBLISHING-REGISTRIES.md` when the user asks to publish outside Rethra Design
...
3. Keep the `SKILL.md` portable. It may mention Rethra Design behavior, but the core workflow must still make sense in any Agent Skills compatible agent.
4. Put OD-specific display, `specVersion`, plugin `version`, inputs, preview, pipeline, atoms, connectors, and capabilities in `rethra-design.json`.
...
- `rethra-design.json` validates against the v1 shape and carries explicit `specVersion` plus plugin `version`.
