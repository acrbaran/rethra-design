...
这是可移植 Rethra Design 插件的精简契约。完整产品规范仍以 `docs/plugins-spec.zh-CN.md` 为准；本文档面向贡献者和外部编码 agent。
...
## 2. 增强版 Rethra Design 插件

当插件需要出现在 Rethra Design marketplace 卡片或 starter 中时，添加 `rethra-design.json`：
...
`rethra-design.json` 指向 skill，并声明产品表面：
...
- `name` 是稳定插件 id。
- `specVersion` 是此 manifest 遵循的 Rethra Design 插件规范版本。除非 schema 升级，否则使用当前规范包的值（`1.0.0`）。
...
1. 校验 JSON 语法。
2. 确认 `rethra-design.json` 包含 `specVersion`，并在行为变化时 bump 插件 `version`。
3. 运行 `pnpm guard`。
4. 运行 `pnpm --filter @rethra-design/plugin-runtime typecheck`。
5. 如果可用，运行 `od plugin validate ./path/to/plugin`。
6. 视觉类插件包含一张截图、渲染 preview 或示例输出。
7. 在 PR body 里说明 trust 和 capabilities。

外部 registry 分发策略见 [`PUBLISHING-REGISTRIES.zh-CN.md`](PUBLISHING-REGISTRIES.zh-CN.md)。简言之：把 GitHub 或 Rethra Design PR 作为 source of truth，让文件夹能作为通用 `SKILL.md` skill 安装；本地验证通过后，再发布或登记到 skills.sh、ClawHub 或其他 registry。
