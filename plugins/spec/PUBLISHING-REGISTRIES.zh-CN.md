...
Rethra Design 插件刻意设计成一个文件夹可以跨多个 agent 生态流转。最稳妥的发布模型是：

1. 把公开 GitHub 仓库或 Rethra Design PR 作为 source of truth。
2. 保持 `SKILL.md` 可移植、适合 registry 读取。
3. 添加 `rethra-design.json` 作为 Rethra Design sidecar。
...
1. 本地验证插件文件夹。
2. 推送公开 GitHub 仓库，或向 Rethra Design 打开 PR。
3. 在 README 中添加 Rethra Design 和通用 Agent Skills 客户端的安装说明。
...
| 目标 | 适合场景 | 源形态 | 发布策略 |
| --- | --- | --- | --- |
| Rethra Design | OD marketplace、composer chips、pipelines、GenUI、artifact provenance | `SKILL.md` + `rethra-design.json` | 向 Rethra Design 提 PR，或发布指向插件仓库的 marketplace index entry。 |
...
对 Rethra Design 插件作者：

- 确保 repo 或 subpath 包含合法 `SKILL.md`。
- 保持 `rethra-design.json` 为纯增量；通用 skill 客户端应能忽略它。
- 在 README 中放一个短安装块：
...
- 使用与插件主类匹配的 GitHub topic 和 README 关键词，例如 `rethra-design-plugin`、`agent-skill`、`prototype`、`deck`、`hyperframes`、`design-system`。
...
普通 Rethra Design 插件应优先走这个路径，因为它们以 `SKILL.md` 为核心。
...
- 链回 canonical GitHub repo 和 Rethra Design PR。
- changelog 诚实且版本化。
- `rethra-design.json` 的 `specVersion` 保持为规范包版本；每次可发布的行为变化都 bump 插件 `version`。
