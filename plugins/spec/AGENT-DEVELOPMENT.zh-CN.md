...
当你希望一个编码 agent 创建或改进 Rethra Design 插件时，可以把这个文件直接交给它。

## 任务目标

创建一个可移植的 Rethra Design 插件，使它可以：

1. 通过 `SKILL.md` 作为普通 Agent Skill 运行。
2. 通过 `rethra-design.json` 安装到 Rethra Design。
3. 在本地完成校验。
4. 发布为独立开源仓库，或作为 PR 提交给 Rethra Design。
...
- 当用户要求发布到 Rethra Design 以外的 registry 时，阅读 `plugins/spec/PUBLISHING-REGISTRIES.zh-CN.md`
...
3. 保持 `SKILL.md` 可移植。它可以提到 Rethra Design 行为，但核心 workflow 必须在任何 Agent Skills 兼容 agent 中都能理解。
4. 把 OD 专属 display、`specVersion`、插件 `version`、inputs、preview、pipeline、atoms、connectors 和 capabilities 放进 `rethra-design.json`。
...
- `rethra-design.json` 符合 v1 形态，并显式携带 `specVersion` 与插件 `version`。
