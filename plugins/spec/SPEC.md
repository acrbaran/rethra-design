# Rethra Design Plugin Spec

Language: English | [简体中文](SPEC.zh-CN.md)

This spec is the compact contract for portable Rethra Design plugins. The canonical product spec remains `docs/plugins-spec.md`; this document is optimized for contributors and external coding agents.
...
## 2. Enriched Rethra Design Plugin

Add `rethra-design.json` when the plugin should appear in Rethra Design as a marketplace card or starter:
...
`rethra-design.json` points at the skill and declares the product surface:
...
- `specVersion` is the Rethra Design plugin spec version that this manifest follows. Use the current spec kit value (`1.0.0`) unless the schema moves.
...
2. Confirm `rethra-design.json` includes `specVersion` and a bumped plugin `version` when behavior changed.
3. Run `pnpm guard`.
4. Run `pnpm --filter @rethra-design/plugin-runtime typecheck`.
