...
When this skill runs inside Rethra Design, the daemon invokes its pinned HyperFrames runtime, which inherits the auto-enable. You do **not** need to add browser flags or pass extra CLI args from the agent.
...
HyperFrames enables the Chrome flag automatically during rendering. Inside Rethra Design, dispatch through the daemon rather than running the Chrome-bound CLI from the agent shell:
...
For Docker renders, the flag is also enabled automatically inside the container. Rethra Design's daemon-managed render inherits the same default — you don't need to thread anything through.
