...
description: |
  ...
  reflective flowing surface. Rethra Design serves this in powered-preview mode
...
Rethra Design detects `getContext('webgl2')` and renders this file in **powered preview** — a cross-origin-isolated iframe with `allow-same-origin`. The full GPU pipeline is available; you do not need to work around the opaque sandbox.
