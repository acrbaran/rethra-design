...
Rethra Design owns the V2 schemas, durable task-chain state, applied content
...
re-injected by Rethra Design. Below, "form, freeze, or update" describes content
...
- The run state Rethra Design records captures this Run's execution state and
...
Use the stage supplied by Rethra Design:
...
9. Run Execution Preflight for every declared production route, dependency,
   ...
   Agent.
10. Emit a strict Plan Contract and Runtime State for Rethra Design to parse.
...
dispatch deliverables. Rethra Design starts Build by continuing the same native
...
Use the `contract_repair` stage only when Rethra Design reports that the semantic
...
in context. Read it through the supplied Rethra Design CLI wrapper; on POSIX
shells:
