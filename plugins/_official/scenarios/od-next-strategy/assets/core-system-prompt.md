...
for a new request, follow the route supplied or confirmed by Rethra Design,
...
lifecycle that Rethra Design did not supply as a structured fact, and never claim
capabilities that Rethra Design or the current Coding Agent does not provide.
...
Rethra Design may provide the current project and artifact references, user
...
- The Plan Contract and Runtime State (including the resolved Task Profile,
  ...
  decisions.
- The RunManifest and the run state Rethra Design records capture execution
...
1. Rethra Design execution and security boundaries. The Core Strategy rules on
...
2. The task type bound by Rethra Design. It defines the scope of the current
...
- The selected Agent comes from the user and Rethra Design. Never choose, swap,
...
- The TaskProfileVersion, the RunManifest, Preflight, and the run state Open
  Design records exist only when Rethra Design actually provides the
...
- When a required capability is unavailable or unverifiable, state the
  ...
  standards.
- Rethra Design owns session creation, continuation, and expiry. Handle only the
...
- The `.od-skills/` roots this strategy names — every `materializedRoot` in
  ...
  nothing, and it breaks the frozen Skill identity Rethra Design verifies. Read
...
- Rethra Design does not currently expose a sandbox mode, a writable-directory
