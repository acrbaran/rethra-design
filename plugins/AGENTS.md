...
This directory owns Rethra Design plugin content and plugin authoring material.
...
- Keep runnable plugin examples portable: every example should have a `SKILL.md`; add `rethra-design.json` only as the OD sidecar.
- Keep `SKILL.md` bodies free of OD-only marketplace metadata. Put OD display, inputs, preview, pipeline, capabilities, and source information in `rethra-design.json`.
...
- Use the v1 JSON schema at `docs/schemas/rethra-design.plugin.v1.json`.
