...
- E2E: run `pnpm --filter @rethra-design/e2e test tests/localized-content.test.ts` to verify real repository resources remain displayable under `de` / `fr` / `ru`, and missing category / tag translations do not block. Source: `e2e/tests/localized-content.test.ts:333-377`; `e2e/AGENTS.md:40-55`
- Web unit: run `pnpm --filter @rethra-design/web test` to cover localized ids still coming from dictionaries, plus skill/design-system/prompt-template field-level fallback. Source: `apps/web/tests/i18n/content.test.ts:12-80`; `apps/AGENTS.md:27-33,47-59`
...
- [x] Step 1: Adjust the localized-content test gate
  ...
  - [x] Substep 1.4 Verify: Run `pnpm --filter @rethra-design/e2e test tests/localized-content.test.ts`.
...
- [x] Step 3: Regression verification
  - [x] Substep 3.1 Verify: Run `pnpm --filter @rethra-design/web test`.
...
- `pnpm --filter @rethra-design/web test tests/i18n/content.test.ts` - passed.
- `pnpm --filter @rethra-design/e2e test tests/localized-content.test.ts` - initially found and cleaned up a pre-existing probe empty directory, then passed.
- Temporarily added English-only skill, design template, design system, and prompt template probes, then ran `pnpm --filter @rethra-design/e2e test tests/localized-content.test.ts` - passed; removed the temporary probe content afterward.
- `pnpm --filter @rethra-design/e2e test tests/localized-content.test.ts` - final passed.
- `pnpm --filter @rethra-design/web test` - passed, 110 files / 1013 tests.
- `pnpm guard` - passed.
