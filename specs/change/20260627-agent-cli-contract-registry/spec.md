...
Public Agent CLI Contract Registry for Rethra Design runtime compatibility.

## Why

Rethra Design depends on many external coding-agent CLIs. Their observable
...
Rethra Design has validated. The registry must support automated drift detection,
safe public publishing, and offline consumption by the Rethra Design repository.
...
- Create a public contract registry, likely `rethra-design-agent-contracts`.
- Publish a pnpm-consumable package, likely `@rethra-design/agent-cli-contracts`.
...
- Let Rethra Design CI replay supported contracts through the daemon parsers.
- Let Rethra Design runtime classify installed CLIs as verified, newer than
...
- Do not automatically generate Rethra Design parser logic from contract data.
...
| `parser_replay.rethra_design_commit` | no | `null` | commit used for parser replay |
...
| `rethra_design_min_commit` | no | `null` | lowest Rethra Design commit that consumed this matrix |
...
```json
{
  ...
  "parser_replay": {
    "rethra_design_commit": "...",
    "status": "compatible",
    "golden": "goldens/rethra-design-parser/json-event-stream/sha256-....events.json"
  }
}
```
...
Rethra Design starts agent CLIs as child processes, but the protocol over the
...
responding to the same handshake Rethra Design uses.
...
For Rethra Design-owned protocol semantics, the runner must not hand-roll a
...
- `acp-json-rpc` captures must reuse or vendor the Rethra Design ACP attach
...
- `pi-rpc` captures must reuse or vendor the Rethra Design Pi RPC attach harness,
...
async function captureContract(input: CaptureInput) {
  ...
  const daemonEvents = replayThroughRethraDesignParser({
    agent: input.agent,
...
prove successful streams parse correctly; it must also prove that Rethra Design's
...
Rethra Design CI should replay error fixtures through the same classifier used by
production:
...
- `confirmed`: observed at least twice, ideally across independent time windows
  or runners.
- `community_confirmed`: independently reproduced by a non-Rethra Design
...
## Rethra Design Consumption

Rethra Design should consume the contract package in three ways.
...
```ts
import contracts from "@rethra-design/agent-cli-contracts";

test("supported agent CLI contracts replay through Rethra Design parsers", () => {
...
```json
{
  "schema_version": 1,
  "generated_at": "2026-06-27T10:00:00Z",
  "rethra_design_min_commit": "b784c8650",
  "agents": [
    {
      "agent": "codex",
...
The registry should cover Rethra Design's integration surface, not every CLI
feature.
...
- Run replay harness against embedded Rethra Design parser package or a checked
  Rethra Design parser adapter.
- Run secret/path scans over all publishable artifacts.
- Generate support matrix from observations and check it is committed.

Rethra Design repository:
...
- Should Rethra Design consume the full contract package at runtime, or only a
