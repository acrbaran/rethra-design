...
- Multiple Claude config directories cause Rethra Design's spawned Claude process to use different state than the user's terminal session.
...
- `401`, `apiKeySource: "none"`, missing/invalid token, or authentication failure: tell the user to run `claude`, use `/login`, then retry the same Rethra Design run.
...
- Windows credential or WSL/native mismatch indicators: suggest re-authenticating in the same shell/environment used by Rethra Design and checking Windows Credential Manager where applicable.
