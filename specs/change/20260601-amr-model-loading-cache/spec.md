...
Rethra Design AMR runtime still calls legacy `vela models`, waits for that
...
longer wants Rethra Design to treat as primary.
...
Rethra Design should use the fast preset to occupy AMR model pickers while a
...
- Vela's new contract separates model discovery into `vela model preset`,
  ...
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:7-21`.
- Vela preset is a fast local picker seed, does not require login/config or
  ...
  Source: `/private/tmp/rethra-design-amr-model-catalog-handoff.md:50-61`.
- Vela preset JSON is requested with `vela model preset --format json` and
  returns `source: "preset"` plus `data[].id`. Source:
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:77-100`.
- Vela remote list is authoritative, requires a valid profile/control key,
  ...
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:102-150`.
- Legacy `vela models` is text-only compatibility output and should not be
  used by new picker/preflight paths. Source:
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:152-164`.
- New `vela model list` ids are already requestable AMR model names, so Open
  ...
  Source: `/private/tmp/rethra-design-amr-model-catalog-handoff.md:166-180`.
- Recommended picker flow is preset first, remote list in parallel or
  ...
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:182-189`.
...
- Decision: Use endpoint `source` values `"preset"` and `"remote"`, matching
  ...
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:77-100,127-150`;
...
- Decision: Keep generic `fallback` behavior unchanged. Non-AMR runtime
  ...
  CLI-provided local seed, not an Rethra Design static fallback. Source:
  `apps/daemon/src/runtimes/detection.ts:27-60`;
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:50-61`.
- Decision: Parse new Vela JSON by `data[].id` directly and do not apply
  ...
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:166-180`;
...
- Decision: Keep AMR execution preflight authoritative by using remote
  ...
  allowlist. Source:
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:56-60,102-112`;
  `apps/daemon/src/server.ts:11328-11414`.
- Decision: Warm the remote model loading cache when
  ...
  `/private/tmp/rethra-design-amr-model-catalog-handoff.md:106-112`.
...
- Final validation: run `pnpm guard`, `pnpm typecheck`,
  `pnpm --filter @rethra-design/daemon test -- amr-acp-integration`, and
...
- Passed: `pnpm --filter @rethra-design/daemon exec vitest run -c vitest.config.ts tests/amr-acp-integration.test.ts`.
- Passed after log-check import fix: `pnpm --filter @rethra-design/daemon typecheck`.
- Passed: `pnpm --filter @rethra-design/daemon exec vitest run -c vitest.config.ts tests/chat-route.test.ts -t "retries transient AMR Link catalog failures"`.
- Passed: `pnpm --filter @rethra-design/web exec vitest run -c vitest.config.ts tests/providers/daemon-amr-models.test.ts`.
- Passed: `pnpm --filter @rethra-design/daemon typecheck && pnpm --filter @rethra-design/web typecheck && pnpm --filter @rethra-design/contracts typecheck`.
