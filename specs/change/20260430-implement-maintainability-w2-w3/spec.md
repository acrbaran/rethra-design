...
- Root scripts run daemon, web, build, tests, and typecheck through pnpm filters; root `typecheck` currently targets only `@rethra-design/web`. Source: `package.json:12-25`
...
- Decision: Broaden typechecking from web-only to contracts, daemon, scripts, and e2e support. Root `typecheck` currently filters only `@rethra-design/web`; daemon has tests but no typecheck script; e2e already uses TypeScript configs and MJS/CJS operational files. Source: `package.json:19-25`, `apps/daemon/package.json:9-23`, `apps/web/tsconfig.json:2-23`, `e2e/package.json:6-12`, `e2e/playwright.config.ts:1-58`
...
- Contracts: run `pnpm --filter @rethra-design/contracts typecheck`; add lightweight type-level coverage via exported example payloads or `tsc`-checked fixture files. Source: `specs/current/maintainability-roadmap.md:57-58`
- Web adoption: run `pnpm --filter @rethra-design/web typecheck` and existing web tests after importing shared DTO/SSE/error types. Source: `apps/web/package.json:6-10`, `apps/web/tsconfig.json:2-23`
- Daemon adoption: add and run `pnpm --filter @rethra-design/daemon typecheck`, then `pnpm --filter @rethra-design/daemon test`; daemon already uses Vitest with TypeScript config. Source: `apps/daemon/package.json:9-23`, `apps/daemon/vitest.config.ts:1-8`
...
- Runtime migration: after each TypeScript conversion batch, run daemon tests and root typecheck; after script/e2e migration, run `pnpm --filter @rethra-design/e2e test` and a Playwright reporter smoke run when feasible. Source: `package.json:19-25`, `e2e/package.json:6-12`, `e2e/playwright.config.ts:22-37`
...
- `packages/contracts/package.json` - added `@rethra-design/contracts` package metadata, source exports, and `typecheck` script.
...
- `apps/web/package.json` and `apps/daemon/package.json` - added workspace dependencies on `@rethra-design/contracts` for boundary type adoption.
...
- `corepack pnpm --filter @rethra-design/contracts typecheck` - passed.
- `corepack pnpm --filter @rethra-design/web typecheck` - passed as a package graph/type resolution sanity check.
...
- `corepack pnpm install` - passed after adding app dependencies on `@rethra-design/contracts`; lockfile links web and daemon to the workspace package.
- `corepack pnpm --filter @rethra-design/contracts typecheck` - passed after Step 2 adoption.
- `corepack pnpm --filter @rethra-design/web typecheck` - passed after Step 2 adoption.
- `corepack pnpm --filter @rethra-design/web test -- src/providers/sse.test.ts` - passed; Vitest also ran existing artifact manifest tests in the web package.
- `corepack pnpm --filter @rethra-design/daemon test -- sse-response.test.mjs` - passed; Vitest also ran existing daemon artifact manifest and json event stream tests.
- `corepack pnpm install` - passed after adding daemon TypeScript/type dependencies.
- `corepack pnpm --filter @rethra-design/daemon typecheck` - passed after adding the daemon TypeScript foundation.
- `corepack pnpm --filter @rethra-design/daemon test` - passed; all 18 daemon tests passed.
...
- `corepack pnpm --filter @rethra-design/daemon typecheck` - passed after daemon module conversion.
- `corepack pnpm --filter @rethra-design/daemon build` - passed and emitted compiled daemon output under `apps/daemon/dist`.
- `corepack pnpm --filter @rethra-design/daemon test` - passed after daemon module conversion; all 18 daemon tests passed.
...
- `corepack pnpm --filter @rethra-design/e2e exec tsc -p ../scripts/tsconfig.json --noEmit` - passed for root TypeScript scripts.
- `corepack pnpm --filter @rethra-design/daemon build` - passed before e2e support typechecking and live-script import validation.
- `corepack pnpm --filter @rethra-design/e2e typecheck` - passed for Playwright config, reporter, report metadata, and e2e support scripts.
...
- `corepack pnpm --filter @rethra-design/e2e test:ui:clean` - passed against the TypeScript cleanup script.
- `corepack pnpm --filter @rethra-design/e2e exec playwright test -c playwright.config.ts --list` - passed as a Playwright config/reporter loading smoke check and listed 15 Chromium UI tests.
...
- `corepack pnpm --filter @rethra-design/e2e exec tsc -p ../scripts/tsconfig.json --noEmit` - passed after adding the residual JavaScript scanner.
