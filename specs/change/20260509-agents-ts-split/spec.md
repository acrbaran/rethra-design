...
- Verification commands should use daemon-scoped checks: `pnpm --filter @rethra-design/daemon typecheck` and `pnpm --filter @rethra-design/daemon test`. Source: `apps/AGENTS.md:39-46`; `specs/change/20260509-agents-ts-split/agents-merge-conflict-report.md:244-249,262-266`
...
- Validation: run `pnpm --filter @rethra-design/daemon typecheck` and `pnpm --filter @rethra-design/daemon test` after each implementation step. Source: `apps/AGENTS.md:39-46`; `specs/change/20260509-agents-ts-split/agents-merge-conflict-report.md:244-266`
...
- [x] Step 3: Split tests by responsibility
  ...
  - [x] Substep 3.3 Verify: run `pnpm --filter @rethra-design/daemon test` and ensure split tests still import compatibility APIs through the facade where relevant.
- [x] Step 4: Stabilize edge cases and review boundaries
  ...
  - [x] Substep 4.2 Verify: run `pnpm --filter @rethra-design/daemon typecheck` and `pnpm --filter @rethra-design/daemon test`.
...
- `pnpm --filter @rethra-design/daemon exec vitest run -c vitest.config.ts tests/runtimes` ✅
- `pnpm --filter @rethra-design/daemon exec vitest run -c vitest.config.ts tests/chat-route.test.ts` ✅ after one full-suite flaky failure in `tests/chat-route.test.ts`.
- `pnpm --filter @rethra-design/daemon test` ⚠️ runtime split tests passed; full suite still fails in existing unrelated `tests/finalize-design.test.ts` assertions where resolved artifact names include long relative temp paths.
