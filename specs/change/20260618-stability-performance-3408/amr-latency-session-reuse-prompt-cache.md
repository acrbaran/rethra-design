...
- **rethra-design daemon side (verified)**:
...
- **Data sources**: PostHog project **Rethra Design=420348** (`run_finished`, requires `phx_` key); Langfuse `us.cloud.langfuse.com` (trace_id==run.id). Queries are in the Reproduction section.
...
vela's Link gateway persists **one row per upstream model call** to Postgres `link.usage_events` (`input_tokens`, `cache_read_tokens`, `cache_write_tokens`, `latency_ms`, `created_at`, and `metadata` carrying `rethraDesignSessionId` / `rethraDesignRunId`). `uncached = input − cache_read − cache_write` (`pricing.go:89`). Querying DeepSeek calls (the AMR lead model; note `provider='openai'`, filter on `model ILIKE '%deepseek%'`), taking the **first upstream call of each run** and bucketing by run ordinal within the session:
...
*(Method note: measured read-only against production `link.usage_events` via an ephemeral in-cluster psql pod, statement-timeout + read-only transaction guarded. Turn ordinal uses `rethraDesignSessionId`/`rethraDesignRunId` from `metadata`; sessions straddling the query window's start can mislabel a mid-conversation run as turn-1, adding minor noise that does not change the picture.)*
...
- **Cross-repo vela**: session/load + persistence are vela changes; user owns vela, non-blocking; requires vela tests + rethra-design daemon integration.
...
PostHog Rethra Design=420348, `POST /api/projects/420348/query/`. HogQL: use `toFloat()` for numbers, `isNull()` for null, `quantile(0.9)` for P90, and `row_number() OVER (PARTITION BY conversation_id ORDER BY timestamp)` for turn ordinal.
