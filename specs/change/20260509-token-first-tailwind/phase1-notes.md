...
### Step 2: Rethra Design Tailwind tokens
...
- `scripts/guard.ts` - added default Tailwind palette utility rejection for classes such as `text-red-500`, `bg-white`, `border-zinc-200`, `from-orange-500`, and related color utility namespaces so app UI uses Rethra Design token utilities.
...
- Style guard allowlist and promotion policy:
  ...
  | Repeated arbitrary app UI colors | Any unregistered hardcoded color after the second real app UI use | Promote to a named Rethra Design token before migration. One-off brand/user-content/illustration data keeps a reasoned allowlist entry. |
...
- Baseline worktree: `/Users/william/projects/rethra-design` on `main`; candidate worktree: `/Users/william/projects/rethra-design-wt-tailwind-phase-1` on `tailwind-phase-1`.
...
- Startup commands:
  ```bash
  # In /Users/william/projects/rethra-design
  pnpm tools-dev run web --namespace tailwind-baseline --daemon-port 18110 --web-port 18111

  # In /Users/william/projects/rethra-design-wt-tailwind-phase-1
...
- `pnpm --filter @rethra-design/web build` - passed with Next.js 16/Turbopack.
- `pnpm --filter @rethra-design/web build` - passed after adding the Rethra Design `@theme` token aliases and source variables.
...
- `pnpm --filter @rethra-design/web test` - passed; 99 files / 925 tests.
...
- Dual-worktree services started successfully for the Phase 1 workflow smoke run:
  - Baseline: `/Users/william/projects/rethra-design`, namespace `tailwind-baseline`, daemon `http://127.0.0.1:18110`, web `http://127.0.0.1:18111`, command `pnpm tools-dev run web --namespace tailwind-baseline --daemon-port 18110 --web-port 18111`.
  - Candidate: `/Users/william/projects/rethra-design-wt-tailwind-phase-1`, namespace `tailwind-candidate`, daemon `http://127.0.0.1:18120`, web `http://127.0.0.1:18121`, command `pnpm tools-dev run web --namespace tailwind-candidate --daemon-port 18120 --web-port 18121`.
