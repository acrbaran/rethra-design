...
Packaged Rethra Design needs a hidden desktop Develop menu for AMR testing.
...
- Do not redefine Rethra Design release channels as AMR environments.
...
- The app-config env allowlist permits AMR keys including
  `RETHRA_DESIGN_AMR_PROFILE` and other AMR overrides such as `VELA_BIN`,
...
- Packaged daemon spawn env forwards a baked packaged `amrProfile` as
  `RETHRA_DESIGN_AMR_PROFILE` when present. Source:
...
- AMR runtime env assembly maps `RETHRA_DESIGN_AMR_PROFILE` into
...
- The AMR model endpoint resolves its probe by reading app config inside the
  ...
  `RETHRA_DESIGN_AMR_PROFILE`, `VELA_PROFILE`, credentials, and launch path.
...
- Handoff conclusion says no AMR runtime core change appears necessary; the
  ...
  `agentCliEnv.amr.RETHRA_DESIGN_AMR_PROFILE`. Source:
  `/private/tmp/rethra-design-amr-profile-packaged-handoff.md`.
- Handoff recommends desktop menu changes call daemon `GET /api/app-config`,
  merge `agentCliEnv.amr.RETHRA_DESIGN_AMR_PROFILE`, and then call
...
merging `agentCliEnv.amr.RETHRA_DESIGN_AMR_PROFILE` into the existing config.
...
- Missing `agentCliEnv.amr.RETHRA_DESIGN_AMR_PROFILE` displays as `prod`, matching
  AMR runtime default semantics.
- Writing a selected profile performs `GET /api/app-config`, merges
  `agentCliEnv.amr.RETHRA_DESIGN_AMR_PROFILE`, then sends `PUT /api/app-config`.
...
- Selecting `prod` writes `RETHRA_DESIGN_AMR_PROFILE: "prod"` explicitly instead
  of deleting the key.
...
sequenceDiagram
  ...
  Desktop->>Daemon: GET /api/app-config
  Desktop->>Daemon: PUT merged agentCliEnv.amr.RETHRA_DESIGN_AMR_PROFILE
  alt write succeeds
...
- `apps/desktop/src/main/index.ts`
  ...
  - Writes selected profile by merging
    `agentCliEnv.amr.RETHRA_DESIGN_AMR_PROFILE`, preserving other app config and
    AMR env overrides.
...
- `pnpm --filter @rethra-design/daemon typecheck`: passed.
- `pnpm --filter @rethra-design/desktop typecheck`: passed.
...
- An accidental broad `pnpm --filter @rethra-design/daemon test --
