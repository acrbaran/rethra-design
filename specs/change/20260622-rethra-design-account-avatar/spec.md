# Rethra Design 账号体系 + 用户头像（订阅/余额）设计方案

> 状态：设计中（design）。本 spec 是评审稿，**独立于** "Rethra Design Cloud 文案改造" PR（`feat/cloud-subscription-copy`），不随其合入。评审通过后另起实现分支。

## Context / 背景

AMR（Agentic Model Router）不再作为 Rethra Design 的独立产品，收敛成 Rethra Design 的订阅 / 云服务。随之而来的产品架构变化：

- **AMR 的账号体系 = Rethra Design 的账号体系。** 不再有"AMR 账号"，只有"Rethra Design 账号"（承接现有 vela / Rethra Design Cloud 账号）。
- Rethra Design 需要成为一个**可登录**的产品，并提供类似 Claude 的**常驻用户头像**：头像 · 套餐 ▾ →（点击）当前套餐 + 余额 + 管理/充值入口。

**已确认的产品决策：**
1. **全局账号，与运行方式解耦**——登录即"登录 Rethra Design"，头像常驻；哪怕用本地 CLI / BYOK 也是登录态。订阅是对 Rethra Design 的订阅，不是对某个运行时。
...
5. **身份源 = 现有 vela 账号（方案 A）**——把 vela / Rethra Design Cloud 账号确立为**唯一的 "Rethra Design 账号"**。身份、计费、未来的 org/seat/role 全部在**托管后端（vela）**演进；开源 repo **不自建第二套身份**，只做瘦客户端（账号契约 + 登录流 + 头像）。团队版的组织/席位/角色是 vela 账号模型的演进，不是本 repo 要造的东西。
...
Rethra Design 本身是开源产品，商业化走 open-core：**账号挂在托管后端，开源端只做可选的瘦客户端**。
...
- 不做团队版。
- 不重命名内部代码标识（agent id `amr`、`vela` 端点、`~/.amr` 等沿用，仅对外措辞用 Rethra Design / Rethra Design Cloud）。
...
- 登录/登出复用现有 `/api/integrations/vela/login|login/cancel|logout`（对外语义改为"Rethra Design 账号"，端点路径可不动）。
...
- **新组件 `AccountAvatar`**（与运行时切换器彻底分开）：
  ...
  - **未登录态**：头像位显示"登录 Rethra Design" CTA，点击触发现有 vela device-auth 登录流。
...
- **P4 文案/验证**：i18n（承接 Rethra Design Cloud 命名）+ 截图 + 测试。
...
5. ~~登录态拿不到套餐时的 fallback~~ → **已定：拿不到套餐就不显示状态**（方案②，只显示 Rethra Design Cloud 名称）。
...
- `pnpm --filter @rethra-design/web typecheck`、`pnpm guard`。
