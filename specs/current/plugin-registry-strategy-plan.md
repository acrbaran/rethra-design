# Rethra Design Plugin Registry Strategy Plan
...
Turn Rethra Design plugins from "installed local workflows plus a light marketplace index" into a registry-shaped ecosystem:

- Rethra Design hosts the official catalog on the main site and ships first-party plugins there.
- Community authors publish by opening GitHub PRs into the Rethra Design registry repository.
- `od` remains the canonical headless API for CRUD, search, install, update, trust, pack, doctor, and publish.
- Third parties can self-host the same `rethra-design-marketplace.json` shape as their own plugin source.
...
- `docs/plugins-spec.zh-CN.md` names CLI as the canonical API, supports headless OD, and reserves federated `rethra-design-marketplace.json`.
...
- The public marketing site has a static `/plugins/` registry renderer and per-plugin detail routes generated from `plugins/registry/*/rethra-design-marketplace.json` plus bundled official manifests.
...
- `apps/daemon/src/plugins/publish.ts` now builds submission links for external catalogs and the Rethra Design registry target. Full GitHub fork/branch/PR mutation is still future backend work.
...
1. **Official web registry**
   - Hosted on the main Rethra Design site.
   - Renders official and community-approved plugins from the Rethra Design registry repo.
...
Plugin source repo
  ...
  rethra-design.json must include plugin.repo.
        |
...
Plugin artifact
  GitHub ref, GitHub Release .tgz, HTTPS archive, or local folder.
        ...
        v
Registry backend
  v1: GitHub repo + generated rethra-design-marketplace.json.
...
- **rethra-design.invalid/plugins** is the public presentation of the official and community registry sources. It is equivalent to a polished static renderer over repo-owned catalog data, not a separate source of truth. `rethra-design.invalid/marketplace` can remain an alias later if needed.
...
- **Rethra Design GitHub registry repo** is the v1 storage backend. It can later be swapped for a database backend without changing user-facing nouns.
...
Rethra Design packaged runtime
...
Create plugin
  ...
  -> merge regenerates rethra-design-marketplace.json
  -> users refresh Sources, see Available, install into Installed
```

The `Create plugin` product entry should therefore start an agent workflow, not a bare JSON form. The agent can gather intent, generate `SKILL.md`, `rethra-design.json`, examples, previews, and capability metadata, run local validation, install a local test copy, pack it, and drive the `gh`-backed publish flow.

High-level architecture relationship:

```text
                    rethra-design.invalid/plugins
                 public registry pages and docs
                               |
                               v
+--------------------------------------------------+
| Rethra Design GitHub registry repo                 |
|                                                  |
| plugins/registry/official/rethra-design-marketplace.json |
| plugins/registry/community/rethra-design-marketplace.json |
| plugins/community/<vendor>/<plugin-name>/rethra-design.json |
| generated rethra-design-marketplace.json           |
+-----------------------------+--------------------+
                              |
...
- The generated `rethra-design-marketplace.json` is the machine-readable artifact fetched by daemon/CLI/UI.
- The per-plugin `community/**/rethra-design.json` or `entry.json` files are the human-reviewable source data used for GitHub PR governance.
- The file can initially live in the main Rethra Design repo if that lowers setup cost, but the product abstraction should still be `RegistryBackend`, not "read this monorepo path". Moving to `rethra-design/plugin-registry` later should be a data relocation, not a product rewrite.
...
`rethra-design-marketplace.json` remains the portable file that third-party hosts can copy. The per-plugin `entry.json` files make PRs reviewable, reduce merge conflicts, and let the build step generate the final marketplace index deterministically.
...
- Community submissions can land as plugin source folders under `plugins/community/<vendor-or-plugin>` and be referenced by `plugins/registry/community/rethra-design-marketplace.json`; trust tier stays encoded per source/entry.
...
- The source-of-truth repo is declared in `rethra-design.json` as `plugin.repo`. Published plugins can be "anything that packs": no special repo layout is required beyond passing `od plugin validate` and `od plugin pack`, shipping a runnable skill anchor, and declaring `plugin.repo`.
- `plugin.repo` points at the canonical source repository or subdirectory, for example `https://github.com/rethra-design/plugins/tree/main/make-a-deck`. Registry entries may mirror it as `sourceRepository` for search and review without replacing the manifest field.

Minimum entry shape:

```json
{
  "name": "rethra-design/make-a-deck",
  "title": "Make a Deck",
  "version": "1.2.0",
  "distTags": {
    "latest": "1.2.0",
  ...
  "source": "github:rethra-design/plugins@3f4c2d1/make-a-deck",
  "ref": "3f4c2d1",
  "dist": {
    "type": "github",
    "archive": "https://github.com/rethra-design/plugins/archive/3f4c2d1.tar.gz",
    "integrity": "sha512-...",
    "manifestDigest": "sha256-..."
  },
  "publisher": {
    "id": "rethra-design",
    "github": "rethra-design"
  },
  "sourceRepository": "https://github.com/rethra-design/plugins/tree/main/make-a-deck",
  "homepage": "https://rethra-design.invalid/plugins/rethra-design/make-a-deck/",
  "license": "MIT",
...
Required `rethra-design.json` fields for registry publish:
...
```json
{
  ...
  "version": "1.2.0",
  "plugin": {
    "repo": "https://github.com/rethra-design/plugins/tree/main/make-a-deck"
  }
}
```
...
- `LocalMarketplaceJsonBackend`: reads cached `rethra-design-marketplace.json` rows from SQLite.
...
- CLI commands must not expose GitHub-specific nouns except where the source explicitly is GitHub. `od plugin publish --to rethra-design` may use `gh` internally, but the command contract should survive a later database backend.
...
- `od plugin login` wraps `gh auth login` with Rethra Design copy and required scopes/host guidance.
...
- [x] `od plugin pack`
- [ ] `od plugin publish --to rethra-design [--dry-run]`
...
- `od plugin publish --to rethra-design` validates, packs if needed, renders the registry entry, creates a branch, opens a PR, and prints the PR URL.
...
- "Contribute to Rethra Design registry" flow with required files, checklist, CLI command, and GitHub PR path.
...
- Credentials live in `gh`, never inside `rethra-design-marketplace.json` and never inside the daemon database.
...
- [ ] Add formal `plugin.repo` schema field to `rethra-design.json` and require it for registry publish.
...
Goal: make Rethra Design contributions feel like npm publish, while actually opening a GitHub PR.
...
- [ ] Add `od plugin publish --to rethra-design --dry-run --json`.
...
- [x] Add public `/plugins/` route on the marketing site for rethra-design.invalid: searchable official/community registry listing, static plugin detail pages, canonical/OG/Twitter metadata, JSON-LD item/detail data, and homepage/header entry points.
...
- [ ] Add migration/import from static `rethra-design-marketplace.json` and GitHub registry repo into database rows.
- [ ] Add export back to `rethra-design-marketplace.json` so enterprises can mirror or air-gap catalogs.
...
- Should the official registry live in this monorepo or a separate `rethra-design/plugin-registry` repo? Separate is cleaner for community PR review and static-site generation.
- Should `official` marketplace trust be allowed for user-added URLs, or only for built-in source ids shipped by Rethra Design? Recommendation: only built-in sources can be `official`; user-added sources can be `trusted` or `restricted`.
...
- Source pointer: registry-published `rethra-design.json` must include `plugin.repo`.
...
The architecture is already pointed in the right direction: Rethra Design can become a "multi-source plugin registry" without giving up local-first/headless operation. The next move should not be a bigger gallery. It should be provenance, trust vocabulary, and exact resolution first; then the UI can safely graduate from "official plugins plus import" to "installed and available plugins across sources."
