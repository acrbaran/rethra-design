...
| W3 | Partial | Migrate project-owned code to TypeScript | R1 | W2 for highest-value shared types | TypeScript is the project-owned source default and the daemon package typechecks, but suppressions remain in core runtime files, newer brand/memory modules, and some tests. Remove them in owner-sized slices, then run `pnpm --filter @rethra-design/daemon typecheck`, `pnpm typecheck`, and `pnpm guard`. |
