...
The selected architecture is not an Rethra Design-packaged SDK carrier. It uses
the user's official `dsh` installation plus an Rethra Design profile bundle.

## 1. Product outcome

Rethra Design integrates DeepSeek Harness like Claude Code or Codex: the user
installs and owns the official coding-agent CLI, while Rethra Design detects and
launches it.

The desired user experience is:

1. The user installs the official `@deepseek-ai/dsh` package.
2. Rethra Design installs or detects an OD-specific profile named `rethra-design`.
...
The stable Rethra Design runtime identity is:
...
- the `rethra-design` profile bundle and its versioned stdio contract;
...
### Rethra Design explicitly does not own
...
into an Rethra Design responsibility.

## 3. Why an Rethra Design profile bundle is needed
...
```text
dsh plugin --profile rethra-design add <pinned OD bundle package>
```

The source lives in the Rethra Design repository so the host types, fake runtime,
profile implementation, and protocol fixtures change atomically. Its package
identity is `@rethra-design/dsh-runtime`, but end-user setup does not require a
...
```text
dsh plugin --profile rethra-design add <embedded pinned tarball>
```
...
2. `dsh --profile rethra-design --probe` emits a compatible probe frame and exits
   successfully.
...
```text
first run
  spawn dsh --profile rethra-design --stdio
  validate ready
...
`rethra-design` profile, Harness-managed credentials, and native session history.
...
| Profile frame | Rethra Design event |
| --- | --- |
...
- no Harness runtime is bundled into Rethra Design.
