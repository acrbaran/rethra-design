...
Define the target observability loop that lets Rethra Design improve agent
...
Issue: [#3713](https://github.com/acrbaran/rethra-design/issues/3713)

## Current State

Rethra Design already reports completed runs to Langfuse and PostHog. The current
...
That means Rethra Design can inspect individual runs, but the system does not yet
...
The gap is therefore not "does Rethra Design know how to talk to object storage?"
...
Rethra Design should treat observability as a closed loop:
...
alerting surface. Rethra Design owns the domain model that maps a design-agent run
...
Rethra Design is a design product, so the input and output surface is heavier than
...
| --- | --- |
| Rethra Design object storage | Stores original user attachments, parsed text, thumbnails, OCR or embedding outputs, generated artifacts, preview screenshots, export bundles, and reproducibility files. |
...
- always emit an `attachment_manifest` and `artifact_manifest` with stable ids,
  ...
  redaction status, and Rethra Design storage references;
...
Rethra Design should add a registry layer above `ProjectStorage` instead of
...
apply, Rethra Design should not write a score value; it should record
...
- Define the Rethra Design object storage contract for attachments, parsed
...
- Store original heavy inputs and outputs in Rethra Design controlled storage,
