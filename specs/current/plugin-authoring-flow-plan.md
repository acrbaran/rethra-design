...
- R2. The create entry navigates to Home, focuses `HomeHero`'s textarea, and pre-fills a prompt grounded in the Rethra Design plugin spec.
- R3. The task should create a real plugin folder containing at minimum `SKILL.md` and `rethra-design.json`, with optional examples/assets when the user asks for them.
...
- No new plugin spec dialect. The generated folder must validate against the existing Rethra Design plugin shape in `docs/plugins-spec.md` and `docs/schemas/rethra-design.plugin.v1.json`.
...
- [x] U3. **Plugin authoring scenario**
  ...
  **Files:**
  - Create: `plugins/_official/scenarios/od-plugin-authoring/rethra-design.json`
  ...
  **Approach:**
  - The scenario should instruct the agent to output a folder such as `generated-plugin/` with `SKILL.md`, `rethra-design.json`, and optional `examples/` or `assets/`.
  - The prompt should cite the Rethra Design plugin spec and require validation-ready output, not a prose-only explanation.
...
- [x] U4. **Generated folder visibility in project output**
  ...
  **Approach:**
  - Preserve relative paths so `generated-plugin/rethra-design.json` and `generated-plugin/SKILL.md` remain visibly grouped.
  ...
  **Test scenarios:**
  - Happy path: a project with `generated-plugin/rethra-design.json` and `generated-plugin/SKILL.md` renders a folder group.
...
```text
Create an Rethra Design plugin for: <user goal>.
...
- rethra-design.json with valid metadata, mode, task kind, inputs, and any pipeline/context references
