...
This spec defines how to bring Monet's **connectors** and **live artifacts** ideas into Rethra Design, but implement the agent-facing surface as **file-based agent skills plus daemon-owned local tools**, not as an in-process tool registry or MCP-first integration.

---

## 1. Product goal

Rethra Design should let an agent create previewable artifacts that are not just one-off generated files, but **live, refreshable, auditable views** backed by external or local data sources.
...
name: live-artifact
description: |
  Create refreshable, auditable Rethra Design artifacts backed by connector or local data.
