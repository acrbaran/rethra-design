...
const CATALOGUE: CuratedSkill[] = [
  ...
  },
  {
    id: 'ui-ux-pro-max',
    description:
      'Catalog-only UI/UX Pro Max entry. The full upstream templates, data, and search workflow are not bundled in RethraDesign.',
    ...
    catalogueOnlyNote:
      'RethraDesign ships this entry as discovery metadata only. The upstream UI/UX Pro Max data CSVs, scripts/search.py helper, templates, references, and related skill instructions are not bundled here; if those files are absent, disclose the limitation before falling back to RethraDesign defaults.',
  },
  {
    id: 'taste-skill',
    description:
...
function buildBody(s: CuratedSkill): string {
  ...
  if (s.catalogueOnlyNote) {
    lines.push('## Current RethraDesign scope');
    lines.push('');
  ...
  lines.push('');
  lines.push(
    'This catalogue entry advertises the skill in RethraDesign so the agent',
  );
  lines.push(
