/**
 * Refresh the commercial metadata for RethraDesign's slide-deck templates.
 *
 ...
 * `rethra-design.json`) and, for `example.html`, it STRIPS any stale
...
function updateRethraDesignJson(raw: string, copy: RefreshCopy): string {
...
function refreshTemplate(root: string, dir: string, copy: RefreshCopy, isOfficial: boolean, counts: { skill: number; manifest: number; html: number }, touched: string[]): void {
  ...
  if (existsSync(skillPath)) {
    ...
    const manifestPath = path.join(root, dir, 'rethra-design.json');
    ...
    if (isDeckSource(rawSkill, rawManifest)) {
      ...
      if (isOfficial && rawManifest !== null) {
        const nextManifest = updateRethraDesignJson(rawManifest, copy);
...
function main(): void {
  ...
  console.log(`Refreshed ${counts.skill} SKILL.md, ${counts.manifest} rethra-design.json, and stripped/retitled ${counts.html} example.html files across ${assignments.size} templates.`);
