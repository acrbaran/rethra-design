...
- `runtime/fossil.mjs` is a thin Shell-owned adapter. Store, update, activation,
  rollback, and lifecycle transactions remain in `@rethra-design/standalone`.
...
- Keep lifecycle execution behind the Standalone `LifecyclePort` seam and adapt
  that port only through the public `@rethra-design/sidecar` boundary. Terminal's fixture updater must exercise the
