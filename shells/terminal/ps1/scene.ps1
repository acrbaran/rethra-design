...
try {
  ...
  foreach ($directory in @("runtime/standalone", "runtime/node_modules/@rethra-design/sidecar/dist", "runtime/node_modules/@rethra-design/platform/dist", "seed", "sh", "ps1", "contract")) { [IO.Directory]::CreateDirectory((Join-Path $stage $directory)) | Out-Null }
  Copy-Item -LiteralPath (Join-Path $Standalone "index.mjs") -Destination (Join-Path $stage "runtime/standalone/index.mjs")
  Copy-Item -LiteralPath (Join-Path $Sidecar "index.mjs") -Destination (Join-Path $stage "runtime/node_modules/@rethra-design/sidecar/dist/index.mjs")
  Copy-Item -LiteralPath (Join-Path $Sidecar "supervisor.mjs") -Destination (Join-Path $stage "runtime/node_modules/@rethra-design/sidecar/dist/supervisor.mjs")
  Copy-Item -LiteralPath (Join-Path $Platform "index.mjs") -Destination (Join-Path $stage "runtime/node_modules/@rethra-design/platform/dist/index.mjs")
  [IO.File]::WriteAllText((Join-Path $stage "runtime/node_modules/@rethra-design/sidecar/package.json"), '{"name":"@rethra-design/sidecar","type":"module","exports":{".":"./dist/index.mjs"}}' + "`n", [Text.UTF8Encoding]::new($false))
  [IO.File]::WriteAllText((Join-Path $stage "runtime/node_modules/@rethra-design/platform/package.json"), '{"name":"@rethra-design/platform","type":"module","exports":{".":"./dist/index.mjs"}}' + "`n", [Text.UTF8Encoding]::new($false))
  ...
  $moduleFiles = @(
    "runtime/node_modules/@rethra-design/platform/dist/index.mjs",
    "runtime/node_modules/@rethra-design/platform/package.json",
    "runtime/node_modules/@rethra-design/sidecar/dist/index.mjs",
    "runtime/node_modules/@rethra-design/sidecar/dist/supervisor.mjs",
    "runtime/node_modules/@rethra-design/sidecar/package.json"
  )
