...
`@rethra-design/sidecar`/`@rethra-design/platform` runtime modules. The fossil adapter
uses only Sidecar's public convergence and control APIs, while lifecycle policy
stays in the installed `@rethra-design/standalone` artifact.
...
process discovery and generation fencing stay private to `@rethra-design/sidecar`.
...
OD_TERMINAL_NODE_ARCHIVE=/path/to/node-v24.18.0-darwin-arm64.tar.gz \
  pnpm --filter @rethra-design/terminal test
```
