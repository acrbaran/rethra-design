...
import { stopSidecars, type SidecarStamp } from "@rethra-design/sidecar";
import { canonicalJson, sha256Hex, signStandaloneChannelHead, signStandaloneMetadata, type StandaloneMetadata } from "@rethra-design/standalone";
...
function startToolsServeReleaseStorage(root: string): string {
  ...
  const server = spawn("pnpm", ["--silent", "--filter", "@rethra-design/tools-serve", "dev", "start", "release-storage", "--json", "--port", "0"], {
    cwd: repoRoot,
