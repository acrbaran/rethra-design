...
export function formatUnsupportedNodeRuntimeMessage(runtime: NodeRuntimeDiagnosticInput = currentNodeRuntime()): string {
  return [
    `tools-dev must run with ${SUPPORTED_NODE_RANGE} before starting RethraDesign.`,
    ...
    "  corepack pnpm --filter @rethra-design/daemon rebuild better-sqlite3 --pending",
  ].join("\n");
}
...
function formatNativeAddonAbiMismatchRecommendation(runtime: NodeRuntimeDiagnosticInput): string {
  const base = [
    "RethraDesign's dev stack must run with Node ~24.",
