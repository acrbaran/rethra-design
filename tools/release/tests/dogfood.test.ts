...
describe("dogfood destination guard", () => {
  it("accepts only keys inside the dogfood prefix", () => {
    expect(() => assertDogfoodObjectKey("dogfood/0.15.3-beta.7/1234-1/rethra-design-setup.exe")).not.toThrow();
  });
  ...
  it("refuses every release channel prefix", () => {
    for (const channel of ["beta", "prerelease", "preview", "stable"]) {
      ...
      expect(() => assertDogfoodObjectKey(`${channel}/versions/0.15.3/rethra-design.dmg`)).toThrow(/refusing to write outside/);
...
describe("dogfood key minting", () => {
  it("pins every minted key under the dogfood root", () => {
    const objectKey = dogfoodObjectKey({ buildId: "1234-1", fileName: "Rethra Design-release-beta-win-setup.exe", version: "0.15.3-beta.7" });
    expect(objectKey).toBe("dogfood/0.15.3-beta.7/1234-1/Rethra-Design-release-beta-win-setup.exe");
