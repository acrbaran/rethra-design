...
const stableTagPattern = /^rethra-design-v(\d+\.\d+\.\d+)$/;
...
function log(message: string): void {
  const prefix = process.env.RETHRA_DESIGN_RELEASE_CHANNEL === "prerelease" ? "release-prerelease" : "release-stable";
  console.log(`[${prefix}] ${message}`);
}

async function execGh(args: string[]): Promise<{ stdout: string }> {
  const nodeScript = process.env.RETHRA_DESIGN_GH_NODE_SCRIPT;
  if (nodeScript != null && nodeScript.length > 0) {
    return execFile(process.execPath, [nodeScript, ...args]);
  }
  return execFile(process.env.RETHRA_DESIGN_GH_BIN ?? "gh", args);
}
...
function parseChannel(value: string | undefined): ReleaseChannel {
  ...
  if (descriptor.channel !== "stable" && descriptor.channel !== "prerelease") {
    fail(`RETHRA_DESIGN_RELEASE_CHANNEL must be stable or prerelease; got ${channel}`);
  }
...
function parseStableDryRunMode(value: string | undefined): StableDryRunMode {
  ...
  fail("RETHRA_DESIGN_RELEASE_DRY_RUN must be metadata, prepublish, true, or false");
}
...
function resolvePrereleaseBaseVersion(branch: string, inputValue: string | undefined): ParsedStableVersion {
  ...
  const inputVersion = parseBaseVersionInput(inputValue, "RETHRA_DESIGN_STABLE_VERSION");

  if (branchVersion != null) {
    if (inputVersion != null && inputVersion.value !== branchVersion.value) {
      fail(
        `RETHRA_DESIGN_STABLE_VERSION ${inputVersion.value} must match release branch version ${branchVersion.value} when both are provided`,
      );
    }
  ...
  fail("release-prerelease requires either a release/vX.Y.Z branch or RETHRA_DESIGN_STABLE_VERSION");
}
...
async function validateStablePrereleaseMetadata(options: {
  ...
  if (prereleaseVersionInput.length === 0) {
    fail("RETHRA_DESIGN_STABLE_PRERELEASE_VERSION is required when channel=stable; pass the exact validated prerelease version");
  }

  const prerelease = parsePrereleaseVersion(prereleaseVersionInput, "RETHRA_DESIGN_STABLE_PRERELEASE_VERSION");
  ...
  const publicOrigin = trimTrailingSlash(
    options.publicOrigin ?? fail("RETHRA_DESIGN_RELEASES_PUBLIC_ORIGIN is required when channel=stable"),
  );
  validateHttpsUrl(publicOrigin, "RETHRA_DESIGN_RELEASES_PUBLIC_ORIGIN");
...
const channel = parseChannel(process.env.RETHRA_DESIGN_RELEASE_CHANNEL);
const stableDryRunMode = channel === "stable" ? parseStableDryRunMode(process.env.RETHRA_DESIGN_RELEASE_DRY_RUN) : "";
...
const stableBaseVersion =
  channel === "stable"
    ...
    : resolvePrereleaseBaseVersion(branch, process.env.RETHRA_DESIGN_STABLE_VERSION);
...
const versionTag = `rethra-design-v${packagedVersion}`;
...
let releaseName = `Rethra Design ${packagedVersion}`;
let prereleaseNumber = "";
let stateSource = channel === "prerelease" ? "R2 metadata.json" : "GitHub Releases";

if (channel === "prerelease") {
  const metadataUrl = process.env.RETHRA_DESIGN_PRERELEASE_METADATA_URL;
  if (metadataUrl == null || metadataUrl.length === 0) {
    fail("RETHRA_DESIGN_PRERELEASE_METADATA_URL is required for prerelease channel");
  }
  validateHttpsUrl(metadataUrl, "RETHRA_DESIGN_PRERELEASE_METADATA_URL");
  ...
  releaseName = `Rethra Design Prerelease ${releaseVersion}`;
  ...
  const stablePrerelease = await validateStablePrereleaseMetadata({
    ...
    commit,
    packagedVersion,
    prereleaseVersionInput: process.env.RETHRA_DESIGN_STABLE_PRERELEASE_VERSION,
    publicOrigin: process.env.RETHRA_DESIGN_RELEASES_PUBLIC_ORIGIN,
    repository,
  });
