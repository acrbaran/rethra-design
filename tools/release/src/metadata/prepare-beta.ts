...
const force = readBooleanEnv("RETHRA_DESIGN_RELEASE_FORCE") || readBooleanEnv("RELEASE_FORCE");

let latestStable: ParsedStableVersion | null = null;
const stableMetadataUrl = process.env.RETHRA_DESIGN_STABLE_METADATA_URL;
if (stableMetadataUrl != null && stableMetadataUrl.length > 0) {
  validateHttpsUrl(stableMetadataUrl, "RETHRA_DESIGN_STABLE_METADATA_URL");
  ...
  const tags = await fetchGitTags("rethra-design-v*");
...
const metadataUrl = process.env.RETHRA_DESIGN_BETA_METADATA_URL;
if (metadataUrl == null || metadataUrl.length === 0) {
  fail("RETHRA_DESIGN_BETA_METADATA_URL is required");
}
validateHttpsUrl(metadataUrl, "RETHRA_DESIGN_BETA_METADATA_URL");
...
const releaseName = `Rethra Design Beta ${betaVersion}`;
