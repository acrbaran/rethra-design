...
function targetConfig(): TargetConfig {
  if (target === "mac_arm64" || target === "mac_x64") {
    ...
    const dmg = `rethra-design-${releaseVersion}${assetSuffix}-mac-${arch}.dmg`;
    const zip = `rethra-design-${releaseVersion}${assetSuffix}-mac-${arch}.zip`;
    ...
    if (artifactMode === "dmg-and-payload" || artifactMode === "all") {
      const payload = `rethra-design-${releaseVersion}${assetSuffix}-mac-${arch}-payload.zip`;
  ...
  if (target === "win_x64") {
    const installer = `rethra-design-${releaseVersion}${assetSuffix}-win-x64-setup.exe`;
    const payload = `rethra-design-${releaseVersion}${assetSuffix}-win-x64-payload.7z`;
    const portableZip = `rethra-design-${releaseVersion}${assetSuffix}-win-x64-portable.zip`;
  ...
  const appImage = `rethra-design-${releaseVersion}${assetSuffix}-linux-x64.AppImage`;
  return {
    arch: "x64",
