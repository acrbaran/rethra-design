...
$InstallRoot = if ($env:OD_DSH_INSTALL_ROOT) { $env:OD_DSH_INSTALL_ROOT } else { Join-Path $LocalData 'RethraDesign\toolchains\dsh' }
...
function Finish([string]$Label) {
  ...
  Write-Host 'RethraDesign can discover this command without editing your PATH.'
  if (-not $NoLaunch) {
...
try {
  ...
  Write-Host "Installing dsh $DshVersion and pnpm $PnpmVersion in RethraDesign's user toolchain..."
