...
describe("updater fixture server", () => {
  ...
  it("serves a local artifact file as the updater installer", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-updater-fixture-"));
    const artifactPath = join(root, "Rethra Design-release-beta-win-setup.exe");
    ...
    try {
      ...
      expect(metadata.platforms?.win?.artifacts?.installer?.name).toBe("Rethra Design-release-beta-win-setup.exe");
  ...
  it("serves launcher payload bytes from a real archive path", async () => {
    ...
    const payloadPath = join(root, "Rethra Design-release-preview-payload.zip");
    ...
    try {
      ...
      expect(metadata.platforms?.mac?.artifacts?.payload?.name).toBe("Rethra Design-release-preview-payload.zip");
  ...
  it("serves a local launcher payload artifact file", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-updater-payload-fixture-"));
    const payloadPath = join(root, "Rethra Design-release-beta-win-payload.7z");
