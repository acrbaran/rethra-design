...
- **App version.** `win.packaged-app` omits it. It is applied through
  ...
  check over the app `package.json` version, the `rethra-design-config.json`
  `appVersion`, and the Windows executable fixed file version.
- **Namespace / channel and runtime endpoints.**
  `win.electron-builder-dir` omits them. `rethra-design-config.json` — which
...
`versionCore`: its content excludes `Rethra Design.exe`,
`resources/app/package.json`, and `resources/rethra-design-config.json`, which
