...
Local packaging control plane for Rethra Design.
...
Public release bundles keep channel-distinct identities: `Rethra Design.app`, `Rethra Design Beta.app`,
`Rethra Design Prerelease.app`, or `Rethra Design Preview.app`. Local `tools-pack install` adds the developer
...
- Finder/manual launches on macOS may not inherit your shell-managed `PATH`. If packaged Rethra Design cannot detect agent CLIs that work in Terminal, expose those binaries to the GUI login environment or launch the packaged app from a shell session that already sees them.
...
- AppImage: `~/.local/bin/Rethra-Design.<namespace>.AppImage`
- Menu entry: `~/.local/share/applications/rethra-design-<namespace>.desktop`
- Icon: `~/.local/share/icons/hicolor/512x512/apps/rethra-design-<namespace>.png`
...
`--headless` makes `install`, `start`, `stop`, `uninstall`, and `cleanup` operate on the headless entry (`@rethra-design/packaged/dist/headless.mjs`) instead of the AppImage. Headless mode runs daemon + web without Electron.

- `install --headless` writes a shell launcher at `~/.local/bin/rethra-design-headless-<namespace>` that bakes in the namespace and resource paths. The launcher is self-contained, but the assembled app directory at those paths must remain in place — don't move it after install.
