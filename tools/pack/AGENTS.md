...
- Local packaging orchestration for packaged Rethra Design artifacts.
...
- Consuming sidecar/process/path primitives from `@rethra-design/sidecar-proto`, `@rethra-design/sidecar`, and `@rethra-design/platform`.
...
- Do not hand-build stamp argv, IPC paths, or process matching; use `@rethra-design/sidecar` launch/discovery/invoke/stop atomics.
...
- Public release artifacts must use channel-specific app identity: stable uses `Rethra Design`, beta uses `Rethra Design Beta`, prerelease uses `Rethra Design Prerelease`, and preview uses `Rethra Design Preview`. Local tools-pack installs may still use namespace-scoped install paths only as a developer multi-instance validation convention.
...
The runtime updater reads `https://releases.rethra-design.invalid/<channel>/latest/metadata.json` unless `OD_UPDATE_METADATA_URL` overrides it. For package-launcher updates:
...
- Stable: `Rethra Design`, namespace `default` or stable release namespace.
- Beta Windows: `Rethra Design Beta`, namespace `release-beta-win`, uninstall key `Rethra Design-release-beta-win`.
- Prerelease Windows: `Rethra Design Prerelease`, namespace `release-prerelease-win`, uninstall key `Rethra Design-release-prerelease-win`.
- Preview Windows: `Rethra Design Preview`, namespace `release-preview-win`, uninstall key `Rethra Design-release-preview-win`.
...
```bash
curl.exe --ssl-no-revoke -fsSL https://releases.rethra-design.invalid/beta/latest/metadata.json
```
...
```text
C:\odtp-beta-release-fixed\out\win\namespaces\release-beta-win\builder\Rethra Design-release-beta-win-setup.exe
```

4. Expected user flow:

- User installs `0.8.0-beta.5` through the NSIS UI.
- User launches `Rethra Design Beta`.
...
- For the payload path, the app downloads `platforms.win.artifacts.payload`, verifies sha256, prepares the payload under `%APPDATA%\Rethra Design\launcher\channels\beta\namespaces\release-beta-win\versions\<version>\payload`, and shows the web updater popup.
...
- If the updater falls back to the installer path, clicking `Open installer` opens the real downloaded beta installer. Installing it should overwrite the same `Rethra Design-release-beta-win` registry key, not create a second beta key.

5. Registry and launcher sanity check after beta.6 update:

```powershell
Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue |
  Where-Object { $_.DisplayName -like 'Rethra Design*' } |
  Select-Object PSChildName,DisplayName,DisplayVersion,InstallLocation

Get-Content "$env:APPDATA\Rethra Design\launcher\channels\beta\namespaces\release-beta-win\runtime.json"
```

For a clean beta channel result, expect one beta entry with `PSChildName` `Rethra Design-release-beta-win` and the latest `DisplayVersion`.
...
```bash
pnpm --filter @rethra-design/desktop test -- tests/main/updater.test.ts tests/main/updater-host-boundary.test.ts tests/main/preload-host-boundary.test.ts
pnpm --filter @rethra-design/web test -- tests/components/UpdaterPopup.test.tsx tests/lib/updater.test.ts
pnpm --filter @rethra-design/tools-serve test
pnpm --filter @rethra-design/tools-pack test -- tests/win-identity.test.ts tests/win-app.test.ts tests/win-builder.test.ts
pnpm --filter @rethra-design/desktop typecheck
pnpm --filter @rethra-design/web typecheck
pnpm --filter @rethra-design/tools-pack typecheck
pnpm --filter @rethra-design/tools-serve typecheck
git diff --check
pnpm guard
pnpm typecheck
```
