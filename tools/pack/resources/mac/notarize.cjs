...
async function submitNotarization(filePath, credentials) {
  const useS3Acceleration = process.env.RETHRA_DESIGN_NOTARIZE_S3_ACCELERATION === "true";
...
async function notarizeApp(appPath, credentials) {
  const tempDir = await mkdtemp(path.join(tmpdir(), "rethra-design-notarize-"));
  try {
...
module.exports = async function notarize(context) {
  ...
  const attempts = parsePositiveInteger(process.env.RETHRA_DESIGN_NOTARIZE_ATTEMPTS, DEFAULT_ATTEMPTS);
  const retryDelayMs = parsePositiveInteger(process.env.RETHRA_DESIGN_NOTARIZE_RETRY_DELAY_MS, DEFAULT_RETRY_DELAY_MS);
