...
export async function buildWinLauncherPayloadArchive(
  ...
  const writeOverlay = async (input: { includeExecutable: boolean }): Promise<void> => {
    ...
    if (input.includeExecutable) {
      await cp(join(builtApp.unpackedRoot, "Rethra Design.exe"), join(overlayRoot, "payload", "Rethra Design.exe"));
    }
    await writeFile(
      join(overlayRoot, "payload", "resources", "rethra-design-config.json"),
...
export async function validateWinLauncherPayloadArchive(input: {
  ...
  try {
    ...
    requirePayloadManifestValue(manifest.entry?.executable, "entry.executable", "payload/Rethra Design.exe");

    await stat(join(extractRoot, archiveRelativePath("payload/Rethra Design.exe")));
    await stat(join(extractRoot, archiveRelativePath("payload/resources/rethra-design-config.json")));
