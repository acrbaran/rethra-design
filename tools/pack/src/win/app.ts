...
import { createCommandInvocation, createPackageManagerInvocation } from "@rethra-design/platform";
...
async function runEsbuild(config: ToolPackConfig, args: string[]): Promise<void> {
  await runPnpm(config, ["--filter", "@rethra-design/packaged", "exec", "esbuild", ...args]);
}
