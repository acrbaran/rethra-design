...
function resolveToolsPackRoot(startDir: string): string {
  ...
  for (let depth = 0; depth < maxDepth; depth += 1) {
    try {
      ...
      if (parsed.name === "@rethra-design/tools-pack") {
        return current;
      }
    } catch {
...
export const linuxResources = {
  ...
  desktopTemplate: join(resourcesRoot, "linux", "rethra-design.desktop.template"),
} as const;
 ...
 * teaches it Rethra Design's versioned JSONL stdio protocol.
 */
export async function packBundledDshRuntime({
  ...
  if (packageJson.name !== "@rethra-design/dsh-runtime" || typeof packageJson.version !== "string") {
    throw new Error("tools-pack: invalid @rethra-design/dsh-runtime package metadata");
  }
