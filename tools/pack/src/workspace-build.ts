...
export const WORKSPACE_BUILD_PACKAGES = [
  ...
  { directory: "packages/dsh-runtime", name: "@rethra-design/dsh-runtime" },
...
export const WORKSPACE_BUILD_COMMANDS = [
  {
    args: [
      "--filter", "@rethra-design/dsh-runtime...",
      "--workspace-concurrency=1", "--if-present", "run", "build",
    ],
  },
  {
    args: [
      "--filter", "@rethra-design/packaged^...",
      "--workspace-concurrency=1", "--if-present", "run", "build",
    ],
    env: ["OD_WEB_OUTPUT_MODE"],
  },
  { args: ["--filter", "@rethra-design/web", "run", "build:sidecar"] },
  { args: ["--filter", "@rethra-design/packaged", "run", "build"] },
] as const;
