...
function makeConfig(overrides: Partial<ToolPackConfig> = {}): ToolPackConfig {
  return {
    ...
    roots: {
      ...
      runtime: {
        namespaceBaseRoot: "C:/users/test/AppData/Local/rethra-design/runtime/win/namespaces",
        namespaceRoot: "C:/users/test/AppData/Local/rethra-design/runtime/win/namespaces/test-namespace",
      },
...
describe("readBuiltAppManifest", () => {
  it("returns null when the manifest file is missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-manifest-"));
    try {
  ...
  it("returns the parsed manifest when the version matches", async () => {
    ...
    try {
      ...
      const manifest: WinBuiltAppManifest = {
        ...
        executablePath: join(root, "Rethra Design.exe"),
        source: "namespace",
...
describe("writePackagedConfigFile", () => {
  it("omits namespaceBaseRoot for portable builds", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-config-"));
    try {
      const filePath = join(root, "config", "rethra-design-config.json");
  ...
  it("includes namespaceBaseRoot for non-portable builds", async () => {
    ...
    try {
      ...
      expect(written.namespaceBaseRoot).toBe(
        "C:/users/test/AppData/Local/rethra-design/runtime/win/namespaces",
      );
    } finally {
      await rm(root, { force: true, recursive: true });
    }
