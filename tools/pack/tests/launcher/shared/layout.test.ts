...
describe("tools-pack launcher layout", () => {
  ...
  it("resolves platform payload archive and extraction paths without changing installed app paths", () => {
    ...
    expect(macPayload.archivePath).toBe(join(mac.roots.output.namespaceRoot, "payload", "Rethra Design-release-beta-payload.zip"));
    expect(winPayload.archivePath).toBe(join(win.roots.output.namespaceRoot, "payload", "Rethra Design-release-beta-win-payload.7z"));
  ...
  it("exposes a stable mac payload zip path next to existing mac artifacts", () => {
    ...
    expect(paths.payloadZipPath).toBe(join(config.roots.output.namespaceRoot, "payload", "Rethra Design-release-beta-payload.zip"));
    expect(paths.dmgPath).toBe(join(config.roots.output.namespaceRoot, "dmg", "Rethra Design-release-beta.dmg"));
    expect(paths.zipPath).toBe(join(config.roots.output.namespaceRoot, "zip", "Rethra Design-release-beta.zip"));
  });
  ...
  it("exposes a stable Windows payload 7z path next to existing Windows artifacts", () => {
    ...
    expect(paths.launcherPayloadPath).toBe(join(config.roots.output.namespaceRoot, "payload", "Rethra Design-release-beta-win-payload.7z"));
    expect(paths.setupPath).toBe(join(config.roots.output.namespaceRoot, "builder", "Rethra Design-release-beta-win-setup.exe"));
    expect(paths.setupZipPath).toBe(join(config.roots.output.namespaceRoot, "builder", "Rethra Design-release-beta-win-portable.zip"));
  });
