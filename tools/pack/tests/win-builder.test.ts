...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Rethra Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Rethra Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Rethra Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Rethra Design", "Rethra Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Rethra Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Rethra Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Rethra Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Rethra Design", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Rethra Design.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-builder-"));
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Rethra Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
        join(cachedUnpackedRoot, "resources", "rethra-design-config.json"),
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "rethra-design-packaged-app", version: "0.5.0-beta.1" })}\n`,
        "utf8",
      );
      ...
      await expect(readFile(join(paths.unpackedRoot, "resources", "rethra-design-config.json"), "utf8")).resolves.toContain(
        '"namespace":"second"',
      );
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Rethra Design.exe"))).resolves.toMatchObject({
...
describe("launcher runtime sync helper", () => {
  it.runIf(process.platform === "win32")("writes cleanup.json for superseded launcher runtime pointers", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-launcher-sync-"));
