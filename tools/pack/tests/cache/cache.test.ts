...
describe("ToolPackCache", () => {
  it("builds once, materializes copies, and reports cache hits", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-tools-pack-cache-"));
  ...
  it("rebuilds stale entries when declared outputs are missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-tools-pack-cache-stale-"));
    const cacheRoot = join(root, "cache");
  ...
  it("reads existing hits without building", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-tools-pack-cache-read-hit-"));
    const cacheRoot = join(root, "cache");
    const out = join(root, "out", "payload");
    let builds = 0;
    const cache = new ToolPackCache(cacheRoot);
  ...
  it("reuses stamped materialized targets until the cache key changes", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-tools-pack-cache-reuse-"));
    const cacheRoot = join(root, "cache");
    const out = join(root, "out", "payload");
    let builds = 0;
    const cache = new ToolPackCache(cacheRoot);
  ...
  it("seeds a miss from an alias but still builds the exact entry", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-tools-pack-cache-seed-"));
    const cacheRoot = join(root, "cache");
    const out = join(root, "out", "payload");
    let builds = 0;
    const cache = new ToolPackCache(cacheRoot);
  ...
  it("skips alias seeds that cannot materialize the requested source", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-tools-pack-cache-missing-seed-source-"));
    const cacheRoot = join(root, "cache");
    const out = join(root, "out", "payload");
    let builds = 0;
    const cache = new ToolPackCache(cacheRoot);
