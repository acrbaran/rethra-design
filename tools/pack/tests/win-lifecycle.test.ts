...
import { SIDECAR_MESSAGES } from "@rethra-design/sidecar-proto";
...
const listProcessSnapshots = vi.hoisted(() =>
  vi.fn<typeof import("@rethra-design/platform").listProcessSnapshots>(async () => []),
);
const matchesStampedProcess = vi.hoisted(() =>
  vi.fn<typeof import("@rethra-design/platform").matchesStampedProcess>(() => false),
);
...
vi.mock("@rethra-design/sidecar", async () => {
  const actual = await vi.importActual<typeof import("@rethra-design/sidecar")>("@rethra-design/sidecar");
  return {
    ...actual,
...
vi.mock("@rethra-design/platform", async () => {
  const actual = await vi.importActual<typeof import("@rethra-design/platform")>("@rethra-design/platform");
  return {
    ...actual,
...
describe("installPackedWinApp", () => {
  it("pins the installed portable config to the tools-pack namespace for bare protocol launches", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-lifecycle-"));
    const config = { ...createConfig(root), portable: true };
    const paths = resolveWinPaths(config);
    const installedConfigPath = join(paths.installDir, "resources", "rethra-design-config.json");
