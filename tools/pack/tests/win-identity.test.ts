...
describe("resolveWinInstallIdentity", () => {
  it("keeps the default namespace on the canonical Windows display name", () => {
    expect(resolveWinInstallIdentity({ namespace: "default" })).toMatchObject({
      displayName: "Rethra Design",
      shortcutName: "Rethra Design.lnk",
      uninstallerName: "Uninstall Rethra Design.exe",
    });
  });
  ...
  it("uses the canonical Windows display name for stable release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-stable-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Rethra Design.exe",
      displayName: "Rethra Design",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Rethra Design-release-stable-win",
      shortcutName: "Rethra Design.lnk",
      uninstallerName: "Uninstall Rethra Design.exe",
    });
  });
  ...
  it("uses first-class beta display identity for beta release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-beta-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Rethra Design Beta.exe",
      displayName: "Rethra Design Beta",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Rethra Design-release-beta-win",
      shortcutName: "Rethra Design Beta.lnk",
      uninstallerName: "Uninstall Rethra Design Beta.exe",
    });
  });
  ...
  it("keeps non-release beta-like namespaces isolated from the real beta channel identity", () => {
    expect(resolveWinInstallIdentity({ namespace: "beta-local-flow" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Rethra Design beta-local-flow.exe",
      displayName: "Rethra Design beta-local-flow",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Rethra Design-beta-local-flow",
      shortcutName: "Rethra Design beta-local-flow.lnk",
      uninstallerName: "Uninstall Rethra Design beta-local-flow.exe",
    });
  });
  ...
  it("uses first-class preview display identity for preview release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-preview-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Rethra Design Preview.exe",
      displayName: "Rethra Design Preview",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Rethra Design-release-preview-win",
      shortcutName: "Rethra Design Preview.lnk",
      uninstallerName: "Uninstall Rethra Design Preview.exe",
    });
  });
  ...
  it("uses first-class prerelease display identity for prerelease release versions and namespaces", () => {
    expect(resolveWinInstallIdentity({
      ...
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Rethra Design Prerelease.exe",
      displayName: "Rethra Design Prerelease",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Rethra Design-release-stable-win",
      shortcutName: "Rethra Design Prerelease.lnk",
      uninstallerName: "Uninstall Rethra Design Prerelease.exe",
    });
  ...
  it("emits a valid NSIS command literal for executable paths containing spaces", () => {
    expect(createNsisQuotedCommandLiteral(["$INSTDIR\\Rethra Design.exe", "%1"])).toBe(
      `'"$INSTDIR\\Rethra Design.exe" "%1"'`,
    );
    expect(createNsisQuotedCommandLiteral(["$INSTDIR\\Rethra Design.exe"])).toBe(
      `'"$INSTDIR\\Rethra Design.exe"'`,
    );
  });

  it("removes an Electron-refreshed invite protocol while this install still owns it", () => {
    const source = winCustomInstallerSource;
    expect(source).toContain('const inviteProtocolKey = "Software\\\\Classes\\\\rethradesign"');
