...
async function createWorkspaceFixture(workspaceRoot: string): Promise<void> {
  ...
  await writeFile(
    join(workspaceRoot, "plugins", "registry", "community", "rethra-design-marketplace.json"),
    "{\"plugins\":[]}\n",
    "utf8",
  );
...
describe("prepareResourceTree", () => {
  it("bundles the DeepSeek Harness runtime into the Windows resource tree", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-dsh-runtime-"));
    const workspaceRoot = join(root, "workspace");
    const resourceRoot = join(root, "materialized", "rethra-design");
  ...
  it("invalidates the Windows resource tree cache when design templates change", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-resources-"));
    const workspaceRoot = join(root, "workspace");
    const resourceRoot = join(root, "materialized", "rethra-design");
    const cache = new ToolPackCache(join(root, "cache"));
    const config = { workspaceRoot } as ToolPackConfig;
  ...
  it("invalidates the Windows resource tree cache when the plugin-preview manifest changes", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-previews-"));
    const workspaceRoot = join(root, "workspace");
    const resourceRoot = join(root, "materialized", "rethra-design");
    const cache = new ToolPackCache(join(root, "cache"));
    const config = { workspaceRoot } as ToolPackConfig;
  ...
  it("copies a configured Vela CLI binary into the Windows resource tree", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-vela-"));
    const workspaceRoot = join(root, "workspace");
    const resourceRoot = join(root, "materialized", "rethra-design");
    ...
    const config = { workspaceRoot } as ToolPackConfig;
    const paths = { resourceRoot } as WinPaths;
    const originalVelaBin = process.env.RETHRA_DESIGN_VELA_CLI_BIN;
    ...
    try {
      ...
      process.env.RETHRA_DESIGN_VELA_CLI_BIN = source;
      ...
      if (originalVelaBin == null) delete process.env.RETHRA_DESIGN_VELA_CLI_BIN;
      else process.env.RETHRA_DESIGN_VELA_CLI_BIN = originalVelaBin;
      await rm(root, { force: true, recursive: true });
    }
  });

  it("fails strict Windows resource preparation when configured Vela CLI is missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-vela-strict-"));
    const workspaceRoot = join(root, "workspace");
    const resourceRoot = join(root, "materialized", "rethra-design");
    const cache = new ToolPackCache(join(root, "cache"));
    const config = {
    ...
    try {
      await createWorkspaceFixture(workspaceRoot);
      process.env.RETHRA_DESIGN_VELA_CLI_BIN = join(root, "missing", "vela.exe");
      await expect(
  ...
  it("invalidates the Windows resource tree cache when the Vela companion changes", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-vela-companion-"));
    const workspaceRoot = join(root, "workspace");
    const resourceRoot = join(root, "materialized", "rethra-design");
    const source = join(root, "source", "vela.exe");
    const cache = new ToolPackCache(join(root, "cache"));
