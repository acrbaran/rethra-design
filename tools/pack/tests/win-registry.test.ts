...
describe("stripRegistryQuotedValue", () => {
  it("returns the quoted body when the value is fully quoted", () => {
    expect(stripRegistryQuotedValue('"C:\\Program Files\\Rethra Design\\Uninstall.exe"')).toBe(
      "C:\\Program Files\\Rethra Design\\Uninstall.exe",
    );
  });

  it("returns the inner segment when the quoted region carries trailing flags", () => {
    expect(stripRegistryQuotedValue('"C:\\Program Files\\Rethra Design\\Uninstall.exe" /S')).toBe(
      "C:\\Program Files\\Rethra Design\\Uninstall.exe",
    );
  });

  it("trims unquoted values and leaves them otherwise unchanged", () => {
    expect(stripRegistryQuotedValue("  C:\\Rethra Design\\Uninstall.exe  ")).toBe(
      "C:\\Rethra Design\\Uninstall.exe",
    );
  });
