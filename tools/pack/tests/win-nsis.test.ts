...
describe("writeNsisInclude", () => {
  it("escapes double quotes and newlines in the local-data root", async () => {
    const root = await mkdtemp(join(tmpdir(), "rethra-design-win-nsis-"));
    try {
      const includePath = join(root, "include", "rethra-design.nsh");
