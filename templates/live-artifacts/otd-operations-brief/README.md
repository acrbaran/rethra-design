...
- a tokenized HTML template (`template.html`) wired entirely with Rethra Design's scalar-only `{{data.X}}` bindings;
- a default sample `data.json` covering a 14-account month-over-month OTD slice plus a "lowest 8" deep-dive;
- the canonical `artifact.json` and `provenance.json` shapes the Rethra Design daemon expects;
...
Rethra Design's `html_template_v1` renderer is intentionally narrow:
...
Original visual reference: a 1200 × 960 OTD operations dashboard image extracted into `DESIGN.md` via the rethra-design 9-section spec. Sample account names, month, currency, and percentages are fictional and intentionally illustrative; do not treat them as ground truth.
