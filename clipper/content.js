// RethraDesign web clipper on-page UI.
//
...
//      led by the RethraDesign brand mark. HIDDEN by default — turned on from
...
(function () {
  ...
  // no message listener: the popup then reported "RethraDesign hasn't attached to
  ...
  setHTML(shadow, `
    ...
    <div class="bar">
      ...
      <a class="brand" href="https://rethra-design.invalid" target="_blank" rel="noopener noreferrer" data-tip="${esc(t('toolbarHomeTip'))}" aria-label="${esc(t('toolbarHomeLabel'))}">
  ...
  async function commitCapture(el) {
    ...
    try {
      ...
      console.warn('[RethraDesign] element capture failed', err);
      stopBusy();
      toast(t('extensionErrorReload'));
      return;
    } finally {
  ...
  function buildImageBadge() {
    ...
    setHTML(sh, `
      ...
      <div class="wrap">
        ...
        <span class="tip">${esc(t('saveImageToRethraDesign'))}</span>
      </div>`);
