# Rethra Design Web Clipper (MV3 — Chrome · Edge · Firefox)

The **Rethra Design Web Clipper**: a browser extension that captures images,
screenshots, and page content from any site straight into your Rethra Design
...
1. Start Rethra Design locally (`pnpm tools-dev`) so the daemon is listening on
...
**● Connected** as soon as it detects Rethra Design running, and you can capture
right away.
...
- **On-page toolbar** → an optional floating page / design-system / Figma JSON /
  ...
  *Show on-page bar* in the popup (the preference is remembered, and the bar's
  ...
  snapshot.
- **Right-click an image** → *Save image to Rethra Design Library*.
