// RethraDesign web clipper popup. Thin UI over the service worker message API.
//
// Zero-config: no pairing. The popup just probes whether RethraDesign is running
...
function render(connected) {
  ...
  // "RethraDesign not running" message instead of being hidden, so it never feels broken.
}
...
function reportCapture(res, okText) {
  ...
  if (res.error === 'not running') {
    setMsg(t('rethraDesignNotRunning'), 'err');
    void refresh();
  } else {
