...
- 💬 **Keep talking while the model is still working.** Queue chat sends mid-stream, then let Rethra Design continue the conversation the moment the current turn finishes. Studio and Draw now follow the same flow, so capturing an idea does not depend on waiting for the previous response to end. (#2870, #3270, #1961) Thanks @zoeforfun, @lefarcen, @leno23.
...
- 📦 **Trying Rethra Design gets easier on Windows and Linux.** Windows gets a portable zip path; Linux gets Docker/Podman Compose one-click setup. Less install friction, faster first run. (#2937, #2414) Thanks @PerishCode, @epicsagas.
- 🔌 **MCP clients can now do real workspace work.** Write files, delete files, delete projects, resolve the active project directory, run generation loops, and bootstrap Codex from one place. External clients can now participate in the Rethra Design workspace instead of only observing it. (#2416, #2802, #3141) Thanks @YOMXXX, @papperrollinggery, @lefarcen.
- 🛂 **Network changes stop requiring a restart.** Switch the system proxy while Rethra Design is running; the next outbound request picks it up automatically. (#3093) Thanks @mrcfps.
...
#### 🤖 Agents & runtimes
- **Rethra Design AMR (vela) as a first-class ACP stdio agent**, with onboarding, sign-in pill, and bundled `vela` CLI inside the installer. (#2355) Thanks @lefarcen.
...
- **Refresh Rethra Design app visuals** — new app icons, logo, brand glyphs. ([#2436])
...
- **Plugin & marketplace system — Phase 2A + 1 + 1.5 + 2B + 2C entry slice + 3 (full) + 4 (full incl. OD_BUNDLED_ATOM_PROMPTS default ON) + 5 (full incl. live S3 impl; postgres adapter still stubbed) + 6 (full incl. asset rasterisation) + 7 (all six code-migration atom impls landed; full pipeline e2e covered by smoke test) + 8 (full incl. GenUI \u2192 decision bridge + handoff promotion ladder bridge) + bundled scenarios + bundled-scenario fallback resolver.** Spec: [`docs/plugins-spec.md`](docs/plugins-spec.md). Living plan: [`docs/plans/plugins-implementation.md`](docs/plans/plugins-implementation.md).
  ...
  - **figma-migration pipeline e2e smoke (Phase 6).** New `plugins-figma-migration-e2e.test.ts` walks figma-extract (stubbed REST) → token-map → diff-review → handoff end-to-end. Locks the scenario folder roster (`plugins/_official/scenarios/od-figma-migration/rethra-design.json` stages: extract → tokens → generate → critique).
...
- **Plugin & marketplace system — earlier landing.** Spec: [`docs/plugins-spec.md`](docs/plugins-spec.md). Living plan: [`docs/plans/plugins-implementation.md`](docs/plans/plugins-implementation.md).
  ...
  - **Phase 6 / 7 / 8 atom SKILL.md substrate.** Nine new `plugins/_official/atoms/<atom>/{SKILL.md, rethra-design.json}` pairs the bundled boot walker registers on startup: `figma-extract`, `token-map` (Phase 6); `code-import`, `design-extract`, `rewrite-plan`, `patch-edit`, `diff-review`, `build-test` (Phase 7); `handoff` (Phase 8). The fragments teach the agent what each (planned) atom expects so a plugin author who references one of these ids in `od.pipeline.stages[*].atoms[]` sees the canonical fragment without a doctor warning. The matching shell-out implementations stay scheduled per spec §16 Phase 6 / 7 / 8.
- **Plugin & marketplace system — earlier landing.** Spec: [`docs/plugins-spec.md`](docs/plugins-spec.md). Living plan: [`docs/plans/plugins-implementation.md`](docs/plans/plugins-implementation.md).
  - **Per-cloud Helm value overrides.** `tools/pack/helm/rethra-design/values-{aws,gcp,azure,aliyun,tencent,huawei,self}.yaml` ship the volume + ingress diffs spec §15.5 enumerates. Operators install with `helm install od ./tools/pack/helm/rethra-design -f values-aws.yaml`.
...
- **Plugin & marketplace system — earlier landing.** Spec: [`docs/plugins-spec.md`](docs/plugins-spec.md). Living plan: [`docs/plans/plugins-implementation.md`](docs/plans/plugins-implementation.md).
  ...
  - **Helm chart templates.** `tools/pack/helm/rethra-design/templates/` ships Deployment, Service, Secret, ConfigMap, two PVCs, optional Ingress, plus _helpers.tpl + NOTES.txt. The chart installs end-to-end with `helm install od ./tools/pack/helm/rethra-design --set secrets.apiToken=$(openssl rand -hex 32)`.
...
- **Plugin & marketplace system — earlier landing.** Spec: [`docs/plugins-spec.md`](docs/plugins-spec.md). Living plan: [`docs/plans/plugins-implementation.md`](docs/plans/plugins-implementation.md).
  ...
  - **Phase 5 Dockerfile + docker-compose + Helm chart entry slice.** `deploy/Dockerfile` now bundles `plugins/_official/` so `registerBundledPlugins()` finds the atom set inside the container. `tools/pack/docker-compose.yml` is the canonical hosted-mode manifest (two-volume layout, OD_API_TOKEN, /api/daemon/status healthcheck). `tools/pack/helm/rethra-design/` pins the Helm chart parameter surface for the per-cloud value overrides spec §15.5 enumerates; templates land in the Phase 5 follow-up PR.
- **Plugin & marketplace system — earlier landing.** Spec: [`docs/plugins-spec.md`](docs/plugins-spec.md). Living plan: [`docs/plans/plugins-implementation.md`](docs/plans/plugins-implementation.md).
...
- **Plugin & marketplace system — Phase 2A + 1 + 1.5 + 2B (composer mount + marketplace deep UI) + 2C entry slice + 3 entry slice + 4 (scaffold / export / atoms doc) + early 5 (earlier landing).** Spec: [`docs/plugins-spec.md`](docs/plugins-spec.md). Living plan: [`docs/plans/plugins-implementation.md`](docs/plans/plugins-implementation.md).
  - **Phase 4 author tooling.** `od plugin scaffold --id <id>` writes a starter SKILL.md + rethra-design.json + README.md (optionally a `.claude-plugin/plugin.json`). `od plugin export <projectId> --as od|claude-plugin|agent-skill --out <dir>` materialises a publish-ready folder from the AppliedPluginSnapshot behind a project (or a snapshot id), so any chat that ran a plugin can be re-published to anthropics/skills, awesome-agent-skills, clawhub, or skills.sh without leaving the terminal. The output's rethra-design.json carries a provenance block (snapshotId + manifestSourceDigest + appliedAt) that reverse-resolves to the originating run.
...
- Shared `CritiqueRoundSummary` / `CritiqueRunStatus` types via the `@rethra-design/contracts` package. ([#1016])
...
- Pack/win: close detection gaps that let `Rethra Design.exe` stay locked at install time. ([#823])
...
- Release: Rethra Design 0.5.0 changelog landing. ([#820])
...
#### Packaging
- Fixed packaged desktop startup by building `@rethra-design/contracts` to `dist/*.mjs` + `.d.ts`, pointing its exports at compiled JavaScript, and building contracts before all packaged lanes pack workspace tarballs. ([#577])
...
A multi-protocol leap: Rethra Design now ships as an MCP server, ships Critique Theater (Design Jury) Phase 4, gains live-reload + Tweaks mode + live artifacts in the preview pane, and adds five new agent / runtime adapters. 71 merged PRs from 40+ contributors over two days. Linux AppImage packaging landed in tooling, but the stable Linux artifact is deferred from 0.4.0 while containerized release packaging is hardened.

### Added

#### MCP & agent integration
- **`od mcp` — expose Rethra Design as a stdio MCP server.** Coding agents in other repos (Claude Code, Codex, Cursor, VS Code, Antigravity, Zed, Windsurf) can read files from local Rethra Design projects directly, including the project the user has open in the Rethra Design app right now. ([#399])
...
- `rethra-design-landing` rename, **kami skill bundle**, and landing OG assets. ([#428])
...
First public release of Rethra Design — a local-first, open-source alternative to Anthropic's Claude Design. It detects your installed code-agent CLI, runs design skills against curated design systems, and streams artifacts into a sandboxed in-app preview.
...
- Initial project structure, project rename "Open Claude Design" → "Rethra Design", naming optimization. ([#1], [#2])
- Initial AGENTS.md and OpenCode agent instructions. ([#114])
- Beta release workflow placeholder. ([#36])
- Git commit co-author policy. ([#131])

[Unreleased]: https://github.com/acrbaran/rethra-design/compare/rethra-design-v0.9.0...HEAD
[0.9.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.9.0
[0.8.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.8.0
[0.7.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.7.0
[0.6.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.6.0
[0.5.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.5.0
[0.4.1]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.4.1
[0.4.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.4.0
[0.3.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.3.0
[0.2.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.2.0
[0.1.0]: https://github.com/acrbaran/rethra-design/releases/tag/rethra-design-v0.1.0

[#1]: https://github.com/acrbaran/rethra-design/pull/1
[#2]: https://github.com/acrbaran/rethra-design/pull/2
[#4]: https://github.com/acrbaran/rethra-design/pull/4
[#5]: https://github.com/acrbaran/rethra-design/pull/5
[#7]: https://github.com/acrbaran/rethra-design/pull/7
[#9]: https://github.com/acrbaran/rethra-design/pull/9
[#12]: https://github.com/acrbaran/rethra-design/pull/12
[#13]: https://github.com/acrbaran/rethra-design/pull/13
[#14]: https://github.com/acrbaran/rethra-design/pull/14
[#15]: https://github.com/acrbaran/rethra-design/pull/15
[#19]: https://github.com/acrbaran/rethra-design/pull/19
[#24]: https://github.com/acrbaran/rethra-design/pull/24
[#26]: https://github.com/acrbaran/rethra-design/pull/26
[#28]: https://github.com/acrbaran/rethra-design/pull/28
