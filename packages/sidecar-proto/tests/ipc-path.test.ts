...
describe("normalizeIpcPath", () => {
  it("returns absolute POSIX paths unchanged", () => {
    expect(normalizeIpcPath("/tmp/rethra-design/ipc/ns/web.sock")).toBe("/tmp/rethra-design/ipc/ns/web.sock");
  });

  it("returns Windows named-pipe paths unchanged without absolute-path checking", () => {
    expect(normalizeIpcPath("\\\\.\\pipe\\rethra-design-ns-web")).toBe("\\\\.\\pipe\\rethra-design-ns-web");
  });
  ...
  it("throws when the path has leading or trailing whitespace", () => {
    expect(() => normalizeIpcPath(" /tmp/rethra-design/ipc/ns/web.sock")).toThrow(/whitespace/);
    expect(() => normalizeIpcPath("/tmp/rethra-design/ipc/ns/web.sock\n")).toThrow(/whitespace/);
  });

  it("throws when the path contains a null byte", () => {
    expect(() => normalizeIpcPath("/tmp/rethra-design/ipc/ns/web.sock\0extra")).toThrow(/null bytes/);
  });
...
describe("isWindowsNamedPipePath", () => {
  it("returns true for paths starting with \\\\.\\pipe\\", () => {
    expect(isWindowsNamedPipePath("\\\\.\\pipe\\rethra-design-ns-web")).toBe(true);
    expect(isWindowsNamedPipePath("\\\\.\\pipe\\")).toBe(true);
  });

  it("returns false for POSIX, drive-letter, or non-string inputs", () => {
    expect(isWindowsNamedPipePath("/tmp/rethra-design/ipc/ns/web.sock")).toBe(false);
    expect(isWindowsNamedPipePath("C:\\Users\\ipc\\web.sock")).toBe(false);
    expect(isWindowsNamedPipePath("\\\\.\\Pipe\\rethra-design")).toBe(false);
