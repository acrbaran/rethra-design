...
- `packages/agui-adapter`: pure TypeScript adapter between persisted Rethra Design agent/GenUI/plugin-pipeline events and the AG-UI event protocol. Keep transport and filesystem concerns out; daemon producers and web/CopilotKit consumers share this conversion boundary.
...
- `packages/sidecar-proto`: Rethra Design sidecar business protocol. Owns business action names and DTO/status shapes; it does not own process identity, private transport, or lifecycle mechanics.
- `packages/sidecar`: complete business-agnostic sidecar client boundary and protocol implementation. Owns the five-field identity, private transport and endpoint derivation, resource ownership, generation fencing, and lifecycle atomics; it must not hard-code Rethra Design app keys or IPC business messages.
...
- Do not hard-code Rethra Design app/source/mode constants in `sidecar` or `platform`.
...
```bash
pnpm --filter @rethra-design/agui-adapter typecheck
pnpm --filter @rethra-design/agui-adapter test
pnpm --filter @rethra-design/contracts typecheck
pnpm --filter @rethra-design/diagnostics typecheck
pnpm --filter @rethra-design/diagnostics test
pnpm --filter @rethra-design/download typecheck
pnpm --filter @rethra-design/download test
pnpm --filter @rethra-design/host typecheck
pnpm --filter @rethra-design/host test
pnpm --filter @rethra-design/launcher-proto typecheck
pnpm --filter @rethra-design/launcher-proto test
pnpm --filter @rethra-design/metatool typecheck
pnpm --filter @rethra-design/metatool test
pnpm --filter @rethra-design/plugin-runtime typecheck
pnpm --filter @rethra-design/plugin-runtime test
pnpm --filter @rethra-design/registry-protocol typecheck
pnpm --filter @rethra-design/registry-protocol test
pnpm --filter @rethra-design/release typecheck
pnpm --filter @rethra-design/release test
pnpm --filter @rethra-design/sidecar-proto typecheck
pnpm --filter @rethra-design/sidecar-proto test
pnpm --filter @rethra-design/sidecar typecheck
pnpm --filter @rethra-design/sidecar test
pnpm --filter @rethra-design/platform typecheck
pnpm --filter @rethra-design/platform test
```
