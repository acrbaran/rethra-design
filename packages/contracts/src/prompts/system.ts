...
function imageLines(
  ...
  if (metadata.kind === 'image') {
    ...
    if (metadata.imageModel?.startsWith('vela/')) {
      out.push(
        'This RethraDesign Cloud `vela/*` model must go through the OD media dispatcher. Do not invoke the `vela` CLI or the remote media API directly; the daemon owns Workspace attribution, downloads, and final project-file placement.',
      );
    }
  }
  return out;
...
function videoLines(
  ...
  if (metadata.kind === 'video') {
    ...
    if (metadata.videoModel?.startsWith('vela/')) {
      out.push(
        'This RethraDesign Cloud `vela/*` model must go through the OD media dispatcher. Do not invoke the `vela` CLI or the remote media API directly; the daemon owns Workspace attribution, polling, downloads, and final project-file placement.',
      );
    }
