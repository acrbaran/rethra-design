...
// composeSystemPrompt from @rethra-design/contracts, so only testing the daemon
...
describe('DISCOVERY_AND_PHILOSOPHY (contracts copy) — TodoWrite plan item count', () => {
  ...
  it('uses a bare, self-contained Ask mode override that drops the discovery layer and charter', () => {
    ...
    expect(prompt).toContain('https://github.com/acrbaran/rethra-design');
    expect(prompt).toContain('https://rethra-design.invalid/');
