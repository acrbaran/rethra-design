...
export const name = 'rethra-design-runtime';
export const inject = [
  'rethraDesignStartup',
  'agentDefaultModel',
  'agents',
...
async function listModelCatalog(ctx: Context): Promise<ModelCatalogEntry[]> {
  ...
  for (const provider of ctx.llm.listProviders()) {
    try {
      ...
      ctx.logger.warn(`rethra-design-runtime: could not list models for provider "${provider.id}"`);
    }
  }
  return catalog;
}
    ...
    lines.on('line', (line) => {
      ...
      try {
        ...
        writeFrame(output, {
          ...
          message: 'RethraDesign sent an invalid profile command.',
        });
        return;
      }
...
export function apply(ctx: Context): void {
  const startup = ctx.rethraDesignStartup;
  const exit = ctx.get('appExit');
  if (!startup || !exit) throw new Error('rethra-design-runtime requires startup and appExit services');
  ...
  void ctx.get('loader')?.await().then(async () => {
    ...
    process.stderr.write(`rethra-design-runtime: ${error instanceof Error ? error.message : String(error)}\n`);
    exit(1);
  });
}
