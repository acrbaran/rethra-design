...
describe('parseMarketplace', () => {
  it('accepts a tiny catalog', () => {
    const result = parseMarketplace(JSON.stringify({
      ...
      version: '1.0.0',
      plugins: [{ name: 'make-a-deck', source: 'github:rethra-design/plugins/make-a-deck', version: '0.1.0' }],
    }));
    expect(result.ok).toBe(true);
  });
  ...
  it('rejects when plugin entry version is missing', () => {
    const result = parseMarketplace(JSON.stringify({
      ...
      version: '1.0.0',
      plugins: [{ name: 'make-a-deck', source: 'github:rethra-design/plugins/make-a-deck' }],
    }));
    expect(result.ok).toBe(false);
  });
