import { MarketplaceManifestSchema, type MarketplaceManifest } from '@rethra-design/contracts';
...
export function parseMarketplace(raw: string): MarketplaceParseResult {
  ...
  try {
    ...
    return { ok: false, errors: [`rethra-design-marketplace.json is not valid JSON: ${(err as Error).message}`] };
  }
